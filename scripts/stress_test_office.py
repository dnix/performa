...
def test_office_fundamental_sanity() -> bool:
    ...
    assert actual_total <= annual_rent * 2, f"Base rent seems too high: {actual_total} vs max expected {annual_rent * 2}"
...
def test_small_office_building() -> Dict[str, Any]:
    ...
    lease_models = [m for m in orchestrator.models if m.__class__.__name__ == 'OfficeLease']
...
def test_multi_tenant_office() -> Dict[str, Any]:
    ...
    expected_monthly_rent = sum(area * rent_psf / 12 for _, area, rent_psf, _ in tenant_info)
    ...
    assert len(expense_models) == 3, f"Expected 3 expense models, got {len(expense_models)}"
...
def test_institutional_office_complex() -> Dict[str, Any]:
    ...
    vacant_suites = [
        OfficeVacantSuite(suite="Suite 2001", floor="20", area=12000, use_type="office"),
        OfficeVacantSuite(suite="Suite 2002", floor="20", area=8000, use_type="office"),
        OfficeVacantSuite(suite="Suite 2101", floor="21", area=15000, use_type="office"),
    ]
    ...
    lease_models = [m for m in orchestrator.models if m.__class__.__name__ == 'OfficeLease']
    expense_models = [m for m in orchestrator.models if 'ExItem' in m.__class__.__name__]
    misc_models = [m for m in orchestrator.models if m.__class__.__name__ == 'OfficeMiscIncome']
    ...
    expected_monthly_rent = sum(area * rent_psf / 12 for _, area, rent_psf, _ in tenant_types)
    ...
    assert len(expense_models) == 5, f"Expected 5 expense models, got {len(expense_models)}"
...
def complex_office_stress_test():
    ...
    conservative_recovery = Recovery(
        expenses=ExpensePool(name="Conservative OpEx Pool", 
                           expenses=[base_operating, insurance]),
        structure="base_year",
        base_year=2022,
    ...
    conservative_method = OfficeRecoveryMethod(
        ...
        gross_up=True, gross_up_percent=0.95,
    ...
    standard_base_recovery = Recovery(
        expenses=ExpensePool(name="Standard Base Pool", 
                           expenses=[base_operating, utilities, insurance]),
        structure="base_year",
        base_year=2023,
    ...
    aggressive_recovery = Recovery(
        expenses=ExpensePool(name="Full Market Pool", 
    ...
    aggressive_method = OfficeRecoveryMethod(
        ...
        gross_up=True, gross_up_percent=0.98,
    ...
    property_model = OfficeProperty(
        ...
        property_type="office",
        net_rentable_area=sum(t.area for t in tenants) + sum(v.area for v in vacant_suites),
        gross_area=sum(t.area for t in tenants) + sum(v.area for v in vacant_suites) * 1.15,
        address=Address(
        ...
        expenses=OfficeExpenses(
            operating_expenses=[base_operating, utilities, taxes, insurance, management],
    ...
    lease_models = sum(1 for model in orchestrator.models if isinstance(model, OfficeLease))
    ti_models = sum(1 for model in orchestrator.models if isinstance(model, OfficeTenantImprovement))
    lc_models = sum(1 for model in orchestrator.models if isinstance(model, OfficeLeasingCommission))
    expense_models = sum(1 for model in orchestrator.models if isinstance(model, (OfficeOpExItem, OfficeCapExItem)))
    misc_models = sum(1 for model in orchestrator.models if isinstance(model, OfficeMiscIncome))
    ...
    assert len(orchestrator.models) > 50, f"Not enough models created: {len(orchestrator.models)}"  # Realistic for 43 tenants
...
def main():
    ...
    try:
        ...
        print(f"🚀 Enhanced assembler pattern handling {complex_result['tenants']} complex leases flawlessly!")
