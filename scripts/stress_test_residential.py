...
def test_small_developer_scale() -> Dict[str, Any]:
    ...
    assert abs(actual_pgr - expected_pgr) < 10, f"PGR mismatch: expected {expected_pgr}, got {actual_pgr}"
...
def test_regional_investor_scale() -> Dict[str, Any]:
    ...
    rollover_profile = create_sophisticated_rollover_profile("Regional Investor Profile")
    ...
    assert abs(actual_pgr - expected_pgr) < 100, f"PGR mismatch: expected {expected_pgr}, got {actual_pgr}"
    assert len(expense_models) == 2, f"Expected 2 expense models, got {len(expense_models)}"
...
def test_institutional_scale() -> Dict[str, Any]:
    ...
    lease_models = [m for m in orchestrator.models if m.__class__.__name__ == 'ResidentialLease']
    expense_models = [m for m in orchestrator.models if 'ExItem' in m.__class__.__name__]
    misc_models = [m for m in orchestrator.models if m.__class__.__name__ == 'ResidentialMiscIncome']
    ...
    assert abs(actual_pgr - expected_pgr) < 500, f"PGR mismatch: expected {expected_pgr}, got {actual_pgr}"
    assert actual_misc > 0, f"Misc income should be positive, got {actual_misc}"
    assert len(misc_models) == 2, f"Expected 2 misc income models, got {len(misc_models)}"
...
def test_fundamental_sanity() -> bool:
    ...
    assert isinstance(cf_result, dict), f"compute_cf should return dict, got {type(cf_result)}"
    assert "base_rent" in cf_result, "compute_cf should have base_rent component"
    assert cf_result["base_rent"].sum() == 6000.0, f"Expected $6,000 total, got {cf_result['base_rent'].sum()}"
    ...
    assert hasattr(project_result, 'columns'), "project_future_cash_flows should return DataFrame"
    ...
    profile = ResidentialRolloverProfile(
        ...
        downtime_months=1,
        term_months=12,
        market_terms=ResidentialRolloverLeaseTerms(market_rent=2000.0, capital_plan_id=None, term_months=12),
        renewal_terms=ResidentialRolloverLeaseTerms(market_rent=2000.0, capital_plan_id=None, term_months=12),
    )
    expenses = ResidentialExpenses()
...
def test_comprehensive_performance_suite() -> List[Dict[str, Any]]:
        ...
        # Small Investor Segment
        {
            ...
            "unit_specs": [
                ResidentialUnitSpec(
                    ...
                    avg_area_sf=750.0,
                    current_avg_monthly_rent=1650.0,
                    rollover_profile=create_sophisticated_rollover_profile("Small Investor Profile"),
                ),
                ResidentialUnitSpec(
                    unit_type_name="2BR/1BA",
                    unit_count=4,
        ...
        # Regional Investor Segment
        {
            ...
            "unit_specs": [
                ResidentialUnitSpec(
                    ...
                    avg_area_sf=750.0,
                    current_avg_monthly_rent=1950.0,
                    rollover_profile=create_sophisticated_rollover_profile("Regional Profile"),
                ),
                ResidentialUnitSpec(
                    unit_type_name="2BR/2BA",
                    unit_count=12,
        ...
        # Mid-size Operator Segment
        {
            ...
            "unit_specs": [
                ResidentialUnitSpec(
                    ...
                    avg_area_sf=750.0,
                    current_avg_monthly_rent=2100.0,
                    rollover_profile=create_sophisticated_rollover_profile("Mid-size Profile"),
                ),
                ResidentialUnitSpec(
                    unit_type_name="2BR/2BA",
                    unit_count=20,
        ...
        # Regional Portfolio Segment
        {
            ...
            "unit_specs": [
                ResidentialUnitSpec(
                    ...
                    avg_area_sf=550.0,
                    current_avg_monthly_rent=1750.0,
                    rollover_profile=create_sophisticated_rollover_profile("Portfolio Profile"),
                ),
                ResidentialUnitSpec(
                    unit_type_name="1BR/1BA",
                    unit_count=40,
        ...
        # Institutional Segment - Class A
        {
            ...
            "unit_specs": [
                ResidentialUnitSpec(
                    ...
                    avg_area_sf=600.0,
                    current_avg_monthly_rent=2200.0,
                    rollover_profile=create_sophisticated_rollover_profile("Class A Profile"),
                ),
                ResidentialUnitSpec(
                    unit_type_name="1BR/1BA - Standard",
                    unit_count=120,
    ...
    for scenario in test_scenarios:
        ...
        start_time = time.time()
        scenario_result = run(model=property_model, timeline=timeline, settings=settings)
        execution_time = time.time() - start_time
        ...
        # Sanity check
        assert len(lease_models) == scenario['units'], f"Expected {scenario['units']} lease models, got {len(lease_models)}"
...
def generate_performance_insights(results: List[Dict[str, Any]]) -> None:
    ...
    if small_dev:
        ...
        for result in small_dev:
            print(f"- **{result['name']} ({result['units']} units)**: {result['execution_ms']:.0f}ms → **{result['units_per_second']:.0f} units/sec**")
    ...
    if mid_market:
        ...
        for result in mid_market:
            print(f"- **{result['name']}**: {result['execution_ms']:.0f}ms → **{result['units_per_second']:.0f} units/sec**")
        print("- Real-time analysis for portfolio decisions")
    
    # Institutional Analysis
    institutional = user_segments.get('Institutional', [])
    institutional.extend(user_segments.get('Regional Portfolio', []))  # Include large regional
    
    if institutional:
        print("\n**🏙️ Institutional-Grade Performance:**")
        for result in institutional:
            if result['execution_time'] >= 1.0:
                print(f"- **{result['name']}**: {result['execution_time']:.2f}s → **{result['units_per_second']:.0f} units/sec**")
            else:
                print(f"- **{result['name']}**: {result['execution_ms']:.0f}ms → **{result['units_per_second']:.0f} units/sec**")
        max_time = max(r['execution_time'] for r in institutional)
        print(f"- Under {max_time:.1f} seconds even for large institutional properties!")
    ...
    for user_type, segment_results in user_segments.items():
        ...
        avg_time = sum(r['execution_time'] for r in segment_results) / len(segment_results)
    ...
    small_dev_avg = sum(r['units_per_second'] for r in user_segments.get('Small Developer', [])) / len(user_segments.get('Small Developer', [1]))
    institutional_avg = sum(r['units_per_second'] for r in user_segments.get('Institutional', [])) / len(user_segments.get('Institutional', [1]))
    ...
    max_institutional_time = max((r['execution_time'] for r in user_segments.get('Institutional', [])), default=0)
    max_units = max((r['units'] for r in user_segments.get('Institutional', [])), default=0)
    print(f"- Even {max_units}-unit properties complete in under {max_institutional_time:.1f}s")
    ...
    print("\n**💡 Universal Benefits:**")
    print(f"- **All scenarios under {max_time_overall:.1f} seconds** (even the largest)")
    ...
    if small_props:
        ...
        print(f"- **Small properties**: Fastest per-unit processing ({small_avg:.0f} units/sec avg - less overhead)")
    
    if large_props:
        large_avg = sum(r['units_per_second'] for r in large_props) / len(large_props)
        print(f"- **Large properties**: Slight efficiency decrease but still excellent absolute performance ({large_avg:.0f} units/sec)")
    
    min_units = min(r['units'] for r in results)
    max_units = max(r['units'] for r in results)
    print(f"- **Sweet spot**: Consistently excellent across {min_units}-{max_units} unit range")
...
def main():
    ...
    try:
        ...
        avg_performance = sum(r["units_per_second"] for r in basic_results) / len(basic_results)
