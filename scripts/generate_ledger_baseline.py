...
try:
    from office_development_comparison import (  # type: ignore[import-not-found]
        demonstrate_pattern_interface as create_office_dev_deal,
    )
    from residential_development_comparison import (  # type: ignore[import-not-found]
        create_deal_via_convention as create_residential_dev_deal,
    )
    from value_add_comparison import (  # type: ignore[import-not-found]
    ...
    try:
        ...
        if isinstance(result, tuple) and len(result) == 2:
            ...
            if hasattr(first, "name") and hasattr(first, "asset"):  # first is deal
                ...
                raise ValueError(
                    f"Could not extract deal from tuple return: {type(first)}, {type(second)}"
                )
        else:
...
def main():
    ...
    for deal_creator, deal_name in deal_configs:
        ...
        if results is not None and metrics is not None:
            ...
            normalized_ledger = normalize_ledger_for_baseline(
                results.ledger_df, deal_name
            )
    ...
    if baseline_metrics:
        ...
        avg_analysis_time = sum(m["analysis_time_ms"] for m in baseline_metrics) / len(
            baseline_metrics
        )
        ...
        for metrics in baseline_metrics:
            print(
                f"{metrics['deal_name']:.<25} {metrics['total_time_ms']:>8.1f}ms ({metrics['ledger_transactions']:>5,} txns)"
            )
        ...
        with open(metrics_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                "| Deal Type | Total Time (ms) | Analysis (ms) | Transactions | Rate (txns/ms) |\n"
            )
            f.write(
                "|-----------|----------------|---------------|-------------|---------------|\n"
            )
            ...
            for m in baseline_metrics:
                rate = (
                    m["ledger_transactions"] / m["analysis_time_ms"]
                    if m["analysis_time_ms"] > 0
                    else 0
                )
                f.write(
                    f"| {m['deal_name']} | {m['total_time_ms']:.1f} | {m['analysis_time_ms']:.1f} | {m['ledger_transactions']:,} | {rate:.2f} |\n"
                )
            ...
            f.write(
                f"- **Baseline Performance:** ~{total_transactions / avg_analysis_time:.1f} transactions per millisecond\n"
            )
            f.write(
