...
def __(mo):
    ...
    total_floors = mo.ui.slider(
        start=2, stop=20, step=1, value=8, label="❷ Number of Floors", show_value=True
    )
...
def __(mo, floor_area):
    ...
    avg_lease_size = mo.ui.slider(
        ...
        step=1_000,
        value=min(
            15_000, floor_area.value
        ),  # Default to 15k or floor area, whichever is smaller
...
def __(mo):
    ...
    lease_frequency = mo.ui.slider(
        start=1, stop=12, step=1, value=3, label="❿ Months Between", show_value=True
    )
...
def __(
    ...
    strategy_elements = [mo.md("## 🎨 Leasing Strategy"), pace_strategy]
    ...
    if "Fixed Quantity" in pace_strategy.value:
        strategy_elements.extend([
            ...
            lease_frequency,
            mo.md(
                f"📝 *Note: Total SF will be subdivided into individual leases averaging ~{avg_lease_size.value:,} SF each*"
            ),
        ])
    ...
    display_interface = mo.hstack(
        [building_section, strategy_section, terms_section],
    ...
    for floor_num in range(1, total_floors.value + 1):
        suite = OfficeVacantSuite(
            ...
            subdivision_average_lease_area=min(
                avg_lease_size.value, floor_area.value
            ),  # Average lease size when subdivided
            subdivision_minimum_lease_area=max(
                3_000, min(avg_lease_size.value, floor_area.value) // 3
            ),  # Minimum viable lease size
...
def __(
    ...
    if "Fixed Quantity" in pace_strategy.value:
        ...
        pace_description = (
            f"Lease {lease_quantity.value:,} SF every {lease_frequency.value} months"
        )
    else:  # equal_spread
        ...
        pace_description = (
            f"Spread into {total_deals.value} deals every {deal_frequency.value} months"
        )
...
def __(
    ...
    leasing_percentage = (
        (total_leased_sf / total_vacant_sf * 100) if total_vacant_sf > 0 else 0
    )

    return (
        generated_lease_specs,
        analysis_start,
        analysis_end,
        total_leased_sf,
        leasing_percentage,
    ...
    if timeline_df.empty:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                line=dict(
                    color="#e74c3c", width=4, shape="hv"
                ),  # 'hv' creates stepped line
        ...
        fig.update_layout(
            ...
            xaxis_title="Date",
            yaxis=dict(
                title="Individual Lease Area (SF)", tickformat=",.0f", side="left"
            ),
            yaxis2=dict(
...
def __(timeline_df, go, building_title, mo, pd, lease_term):
    ...
    if timeline_df.empty:
        ...
        gantt_sorted_leases = timeline_df.sort_values("Lease_Date").reset_index(
            drop=True
        )
        ...
        max_height = (
            usable_height / num_leases * 2.0
        )  # Allow larger bars to be up to 2x average
        ...
        for gantt_idx, lease_row in gantt_sorted_leases.iterrows():
            ...
            gantt_fig.add_trace(
                go.Scatter(
                    ...
                    line=dict(
                        color=f"hsl({(gantt_idx * 40) % 360}, 70%, 50%)", width=2
                    ),
        ...
        for i, (y_bottom, bar_height) in enumerate(y_positions):
            ...
            tick_labels.append(
                f"Lease {i + 1}"
            )  # Chronological order (1st, 2nd, 3rd...)
        ...
        gantt_fig.update_layout(
            ...
            yaxis=dict(
                ...
                range=[
                    -available_height * 0.05,
                    available_height * 1.05,
...
def __(timeline_df, total_vacant_sf, go, building_title, mo, pd, lease_term):
    ...
    if timeline_df.empty:
        ...
        chart_last_lease_expiration = chart_last_lease_date + pd.DateOffset(
            months=lease_term.value
        )
        ...
        occupancy_fig.update_layout(
            ...
            xaxis=dict(
                ...
                range=[
                    chart_start_date,
                    chart_last_lease_expiration,
                ],  # Match Gantt chart time domain
            ),
            showlegend=False,
        )
...
def __(timeline_df, total_vacant_sf, mo, pd):
    ...
    if timeline_df.empty:
        ...
        for _, milestone_row in milestone_sorted_leases.iterrows():
            ...
            milestone_occupancy_pct = (
                milestone_cumulative_occupied / total_vacant_sf
            ) * 100

            # Check if we've hit any milestones
            while (
                milestone_idx < len(milestone_targets)
                and milestone_occupancy_pct >= milestone_targets[milestone_idx]
            ):
                milestones.append({
        ...
        if milestones:
            milestones_output = mo.ui.table(
                data=milestones, selection=None, pagination=False
            )
        else:
...
def __(timeline_df, mo):
    ...
    if timeline_df.empty:
        ...
        table_output = mo.ui.table(
            data=table_data, selection=None, pagination=True, page_size=100
        )
...
def __(timeline_df, mo, total_vacant_sf):
    ...
    if timeline_df.empty:
        ...
        revenue_avg_rent_psf = (
            revenue_total_annual_rent / timeline_df["Area_SF"].sum()
