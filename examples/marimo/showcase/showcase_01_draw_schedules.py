...
def __(
    ...
    # Create timeline
    timeline = Timeline(
        start_date=date(2024, 1, 1), duration_months=duration_months.value
    )
    ...
    schedules = {
        ...
        "First/Last Split": FirstLastDrawSchedule(
            first_percentage=first_percentage.value
        ),
        "Custom Manual": ManualDrawSchedule(
            values=[1, 2, 4, 6, 4, 2, 1] + [1] * max(0, duration_months.value - 7)
        ),
    }
...
def __(cf_df_plot, px, total_amount):
    ...
    cf_melted = cf_df_plot.melt(
        id_vars=["Month"], var_name="Draw Schedule", value_name="Cash Flow"
    )
    ...
    fig = px.bar(
        ...
        labels={"Cash Flow": "Monthly Cash Flow ($)", "Month": "Project Month"},
