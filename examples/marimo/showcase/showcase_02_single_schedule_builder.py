...
def __(mo):
    ...
    # Budget item basics
    item_name = mo.ui.text(placeholder="Site Preparation", label="💰 Budget Item Name")
...
def __(
    ...
    right_column_elements = [mo.md("## 🎨 Schedule Pattern Selection"), schedule_type]
    ...
    if "S-Curve" in schedule_type.value:
        ...
        right_column_elements.extend([
            ...
            mo.md(
                "Enter comma-separated weights (will be normalized to your total cost):"
            ),
            manual_values,
        ])
    ...
    ui_elements = [mo.hstack([left_column, right_column], justify="space-between")]
...
def __(
    ...
    # Create timeline
    timeline = Timeline(start_date=date(2024, 1, 1), duration_months=duration)
    ...
    cash_flows = schedule.apply_to_amount(amount=amount, periods=duration)
...
def __(cf_df, item_title, amount, schedule_name, cash_flows, px, go):
    ...
    fig.update_layout(
        ...
        xaxis_title="Month",
        yaxis=dict(title="Monthly Cash Flow ($)", tickformat="$,.0f", side="left"),
        yaxis2=dict(
...
def __(cf_df, amount, cash_flows, mo):
    ...
    mo.ui.table(data=table_data, selection=None, pagination=True, page_size=12)
    return
