...
def __(mo, distribution_method):
    ...
    pref_return = mo.ui.slider(
        0.04, 0.12, 0.005, value=0.08, label="Preferred Return (8.0%)"
    )

    # First tier hurdle and promote
    tier1_hurdle = mo.ui.slider(
        0.08, 0.20, 0.005, value=0.12, label="Tier 1 IRR Hurdle (12.0%)"
    )
    tier1_promote = mo.ui.slider(
        0.0, 0.5, 0.05, value=0.20, label="Tier 1 GP Promote (20%)"
    )

    # Second tier hurdle and promote
    tier2_hurdle = mo.ui.slider(
        0.12, 0.25, 0.005, value=0.18, label="Tier 2 IRR Hurdle (18.0%)"
    )
    tier2_promote = mo.ui.slider(
        0.20, 0.60, 0.05, value=0.35, label="Tier 2 GP Promote (35%)"
    )

    # Final promote above all tiers
    final_promote = mo.ui.slider(
        0.30, 0.70, 0.05, value=0.50, label="Final GP Promote (50%)"
    )
...
@app.cell
def __(scenario_type, timeline_years, datetime, Timeline, pd):
...
def __(
    ...
    if method == "waterfall":
        tiers = [
            WaterfallTier(
                tier_hurdle_rate=tier1_hurdle.value, promote_rate=tier1_promote.value
            ),
            WaterfallTier(
                tier_hurdle_rate=tier2_hurdle.value, promote_rate=tier2_promote.value
            ),
        ]
    ...
    partnership = PartnershipStructure(
        partners=partners, distribution_method=method, promote=promote
    )
...
def __(results, mo, pd, px):
    ...
    **IRR:** **{irr_display}**
    ...
    mo.hstack(
        [
            mo.vstack([mo.md(structure_md)]),
            mo.vstack([mo.md("## 📊 Ownership"), pie_fig]),
            mo.vstack([mo.md(performance_md)]),
        ],
        widths="equal",
        gap=2,
    )
...
def __(
    ...
    fig.layout.yaxis.range = [
        0,
        35_000_000,
    ]  # Fixed range so bars move instead of axis scaling
    fig.layout.legend = dict(
        orientation="h", yanchor="bottom", y=0.95, xanchor="center", x=0.5
    )  # Legend at top under title
    ...
    for partner_name, timeline_metrics in partner_dist.items():
        ...
        for year in all_years:
            ...
            timeline_data.append({
                ...
                if amount < 0
                else ("Distribution" if amount > 0 else "Hold Period"),
    ...
    fig_timeline.layout.legend = dict(
        orientation="h", yanchor="bottom", y=0.95, xanchor="center", x=0.5
    )  # Legend at top
    fig_timeline.layout.yaxis.range = [
        -12_000_000,
        30_000_000,
    ]  # Fixed range for bar movement visibility
    ...
    controls_column = mo.vstack(
        [
            ...
            mo.hstack(
                [
                    mo.vstack([mo.md("**General Partner (GP)**"), gp_name, gp_share]),
                    mo.vstack([
            ...
            mo.vstack([mo.md("**Distribution Method**"), distribution_method]),
            # Conditional waterfall controls
            mo.vstack([
                mo.md("## 📊 Waterfall Structure")
                if "Waterfall" in distribution_method.value
                else mo.md(""),
                mo.md("---"),
                mo.vstack([
    ...
    charts_column = mo.vstack(
        [
            ...
            mo.vstack([
                scenario_type,
                mo.md(
                    "_Choose a performance scenario to see how it affects distributions_"
                ),
                timeline_years,
    ...
    mo.hstack([controls_column, charts_column], widths=[1, 1], gap=3)
