...
def main():
    ...
    try:
        ...
        if hasattr(results, "deal_metrics") and results.deal_metrics:
            if results.deal_metrics.irr is not None:
                print(f"   Deal IRR: {results.deal_metrics.irr:.2%}")
            if results.deal_metrics.equity_multiple is not None:
                print(f"   Deal Multiple: {results.deal_metrics.equity_multiple:.2f}x")
