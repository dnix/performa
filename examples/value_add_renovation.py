    ...
    **Value Creation**:
    ...
    operating_units = [
        ResidentialUnitSpec(
            ...
            lease_start_date=date(2023, 4, 1),  # Started 9 months before analysis (will expire Month 12)
        ),
        ResidentialUnitSpec(
    ...
    return ResidentialProperty(
        ...
        absorption_plans=[post_renovation_absorption],  # Premium leasing post-renovation
...
def create_acquisition_expenses() -> ResidentialExpenses:
    ...
    """
    timeline = Timeline(start_date=date(2024, 1, 1), duration_months=84)  # 7-year analysis to match script
...
def demonstrate_rolling_renovation_analysis():
    ...
    avg_current_rent = sum(spec.current_avg_monthly_rent * spec.unit_count for spec in operating_units) / total_operating_units
    ...
    print(f"   Current Market Rent: ${rollover_profile.market_terms.market_rent:,.0f}/month")
    print(f"   Target Absorption Plan: {rollover_profile.target_absorption_plan_id}")
    print(f"   Post-Renovation Rent: ${property_model.absorption_plans[0].leasing_assumptions.monthly_rent:,.0f}/month")
    ...
    avg_post_renovation_rent = property_model.absorption_plans[0].leasing_assumptions.monthly_rent
    monthly_rent_increase = avg_post_renovation_rent - avg_pre_renovation_rent
    annual_noi_increase = monthly_rent_increase * total_units * 12 * 0.85  # 85% net factor
    ...
    print(f"✓ Renovation Timeline: {total_months} months (Industry range: 18-36 months)")
    ...
    value_creation_range = f"${property_value_at_6_cap:,.0f} - ${property_value_at_5_cap:,.0f}"
    ...
    print(f"✓ Return Multiple: {return_multiple_low:.1f}x - {return_multiple_high:.1f}x on renovation investment")
    ...
    print(f"   • {rent_increase_pct:.1f}% rent increase is achievable with rolling renovation")
    print(f"   • {return_multiple_low:.1f}x-{return_multiple_high:.1f}x return multiple demonstrates value creation")
    print("   • Property-level absorption architecture correctly models rolling renovation")
    print()
    ...
    print("✅ Rolling Renovation Analysis Complete - Value-Add Financial Model Available")
    print()
    ...
    if not summary_df.empty:
        ...
        year7_data = summary_df.iloc[72:84].sum() if len(summary_df) >= 84 else summary_df.iloc[72:].sum()
        ...
        renovation_yield = noi_increase / total_renovation_cost if total_renovation_cost > 0 else 0
        ...
        return {
            ...
            'total_return': (noi_increase + value_creation) / renovation_cost if renovation_cost > 0 else 0
        }
...
def main():
    ...
    try:
        ...
        if results:
            ...
            print("• Two-pass orchestration creates both original and post-renovation leases")
            print()
            print("NEXT STEPS:")
