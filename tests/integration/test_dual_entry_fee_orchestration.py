...
class TestDualEntryFeeOrchestration:
    ...
    def developer_partner(self):
        ...
        return Partner(name="Developer", kind="GP", share=0.20)

    @pytest.fixture
    def investor_partner(self):
        """Create an investor (LP) partner."""
        return Partner(name="Investor", kind="LP", share=0.80)
    ...
    def test_dual_entry_fee_processing(
        self, mock_deal, timeline, settings, mock_levered_cash_flows
    ):
        """Test that fees are processed with dual-entry accounting."""

        # Create calculator
        calculator = DealCalculator(
            deal=mock_deal, timeline=timeline, settings=settings
        )
    ...
    def test_fee_priority_payment_logic(self, mock_deal, timeline, settings):
        ...
        cash_flows = pd.Series(
            100_000.0, index=timeline.period_index
        )  # $100k positive each period
        ...
        remaining_positive_cf = fee_details["remaining_cash_flows_after_fee"][
            fee_details["remaining_cash_flows_after_fee"] > 0
        ].sum()
    ...
    def test_payee_validation(self, timeline, settings, development_fee):
        ...
        mock_deal.deal_fees = [
            development_fee
        ]  # Fee payee is "Developer" but not in partnership
    ...
    def test_combination_logic(
        ...
        fee_details = {
            ...
            "fee_cash_flows_by_partner": {
                "Developer": pd.Series(
                    [500_000] + [20_000] * 11, index=timeline.period_index
                ),
        ...
        waterfall_results = {
            ...
            "partner_distributions": {
                "Developer": {
                    "cash_flows": pd.Series(
                        [0, 0, 0, 0, 0, 0, 200_000, 0, 0, 0, 0, 500_000],
                ...
                "Investor": {
                    "cash_flows": pd.Series(
                        [0, 0, 0, 0, 0, 0, 800_000, 0, 0, 0, 0, 2_000_000],
                        index=timeline.period_index,
                    ),
        ...
        # Combine results
        combined = calculator._combine_fee_and_waterfall_results(
    ...
    def test_multiple_fees_same_payee(self, timeline, settings):
        ...
        fee1 = DealFee.create_upfront_fee(
            name="Development Fee", value=500_000, payee=developer, timeline=timeline
        )
