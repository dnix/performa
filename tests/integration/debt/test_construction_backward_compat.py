...
def test_simple_method_unchanged():
    ...
    assert len(interest_from_sweep) == 0, "SIMPLE should NOT have synchronous 'Interest from Sweep' artifacts"
...
def test_simple_vs_scheduled_comparison():
    ...
    simple_interest = len(simple_ledger[simple_ledger['subcategory'] == 'Interest Payment'])
    scheduled_interest = len(scheduled_ledger[scheduled_ledger['subcategory'] == 'Interest Payment'])
    ...
    simple_sweeps = len(simple_ledger[simple_ledger['subcategory'] == 'Sweep Prepayment'])
    scheduled_sweeps = len(scheduled_ledger[scheduled_ledger['subcategory'] == 'Sweep Prepayment'])
