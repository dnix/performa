...
def test_comprehensive_debt_integration():
    ...
    print(f"✓ Timeline: {len(timeline.period_index)} months ({timeline.start_date} to {timeline.end_date})")
    ...
    print("✓ Permanent Facility: 5.5% fixed, auto-sizing, 24-month I/O, covenant monitoring")
    ...
    print(f"  Rate Cap Applied: {is_capped} (effective: {high_rate:.3%}, uncapped: {expected_uncapped:.3%})")
    ...
    print(f"  Timing: {transaction['transaction_date']} (month {permanent_facility.refinance_timing})")
    print(f"  From: {transaction['payoff_facility']} → To: {transaction['new_facility']}")
    ...
    most_restrictive_amount = min(ltv_loan, sizing_analysis['dscr_constraint'], debt_yield_loan)
    ...
    assert (io_schedule['Principal'] == 0).all(), "I/O periods should have zero principal payment"
    
    # Verify amortizing periods have principal > 0
    amortizing_schedule = amortization.iloc[io_periods:]
    assert (amortizing_schedule['Principal'] > 0).all(), "Amortizing periods should have principal > 0"
    ...
    print(f"  Ongoing Debt Yield Min: {covenant_monitoring['ongoing_debt_yield_min']:.1%}")
    
    # Verify covenant monitoring configuration
    assert covenant_monitoring['ongoing_ltv_max'] > 0, "Ongoing LTV should be positive"
    assert covenant_monitoring['ongoing_dscr_min'] > 0, "Ongoing DSCR should be positive"  
    assert covenant_monitoring['ongoing_debt_yield_min'] > 0, "Ongoing debt yield should be positive"
    ...
    assert abs(net_proceeds - expected_net) < 1, "Net proceeds calculation should be accurate"
    ...
    print(f"   - Rate range: {floating_rate.get_rate_for_period(periods[0], sofr_curve):.2%} to {high_rate:.2%}")
    ...
    print("Complete debt module integration validated in realistic development scenario.")
