...
class TestRefinancingOrchestrationIntegration:
    ...
    def test_refinancing_with_covenant_monitoring_setup(self):
        ...
        print(
            f"✓ Covenant status range: {covenant_results['Covenant_Status'].unique()}"
        )
...
class TestRateCapEnforcementIntegration:
    ...
    def test_rate_cap_enforcement_in_amortization(self):
        ...
        capped_rate = floating_rate.get_rate_for_period(
            timeline.period_index[0], high_sofr
        )
...
class TestFullDebtModuleIntegration:
    ...
    def test_institutional_loan_workflow(self):
        ...
        property_values = pd.Series(
            [property_value] * len(timeline.period_index), index=timeline.period_index
        )
        noi_values = pd.Series(
            [noi] * len(timeline.period_index), index=timeline.period_index
        )
