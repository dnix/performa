...
def test_debt_service_with_interest_only():
    ...
    assert (first_24_months['Principal'] == 0).all(), "First 24 months should be interest-only"
    ...
    assert (amortizing_months['Principal'] > 0).all(), "Should have principal payments after I/O period"
...
def test_construction_to_permanent_refinancing():
    ...
    assert len(refinancing_transactions) == 1, "Should generate one refinancing transaction"
...
def test_loan_covenant_monitoring():
    ...
    assert breach_summary_good['Breach_Rate'] < breach_summary_bad['Breach_Rate'], "Should detect more breaches in stressed scenario"
...
def test_interest_rate_caps():
    ...
    high_sofr = pd.Series([0.055] * len(timeline.period_index), index=timeline.period_index)  # 5.5% SOFR
