...
class TestOrchestratorTransactionBatching:
    ...
    def test_orchestrator_uses_transaction_context(self):
        ...
        assert len(transaction_calls) >= 2, "Expected at least two transactions (Phase 1 and Phase 2)"
    ...
    def test_transaction_batching_performance_benefit(self):
        ...
        assert final_count == expected_count, f"Expected {expected_count} transactions, got {final_count}"
    ...
    def test_transaction_rollback_on_error(self):
        ...
        assert len(self.ledger) == 0, "Ledger should be empty after transaction rollback"
    ...
    def test_dependency_graph_preserved_with_transactions(self):
        ...
        independent_model.calculation_pass = OrchestrationPass.INDEPENDENT_MODELS
        independent_model.reference = None
        independent_model.compute_cf.return_value = pd.Series([1000] * 12, index=self.timeline.period_index)
        ...
        dependent_model.calculation_pass = OrchestrationPass.DEPENDENT_MODELS
        dependent_model.reference = None
        dependent_model.compute_cf.return_value = pd.Series([50] * 12, index=self.timeline.period_index)
        ...
        assert len(self.ledger) > 0, "Ledger should contain data after successful execution"
        
        # Verify summary data was generated
        assert orchestrator.summary_df is not None, "Summary DataFrame should be generated"
        assert not orchestrator.summary_df.empty, "Summary DataFrame should not be empty"
...
class TestTransactionPerformanceComparison:
    ...
    def test_batched_vs_non_batched_ledger_operations(self):
        ...
        assert len(ledger1) == len(ledger2), "Both ledgers should have the same number of records"
        ...
        assert batched_time <= non_batched_time * 2.0, "Batched operations should not be significantly slower"
    ...
    def test_no_record_leakage_during_orchestrator_transaction(self):
        ...
        assert before_flush_count > 0, "Phase 1 records should be visible before flush() call"
        assert after_flush_count >= before_flush_count, "after_flush should be at least Phase 1 count"
