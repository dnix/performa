...
class TestDealResultsCore:
    ...
    def test_equity_cash_flow_drives_irr(self):
        ...
        assert results.equity_multiple is not None, "Equity multiple should be calculable"
...
class TestArchetypeDetection:
    ...
    def test_development_archetype_detection(self):
        ...
        pattern = ResidentialDevelopmentPattern(
            ...
            land_cost=1_000_000,
            total_units=40,
            unit_mix=[{"unit_type": "1BR", "count": 40, "avg_sf": 750, "target_rent": 2200}],
...
class TestCashFlowHierarchy:
    ...
    def test_smart_unlevered_cash_flow_behavior(self):
        ...
        dev_pattern = ResidentialDevelopmentPattern(
            ...
            land_cost=800_000,
            total_units=30,
            unit_mix=[{"unit_type": "1BR", "count": 30, "avg_sf": 700, "target_rent": 2000}],
    ...
    def test_universal_project_cash_flow(self):
        ...
        patterns = [
            ResidentialDevelopmentPattern(
                ...
                land_cost=600_000,
                total_units=25,
                unit_mix=[{"unit_type": "1BR", "count": 25, "avg_sf": 650, "target_rent": 1800}],
        ...
        for pattern in patterns:
            ...
            if results.archetype == "Development":
                ...
                construction_periods = results.unlevered_cash_flow.iloc[:12]  # First 12 months
                ...
                acquisition_costs = results.queries.ledger[
                    results.queries.ledger["subcategory"].isin(["Purchase Price", "Closing Costs", "Transaction Costs"])
                ]["amount"].sum()
...
class TestValidationFramework:
    ...
    def test_validation_catches_development_issues(self):
        ...
        pattern = ResidentialDevelopmentPattern(
            ...
            land_cost=400_000,
            total_units=15,
            unit_mix=[{"unit_type": "1BR", "count": 15, "avg_sf": 600, "target_rent": 1600}],
        ...
        from performa.reporting.debug import validate_flow_reasonableness
        ...
        assert financing.get('loan_proceeds', 0) > 0, "Should have construction financing"
        
        # Should calculate meaningful metrics
        overall = validation['overall_analysis']
        assert overall.get('irr') is not None, "Should calculate IRR for development deal"
        assert overall.get('equity_multiple') is not None, "Should calculate equity multiple"
        
        print(f"Development validation completed - IRR: {overall.get('irr', 'N/A')}, EM: {overall.get('equity_multiple', 'N/A')}")
    ...
    def test_validation_return_metrics_bounds(self):
        ...
        assert 'equity_multiple' in validation['overall_analysis'], "Should analyze equity multiple"
        
        # Validate basic ledger consistency
        assert 'balance_checks' in ledger_analysis, "Should include balance checks"
        assert 'total_net_flow' in ledger_analysis['balance_checks'], "Should calculate net flow"
        ...
        print(f"Ledger net flow: ${ledger_analysis['balance_checks']['total_net_flow']:,.0f}")
