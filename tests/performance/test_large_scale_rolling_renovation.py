...
class TestLargeScaleRollingRenovation:
    ...
    def _create_value_add_property(self, unit_count: int, name_suffix: str = ""):
        ...
        absorption_plan = ResidentialAbsorptionPlan(
            ...
            pace=FixedQuantityPace(quantity=min(unit_count, 10), unit="Units", frequency_months=1),
    ...
    def _measure_performance(self, unit_count: int):
        ...
        property_model = self._create_value_add_property(unit_count, f"Scale-{unit_count}")
    ...
    def test_scale_performance(self, unit_count):
        ...
        assert metrics['total_duration'] < 60.0, f"Analysis took too long: {metrics['total_duration']:.2f}s"
        assert metrics['memory_delta_mb'] < 500.0, f"Memory usage too high: {metrics['memory_delta_mb']:.1f} MB"
        ...
        assert 0.7 <= revenue_ratio <= 1.1, f"Revenue seems incorrect: {revenue_ratio:.2%} of expected"
    ...
    def test_rolling_completion_rate_at_scale(self):
        ...
        timeline = Timeline(start_date=date(2024, 1, 1), duration_months=60)  # Longer timeline
        settings = GlobalSettings()
...
class TestScalabilityLimits:
    ...
    def test_many_absorption_plans(self):
        ...
        property_model = ResidentialProperty(
            ...
            address=Address(street="123 MultiPlan St", city="Test", state="CA", zip_code="90210", country="USA"),
            gross_area=40000.0,
    ...
    def test_memory_efficiency(self):
        ...
        assert memory_growth < 100.0, f"Possible memory leak: {memory_growth:.1f} MB growth"
    ...
    def _create_value_add_property(self, unit_count: int, name_suffix: str = ""):
        ...
        return ResidentialProperty(
            ...
            address=Address(street=f"123 Memory St {name_suffix}", city="Test", state="CA", zip_code="90210", country="USA"),
            gross_area=float(unit_count * 800),
            net_rentable_area=float(unit_count * 800),
            unit_mix=ResidentialRentRoll(unit_specs=unit_specs),
            capital_plans=[],
