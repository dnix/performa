...
class TestLedgerBasicFunctionality:
    ...
    def test_ledger_is_created(self, simple_residential_property, analysis_timeline, global_settings):
        ...
        required_columns = ['date', 'amount', 'flow_purpose', 'category', 'subcategory', 'item_name']
        for col in required_columns:
            assert col in ledger_df.columns, f"Missing required column: {col}"
    
    def test_ledger_has_transactions(self, simple_residential_property, analysis_timeline, global_settings):
    ...
    def test_ledger_dates_align_with_timeline(self, simple_residential_property, analysis_timeline, global_settings):
...
class TestLedgerAccuracy:
    ...
    def test_ledger_noi_matches_summary(self, simple_residential_property, analysis_timeline, global_settings):
        ...
        revenue_by_date = revenue_flows.groupby('date')['amount'].sum() if not revenue_flows.empty else pd.Series(dtype=float)
        expense_by_date = expense_flows.groupby('date')['amount'].sum() if not expense_flows.empty else pd.Series(dtype=float)
        
        # Align indices and calculate NOI
        all_dates = revenue_by_date.index.union(expense_by_date.index) if not revenue_by_date.empty or not expense_by_date.empty else pd.Index([])
        ...
        ledger_noi = revenue_aligned + expense_aligned  # Expenses are now negative, so addition gives subtraction
        ...
        for date in ledger_noi.index[:3]:  # Check first 3 months
            ...
            assert abs(ledger_value - result_value) < 0.01, f"NOI mismatch for {date}: ledger={ledger_value}, result={result_value}"
    
    def test_revenue_transactions_make_sense(self, simple_residential_property, analysis_timeline, global_settings):
        ...
        lease_revenue = revenue_transactions[revenue_transactions['subcategory'] == 'Lease']
        ...
        assert 15000 <= first_month_revenue <= 25000, f"Unexpected first month revenue: {first_month_revenue}"


class TestLedgerMetadata:
    """Test that ledger metadata is populated correctly."""
    
    def test_transactions_have_source_ids(self, simple_residential_property, analysis_timeline, global_settings):
        ...
        assert len(transactions_with_ids) == len(ledger_df), "All transactions should have source IDs"
    
    def test_transactions_have_pass_numbers(self, simple_residential_property, analysis_timeline, global_settings):
    ...
    def test_asset_id_is_consistent(self, simple_residential_property, analysis_timeline, global_settings):
        ...
        assert len(unique_asset_ids) == 1, f"Expected 1 asset ID, got {len(unique_asset_ids)}"
...
class TestLedgerQuerying:
    ...
    def test_ledger_query_object_works(self, simple_residential_property, analysis_timeline, global_settings):
    ...
    def test_can_filter_by_transaction_type(self, simple_residential_property, analysis_timeline, global_settings):
        ...
        assert len(revenue_only) + len(expense_only) <= len(ledger_df)  # <= because there might be other categories
