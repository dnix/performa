...
class TestAcquisitionTermsInstantiation:
    ...
    def test_validation_error_missing_acquisition_date(self):
        ...
        with pytest.raises(
            ValueError,
            match="'acquisition_date' is required when 'value' is a single number",
        ):
            AcquisitionTerms(
                name="Test",
                timeline=timeline,
    ...
    def test_validation_error_acquisition_date_with_dict_value(self):
        ...
        with pytest.raises(
            ValueError,
            match="'acquisition_date' must not be provided when 'value' is a Series or dict",
        ):
            AcquisitionTerms(
                name="Test",
                timeline=timeline,
...
class TestAcquisitionTermsCashFlowComputation:
    ...
    def test_payment_outside_timeline(self, mock_context):
        ...
        timeline = Timeline(
            start_date=date(2024, 3, 1), duration_months=6
        )  # Mar-Aug 2024
...
class TestAcquisitionTermsEdgeCases:
    ...
    def test_compute_cf_error_handling(self):
        ...
        acquisition = AcquisitionTerms(
            ...
            timeline=timeline,
            value={
                pd.Period("2024-01", freq="M"): 1_000_000
            },  # Dict to bypass validation
            closing_costs_rate=0.02,
        )
