...
class TestEnhancedResultsModel:
    ...
    def test_enhanced_fee_accounting_details_fields(
        self, mock_deal, timeline, settings
    ):
        ...
        assert fee_details.total_fees_by_type["Asset Management"] == pytest.approx(
            120000, rel=1e-6
        )
    ...
    def test_backward_compatibility(self, mock_deal, timeline, settings):
        ...
        positive_cash_flows = pd.Series(
            [0, 0, 0, 50000, 50000, 50000] + [0] * 30, index=timeline.period_index
        )
        ...
        for partner_name, partner_result in partner_results.items():
            ...
            assert (
                partner_result.developer_fee == partner_result.distributions_from_fees
            )
    ...
    def test_zero_fees_handling(self, partners, timeline, settings):
        ...
        for partner_name, partner_result in partner_results.items():
            ...
            assert (
                partner_result.distributions_from_waterfall > 0
            )  # Should have waterfall
            assert (
        ...
        assert all(
            len(fees) == 0 for fees in fee_details.fee_details_by_partner.values()
        )
    ...
    def test_results_model_validation(self, mock_deal, timeline, settings):
        ...
        assert isinstance(
            calculator.partner_distributions.fee_accounting_details,
            FeeAccountingDetails,
        )
    ...
    def test_complex_fee_structure(self, partners, timeline, settings):
        ...
        positive_cash_flows = pd.Series(
            [0, 0, 0] + [200000] * 12 + [0] * 21, index=timeline.period_index
        )
