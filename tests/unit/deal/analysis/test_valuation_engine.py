...
def mock_deal_with_exit_valuation() -> Deal:
    ...
    exit_valuation.compute_cf = Mock(return_value=pd.Series([0.0, 0.0, 0.0, 3500000.0] * 6))  # Disposition in last month
...
@pytest.fixture
def sample_unlevered_analysis_with_noi(sample_timeline: Timeline) -> UnleveredAnalysisResult:
    ...
    cash_flows = pd.DataFrame({
        ...
        UnleveredAggregateLineKey.TOTAL_OPERATING_EXPENSES.value: [40000.0] * 12 + [35000.0] * 12,
...
@pytest.fixture
def sample_unlevered_analysis_with_values(sample_timeline: Timeline) -> UnleveredAnalysisResult:
...
class TestValuationEngineBasic:
    ...
    def test_valuation_engine_can_be_instantiated(self, mock_deal_basic, sample_timeline, sample_settings):
        """Test that ValuationEngine can be instantiated with basic parameters."""
        engine = ValuationEngine(deal=mock_deal_basic, timeline=sample_timeline, settings=sample_settings)
    ...
    def test_valuation_engine_has_required_methods(self, mock_deal_basic, sample_timeline, sample_settings):
...
class TestPropertyValueExtraction:
    ...
    def test_extract_property_value_direct_from_cash_flows(self, mock_deal_basic, sample_timeline, sample_settings, sample_unlevered_analysis_with_values):
        """Test Strategy 1: Direct extraction from cash flow data."""
        engine = ValuationEngine(deal=mock_deal_basic, timeline=sample_timeline, settings=sample_settings)
        
        property_values = engine.extract_property_value_series(sample_unlevered_analysis_with_values)
        ...
        assert property_values.iloc[-1] > property_values.iloc[0]  # Appreciation over time
        
        # Should cache the result
        assert engine.property_value_series is not None
        assert engine.property_value_series.equals(property_values)
    
    def test_extract_property_value_noi_based_default_cap_rate(self, mock_deal_basic, sample_timeline, sample_settings, sample_unlevered_analysis_with_noi):
        """Test Strategy 2: NOI-based valuation with default cap rate."""
        engine = ValuationEngine(deal=mock_deal_basic, timeline=sample_timeline, settings=sample_settings)
        
        property_values = engine.extract_property_value_series(sample_unlevered_analysis_with_noi)
    ...
    def test_extract_property_value_noi_based_deal_cap_rate(self, mock_deal_with_exit_valuation, sample_timeline, sample_settings, sample_unlevered_analysis_with_noi):
        """Test Strategy 2: NOI-based valuation with deal-specific cap rate."""
        engine = ValuationEngine(deal=mock_deal_with_exit_valuation, timeline=sample_timeline, settings=sample_settings)
        ...
        assert property_values.iloc[0] > 80000.0 / 0.065  # Higher than default cap rate would give
    
    def test_extract_property_value_cost_based_appreciation(self, mock_deal_with_acquisition, sample_timeline, sample_settings, empty_unlevered_analysis):
        """Test Strategy 3: Cost-based appreciation using acquisition cost."""
        engine = ValuationEngine(deal=mock_deal_with_acquisition, timeline=sample_timeline, settings=sample_settings)
        ...
        assert abs(property_values.iloc[-1] - expected_final_value) < 6000.0  # Slightly looser tolerance
        
        # Should show gradual appreciation
        assert property_values.iloc[-1] > property_values.iloc[0]
    
    def test_extract_property_value_zero_fallback(self, mock_deal_basic, sample_timeline, sample_settings, empty_unlevered_analysis):
    ...
    def test_extract_property_value_noi_error_fallback(self, mock_deal_with_acquisition, sample_timeline, sample_settings):
        ...
        assert abs(property_values.iloc[0] - 2000000.0) < 1000.0  # Should use acquisition cost


class TestNOIExtraction:
    """Test NOI extraction using type-safe enum access."""
    
    def test_extract_noi_series_success(self, mock_deal_basic, sample_timeline, sample_settings, sample_unlevered_analysis_with_noi):
    ...
    def test_extract_noi_series_with_missing_data(self, mock_deal_basic, sample_timeline, sample_settings):
    ...
    def test_extract_noi_series_multiple_calls_consistent(self, mock_deal_basic, sample_timeline, sample_settings, sample_unlevered_analysis_with_noi):
...
class TestDispositionProceeds:
    ...
    def test_calculate_disposition_proceeds_with_exit_valuation(self, mock_deal_with_exit_valuation, sample_timeline, sample_settings, sample_unlevered_analysis_with_noi):
        """Test disposition proceeds calculation with exit valuation model."""
        engine = ValuationEngine(deal=mock_deal_with_exit_valuation, timeline=sample_timeline, settings=sample_settings)
        
        # Mock the compute_cf method to return disposition proceeds
        disposition_cf = pd.Series([0.0] * 23 + [3500000.0], index=sample_timeline.period_index)
        mock_deal_with_exit_valuation.exit_valuation.compute_cf.return_value = disposition_cf
        ...
        with patch('performa.analysis.AnalysisContext') as mock_context_class:
            ...
            proceeds = engine.calculate_disposition_proceeds(sample_unlevered_analysis_with_noi)
    ...
    def test_calculate_disposition_proceeds_no_exit_valuation(self, mock_deal_basic, sample_timeline, sample_settings, sample_unlevered_analysis_with_noi):
    ...
    def test_calculate_disposition_proceeds_without_unlevered_analysis(self, mock_deal_with_exit_valuation, sample_timeline, sample_settings):
        """Test disposition proceeds calculation without unlevered analysis."""
        engine = ValuationEngine(deal=mock_deal_with_exit_valuation, timeline=sample_timeline, settings=sample_settings)
        
        # Mock the compute_cf method
        disposition_cf = pd.Series([0.0] * 23 + [4000000.0], index=sample_timeline.period_index)
        mock_deal_with_exit_valuation.exit_valuation.compute_cf.return_value = disposition_cf
    ...
    def test_calculate_disposition_proceeds_error_handling(self, mock_deal_with_exit_valuation, sample_timeline, sample_settings, sample_unlevered_analysis_with_noi):
        """Test error handling in disposition proceeds calculation."""
        engine = ValuationEngine(deal=mock_deal_with_exit_valuation, timeline=sample_timeline, settings=sample_settings)
        
        # Make the exit valuation raise an error
        mock_deal_with_exit_valuation.exit_valuation.compute_cf.side_effect = Exception("Valuation failed")
    ...
    def test_calculate_disposition_proceeds_negative_values_converted(self, mock_deal_with_exit_valuation, sample_timeline, sample_settings):
        ...
        disposition_cf = pd.Series([0.0] * 23 + [-2500000.0], index=sample_timeline.period_index)
        mock_deal_with_exit_valuation.exit_valuation.compute_cf.return_value = disposition_cf
...
class TestIntegrationScenarios:
    ...
    def test_complete_valuation_workflow(self, mock_deal_with_exit_valuation, sample_timeline, sample_settings, sample_unlevered_analysis_with_noi):
        ...
        with patch('performa.analysis.AnalysisContext') as mock_context_class:
            ...
            disposition_cf = pd.Series([0.0] * 23 + [3000000.0], index=sample_timeline.period_index)
            mock_deal_with_exit_valuation.exit_valuation.compute_cf.return_value = disposition_cf
            
            disposition_proceeds = engine.calculate_disposition_proceeds(sample_unlevered_analysis_with_noi)
    ...
    def test_valuation_with_growth_scenario(self, mock_deal_with_exit_valuation, sample_timeline, sample_settings):
...
class TestEdgeCasesAndErrorHandling:
    ...
    def test_extract_property_value_zero_noi(self, mock_deal_basic, sample_timeline, sample_settings):
    ...
    def test_extract_property_value_negative_noi(self, mock_deal_basic, sample_timeline, sample_settings):
    ...
    def test_extract_property_value_with_partial_data(self, mock_deal_with_acquisition, sample_timeline, sample_settings):
        """Test property value extraction with partial/incomplete data."""
        # Create analysis with some NOI but not enough for valuation
        cash_flows = pd.DataFrame({
            UnleveredAggregateLineKey.NET_OPERATING_INCOME.value: [10000.0] * 6 + [0.0] * 18,  # Only first 6 months
        }, index=sample_timeline.period_index)
    ...
    def test_extract_property_value_noi_get_series_exception(self, mock_deal_with_acquisition, sample_timeline, sample_settings):
        ...
        assert abs(property_values.iloc[0] - 2000000.0) < 1000.0  # Should use acquisition cost
        assert property_values.iloc[-1] > property_values.iloc[0]  # Should show appreciation
    
    def test_disposition_proceeds_context_creation_failure(self, mock_deal_with_exit_valuation, sample_timeline, sample_settings):
        """Test disposition proceeds when context creation fails."""
        engine = ValuationEngine(deal=mock_deal_with_exit_valuation, timeline=sample_timeline, settings=sample_settings)
        
        # Mock AnalysisContext to raise an error
        with patch('performa.analysis.AnalysisContext', side_effect=Exception("Context creation failed")):
            proceeds = engine.calculate_disposition_proceeds()
    ...
    def test_disposition_proceeds_noi_extraction_exception(self, mock_deal_with_exit_valuation, sample_timeline, sample_settings):
    ...
    def test_multiple_value_columns_uses_first(self, mock_deal_basic, sample_timeline, sample_settings):
