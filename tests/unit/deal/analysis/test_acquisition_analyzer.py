...
def create_sample_context(
    timeline: Timeline, settings: GlobalSettings, ledger: Ledger, deal: Mock
) -> DealContext:
    """Helper to create DealContext for testing."""
    return DealContext(timeline=timeline, settings=settings, ledger=ledger, deal=deal)
...
class TestAcquisitionAnalyzerBasics:
    ...
    def test_acquisition_analyzer_instantiation(
        ...
        context = create_sample_context(
            sample_timeline, sample_settings, sample_ledger, sample_deal
        )
...
class TestDealFeeProcessing:
    ...
    def test_add_deal_fees_multiple_types(
        ...
        fee_records = ledger_df[ledger_df["category"] == CashFlowCategoryEnum.CAPITAL]
...
class TestEdgeCasesAndErrorHandling:
    ...
    def test_process_no_acquisition_no_renovation(
        ...
        context = create_sample_context(
            sample_timeline, sample_settings, sample_ledger, deal
        )
        analyzer = AcquisitionAnalyzer(context)
