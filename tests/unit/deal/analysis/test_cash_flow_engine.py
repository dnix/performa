...
def mock_deal() -> Deal:
    ...
    mock_capital_item.compute_cf = Mock(return_value=pd.Series([50000.0] * 24 + [25000.0] * 12, 
...
def sample_unlevered_analysis(sample_timeline: Timeline) -> UnleveredAnalysisResult:
    ...
    cash_flows = pd.DataFrame({
        ...
        "Total_Uses": [100000.0] * 24 + [10000.0] * 12,  # High uses during construction
...
def sample_financing_analysis(sample_timeline: Timeline) -> FinancingAnalysisResult:
    ...
    result.facilities = [
        FacilityInfo(name="Construction Loan", type="ConstructionFacility", description="Primary construction financing")
    ]
    result.debt_service = {
        "Construction Loan": pd.Series([0.0] * 24 + [15000.0] * 12, index=sample_timeline.period_index)
    }
    result.loan_proceeds = {
        "Construction Loan": pd.Series([300000.0] + [0.0] * 35, index=sample_timeline.period_index)
    }
    return result
...
class TestCashFlowEngine:
    ...
    def test_cash_flow_engine_instantiation(self, mock_deal, sample_timeline, sample_settings):
        """Test that CashFlowEngine can be instantiated."""
        engine = CashFlowEngine(deal=mock_deal, timeline=sample_timeline, settings=sample_settings)
    ...
    def test_all_equity_funding(self, mock_deal, sample_timeline, sample_settings, sample_unlevered_analysis, no_financing_analysis):
        """Assert equity_contributions exactly equals total_uses."""
        engine = CashFlowEngine(deal=mock_deal, timeline=sample_timeline, settings=sample_settings)
        
        # Create disposition proceeds
        disposition_proceeds = pd.Series([0.0] * 35 + [3000000.0], index=sample_timeline.period_index)
        ...
        # Should have no debt
        total_debt = result.cash_flow_components.loan_proceeds.sum()  # Changed from debt_proceeds to loan_proceeds
    ...
    def test_leveraged_funding_with_no_interest(self, mock_deal, sample_timeline, sample_settings):
        ...
        zero_interest_financing.facilities = [
            FacilityInfo(name="Zero Interest Loan", type="ConstructionFacility", description="Zero interest loan")
        ]
        zero_interest_financing.debt_service = {
            "Zero Interest Loan": pd.Series([0.0] * 36, index=sample_timeline.period_index)  # No interest payments
        }
        zero_interest_financing.loan_proceeds = {
            "Zero Interest Loan": pd.Series([500000.0] + [0.0] * 35, index=sample_timeline.period_index)
        }
        
        # Create simple unlevered analysis
        simple_cash_flows = pd.DataFrame({
            UnleveredAggregateLineKey.NET_OPERATING_INCOME.value: [0.0] * 24 + [40000.0] * 12,
        ...
        disposition_proceeds = pd.Series([0.0] * 35 + [2500000.0], index=sample_timeline.period_index)
        ...
        assert abs((total_equity + total_debt) - total_uses) < 10.0, "Equity + Debt should equal Uses in zero-interest scenario"
    
    def test_interest_compounding(self, mock_deal, sample_timeline, sample_settings, sample_unlevered_analysis, sample_financing_analysis):
        ...
        disposition_proceeds = pd.Series([0.0] * 35 + [3500000.0], index=sample_timeline.period_index)
    ...
    def test_interest_reserve_funding(self, mock_deal, sample_timeline, sample_settings):
        ...
        reserve_capacity = engine._calculate_interest_reserve_capacity(total_project_cost)
        ...
        if reserve_capacity > 0:
            assert reserve_capacity <= total_project_cost * 0.1, "Reserve capacity should be reasonable percentage"
    
    def test_multi_tranche_debt_logic(self, mock_deal, sample_timeline, sample_settings):
    ...
    def test_equity_cumulative_tracking(self, mock_deal, sample_timeline, sample_settings):
        ...
        for cost in total_project_cost_scenarios:
            ...
            assert 0.1 <= equity_percentage <= 1.0, "Equity percentage should be reasonable"


class TestFundingCascadeLogic:
    """Specific tests for the funding cascade implementation."""
    
    def test_period_by_period_funding(self, mock_deal, sample_timeline, sample_settings, sample_unlevered_analysis):
        ...
        assert isinstance(period_uses_df, pd.DataFrame), "Period uses should be a DataFrame"
        assert len(period_uses_df) == len(sample_timeline.period_index), "Should have entry for each period"
        assert 'Acquisition Costs' in period_uses_df.columns, "Should include acquisition costs"
        assert 'Construction Costs' in period_uses_df.columns, "Should include construction costs"
    ...
    def test_funding_components_initialization(self, mock_deal, sample_timeline, sample_settings):
        ...
        for key in required_keys:
            ...
            assert isinstance(components[key], pd.Series), f"{key} should be a pandas Series"
            assert len(components[key]) == len(sample_timeline.period_index), f"{key} should have correct length"
    
    def test_tranche_tracking_initialization(self, mock_deal, sample_timeline, sample_settings):
...
class TestCashFlowStructure:
    ...
    def test_unlevered_cash_flows_extraction(self, mock_deal, sample_timeline, sample_settings, sample_unlevered_analysis):
        ...
        assert isinstance(unlevered_cf, pd.Series), "Unlevered cash flows should be a Series"
        assert len(unlevered_cf) == len(sample_timeline.period_index), "Should have correct length"
        
        # Should have positive cash flows in operational periods
        operational_periods = unlevered_cf.iloc[24:]  # Last 12 months
        assert operational_periods.sum() > 0, "Should have positive operational cash flows"
    
    def test_debt_service_calculation(self, mock_deal, sample_timeline, sample_settings, sample_financing_analysis):
        """Test debt service series calculation."""
        engine = CashFlowEngine(deal=mock_deal, timeline=sample_timeline, settings=sample_settings)
        
        debt_service_series = engine._calculate_debt_service_series(sample_financing_analysis)
        
        assert isinstance(debt_service_series, pd.Series), "Debt service should be a Series"
        assert len(debt_service_series) == len(sample_timeline.period_index), "Should have correct length"
        ...
        assert all(debt_service_series >= 0), "All debt service values should be non-negative"
    
    def test_loan_payoff_calculation(self, mock_deal, sample_timeline, sample_settings, sample_financing_analysis):
        ...
        assert len(payoff_series) == len(sample_timeline.period_index), "Should have correct length"
        
        # Payoff should be zero or positive
        assert (payoff_series >= 0).all(), "Payoff amounts should be non-negative"
    
    def test_disposition_proceeds_calculation(self, mock_deal, sample_timeline, sample_settings):
        ...
        assert isinstance(disposition_proceeds, pd.Series), "Disposition proceeds should be a Series"
        assert len(disposition_proceeds) == len(sample_timeline.period_index), "Should have correct length"
        assert (disposition_proceeds >= 0).all(), "Disposition proceeds should be non-negative"


class TestFundingCascadeExecution:
    """Tests for the core funding cascade execution logic."""
    
    def test_funding_cascade_execution(self, mock_deal, sample_timeline, sample_settings):
        """Test funding cascade execution with various scenarios."""
        engine = CashFlowEngine(deal=mock_deal, timeline=sample_timeline, settings=sample_settings)
        
        # Create base uses series
        base_uses = pd.Series([100000.0] * 24 + [50000.0] * 12, index=sample_timeline.period_index)
    ...
    def test_period_funding_logic(self, mock_deal, sample_timeline, sample_settings):
        ...
        assert total_funding <= period_uses + 1.0, "Total funding should not significantly exceed period uses"


class TestInterestCalculations:
    """Tests for interest calculation logic."""
    
    def test_interest_details_compilation(self, mock_deal, sample_timeline, sample_settings):
        """Test interest details compilation."""
        engine = CashFlowEngine(deal=mock_deal, timeline=sample_timeline, settings=sample_settings)
        
        # Create mock funding components with interest data
        funding_components = {
            'interest_expense': pd.Series([5000.0] * 36, index=sample_timeline.period_index),
            'equity_contributions': pd.Series([50000.0] * 36, index=sample_timeline.period_index),
            'loan_proceeds': pd.Series([30000.0] * 36, index=sample_timeline.period_index),
        ...
        assert isinstance(interest_details, dict), "Interest details should be a dictionary"
        ...
        for key in expected_keys:
            if key in interest_details:
                assert isinstance(interest_details[key], pd.Series), f"{key} should be a pandas Series"
...
class TestErrorHandling:
    ...
    def test_empty_unlevered_analysis(self, mock_deal, sample_timeline, sample_settings, no_financing_analysis):
        ...
        try:
            ...
            assert isinstance(result, LeveredCashFlowResult), "Should return valid result structure"
        except Exception:
...
class TestCashFlowEngineIntegration:
    ...
    def test_full_integration_scenario(self, mock_deal, sample_timeline, sample_settings, sample_unlevered_analysis, sample_financing_analysis):
        """Test complete integration scenario with realistic data."""
        engine = CashFlowEngine(deal=mock_deal, timeline=sample_timeline, settings=sample_settings)
        
        # Create realistic disposition proceeds
        disposition_proceeds = pd.Series([0.0] * 35 + [4000000.0], index=sample_timeline.period_index)
        ...
        assert isinstance(result, LeveredCashFlowResult), "Should return LeveredCashFlowResult"
        assert hasattr(result, 'cash_flow_components'), "Should have cash flow components"
        assert hasattr(result, 'cash_flow_summary'), "Should have cash flow summary"
        assert hasattr(result, 'funding_cascade_details'), "Should have funding cascade details"
        
        # Verify cash flow components
        components = result.cash_flow_components
        assert isinstance(components, CashFlowComponents), "Should have proper components structure"
        
        # Verify all series have correct length
        assert len(components.equity_contributions) == 36, "Equity contributions should have correct length"
        assert len(components.loan_proceeds) == 36, "Debt proceeds should have correct length"
        assert len(components.total_uses) == 36, "Total uses should have correct length"
        assert len(components.unlevered_cash_flows) == 36, "Unlevered cash flows should have correct length"
    ...
    def test_with_debt_analyzer_results(self, mock_deal, sample_timeline, sample_settings, sample_unlevered_analysis, sample_financing_analysis):
        ...
        assert result.cash_flow_components.debt_service is not None, "Should have debt service component"
        assert isinstance(result.cash_flow_components.debt_service, pd.Series), "Debt service should be a series"
        
        # Basic structure verification
        assert len(result.cash_flow_components.debt_service) == 36, "Should have correct length"
    
    def test_with_asset_analyzer_results(self, mock_deal, sample_timeline, sample_settings, sample_unlevered_analysis, no_financing_analysis):
        ...
        assert operational_cf > 0, "Should extract positive operational cash flows from unlevered analysis"
        
        # Total distributions should include disposition proceeds
        total_distributions = engine._calculate_total_distributions(sample_unlevered_analysis, disposition_proceeds)
        assert total_distributions > 3000000.0, "Total distributions should include disposition proceeds"
