...
class TestUnleveredAnalysisResultGetSeries:
    ...
    def test_get_series_alignment_with_different_timeline(self):
        ...
        noi_series = result.get_series(
            UnleveredAggregateLineKey.NET_OPERATING_INCOME, target_timeline
        )
...
class TestDealCalculatorDataAccess:
    ...
    def test_extract_noi_series_uses_get_series(self):
        ...
        # Create calculator with mocked dependencies
        calculator = DealCalculator(deal=Mock(), timeline=timeline, settings=Mock())
...
class TestRobustnessImprovements:
    ...
    def test_resilient_to_column_name_changes(self):
        ...
        noi_exact = result_exact.get_series(
            UnleveredAggregateLineKey.NET_OPERATING_INCOME, timeline
        )
        ...
        noi_different = result_different.get_series(
            UnleveredAggregateLineKey.NET_OPERATING_INCOME, timeline
        )
    ...
    def test_performance_benefit_of_direct_access(self):
        ...
        columns = {"Column_" + str(i): [i] * 12 for i in range(100)}
    ...
    cash_flows = pd.DataFrame(
        {
            ...
            UnleveredAggregateLineKey.EFFECTIVE_GROSS_INCOME.value: [
                1600 + i * 100 for i in range(12)
            ],
        },
        index=timeline.period_index,
    )
