...
class TestDealFee:
    ...
    def test_different_partner_types(self, sample_timeline):
        ...
        gp_fee = DealFee.create_upfront_fee(
            name="GP Fee", value=100000, payee=gp_partner, timeline=sample_timeline
        )

        lp_fee = DealFee.create_upfront_fee(
            name="LP Fee", value=50000, payee=lp_partner, timeline=sample_timeline
        )
    ...
    def test_factory_methods_require_payee(self, sample_timeline):
        ...
        with pytest.raises(
            TypeError, match="missing 1 required positional argument: 'payee'"
        ):
