...
class TestIndustryStandardWaterfallValidation:
    ...
    def test_simple_institutional_waterfall(self):
        ...
        annual_distributions = [120_000, 130_000, 140_000, 150_000, 1_200_000]  # Last year includes sale
        ...
        print(f"   GP promote achieved: {gp_distribution_ratio:.1%} vs {gp_contribution_ratio:.1%} contribution")
    ...
    def test_performance_benchmark_validation(self):
        ...
        expected_lp_irr_range = (0.06, 0.15)  # 6-15% range (realistic for moderate performance)
        assert expected_lp_irr_range[0] <= lp_result["irr"] <= expected_lp_irr_range[1], \
        ...
        expected_gp_irr_range = (0.06, 0.30)  # 6-30% range (realistic for varying performance)
        assert expected_gp_irr_range[0] <= gp_result["irr"] <= expected_gp_irr_range[1], \
        ...
        print(f"   LP Performance: {lp_result['irr']:.2%} IRR, {lp_result['equity_multiple']:.2f}x")
        print(f"   GP Performance: {gp_result['irr']:.2%} IRR, {gp_result['equity_multiple']:.2f}x")
