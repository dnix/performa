...
class TestEntityModels:
    ...
    def test_partner_creation(self):
        ...
        partner = Partner(
            name="Developer LLC", kind="GP", share=0.25, description="Lead developer"
        )
    ...
    def test_third_party_creation(self):
        ...
        third_party = ThirdParty(
            name="Smith Architecture", description="Architectural design services"
        )
...
class TestThirdPartyFees:
    ...
    def developer_partner(self):
        ...
        return Partner(name="ABC Development", kind="GP", share=0.20)
    ...
    def test_fee_string_representation(
        self, timeline, developer_partner, architect_third_party
    ):
        """Test fee string representation with different entity types."""
        partner_fee = DealFee.create_upfront_fee(
            name="Dev Fee", value=500_000, payee=developer_partner, timeline=timeline
        )
    ...
    def test_factory_methods_with_entities(self, timeline):
        ...
        upfront_partner = DealFee.create_upfront_fee(
            "Upfront", 100_000, partner, timeline
        )
        completion_partner = DealFee.create_completion_fee(
            "Completion", 200_000, partner, timeline
        )
        split_partner = DealFee.create_split_fee(
            "Split", 300_000, partner, timeline, first_percentage=0.3
        )
        uniform_partner = DealFee.create_uniform_fee(
            "Uniform", 400_000, partner, timeline
        )

        # Test all factory methods with ThirdParty
        upfront_third = DealFee.create_upfront_fee(
            "Upfront", 50_000, third_party, timeline
        )
        completion_third = DealFee.create_completion_fee(
            "Completion", 75_000, third_party, timeline
        )
        split_third = DealFee.create_split_fee(
            "Split", 100_000, third_party, timeline, last_percentage=0.7
        )
        uniform_third = DealFee.create_uniform_fee(
            "Uniform", 125_000, third_party, timeline
        )
