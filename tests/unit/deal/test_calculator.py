...
class TestFundingCascade:
    ...
    def test_orchestrate_funding_step_a_calculate_total_uses(self, sample_deal_all_equity, sample_timeline, sample_settings):
        """
        ...
        expected_columns = ["Acquisition Costs", "Construction Costs", "Other Project Costs", "Total Uses"]
        ...
        assert first_period_acquisition == expected_acquisition, "All acquisition costs should appear in first period"
    ...
    def test_orchestrate_funding_step_a_uses_structure(self, sample_deal_all_equity, sample_timeline, sample_settings):
        """
    ...
    def test_orchestrate_funding_step_a_uses_values(self, sample_deal_all_equity, sample_timeline, sample_settings):
        """
    ...
    def test_orchestrate_funding_step_a_zero_uses_handling(self, sample_timeline, sample_settings):
        """
    ...
    def test_orchestrate_funding_step_a_acquisition_timing(self, sample_deal_all_equity, sample_timeline, sample_settings):
        """
    ...
    def test_orchestrate_funding_step_a_construction_timing(self, sample_deal_all_equity, sample_timeline, sample_settings):
        """
    ...
    def test_orchestrate_funding_step_b_equity_only_deal(self, sample_deal_all_equity, sample_timeline, sample_settings):
        """
    ...
    def test_orchestrate_funding_step_b_equity_timing_validation(self, sample_deal_all_equity, sample_timeline, sample_settings):
        ...
        for period in sample_timeline.period_index:
            ...
            if period_uses == 0:
                assert period_equity == 0, f"No equity should be contributed in period {period} with zero Uses"
            else:
                assert period_equity > 0, f"Equity should be contributed in period {period} with Uses ${period_uses:,.0f}"
        ...
        pd.testing.assert_series_equal(equity_cumulative, calculated_cumulative, 
                                     check_names=False)
        # Note: Cumulative equity should equal cumsum of contributions

    def test_orchestrate_funding_step_b_equity_component_structure(self, sample_deal_all_equity, sample_timeline, sample_settings):
        """
        ...
        assert equity_contributions.index.equals(sample_timeline.period_index), "Should use timeline period_index"
        assert len(equity_contributions) == len(sample_timeline.period_index), "Should have value for each period"
        
        # Verify data types and values
        assert equity_contributions.dtype in ['float64', 'Float64'], "Should be numeric type"
        assert (equity_contributions >= 0).all(), "All equity contributions should be non-negative"
        ...
        pd.testing.assert_series_equal(levered_cash_flows, expected_levered_cf, 
                                     check_names=False)
        # Note: Levered CF should equal -Uses + Equity

    def test_orchestrate_funding_step_b_zero_uses_periods(self, sample_timeline, sample_settings):
        """
        ...
        for period in sample_timeline.period_index:
            ...
            if period_uses == 0:
                assert period_equity == 0, f"Should have zero equity in zero-use period {period}"

    def test_orchestrate_funding_step_c_leveraged_deal_funding(self, sample_development_project, sample_acquisition, sample_timeline, sample_settings):
        """
    ...
    def test_orchestrate_funding_step_c_component_integration(self, sample_development_project, sample_acquisition, sample_timeline, sample_settings):
        ...
        assert isinstance(loan_proceeds, pd.Series), "loan_proceeds should be pandas Series"
        assert debt_draws.index.equals(sample_timeline.period_index), "debt_draws should use timeline period_index"
        assert loan_proceeds.index.equals(sample_timeline.period_index), "loan_proceeds should use timeline period_index"
        
        # Verify data types and values
        assert debt_draws.dtype in ['float64', 'Float64'], "debt_draws should be numeric type"
        assert loan_proceeds.dtype in ['float64', 'Float64'], "loan_proceeds should be numeric type"
    ...
    def test_orchestrate_funding_step_d_interest_calculation(self, sample_development_project, sample_acquisition, sample_timeline, sample_settings):
        """
    ...
    def test_orchestrate_funding_step_d_interest_compounding(self, sample_development_project, sample_acquisition, sample_timeline, sample_settings):
        """
        ...
        pd.testing.assert_series_equal(total_uses, expected_total_uses, check_names=False)
    ...
    def test_orchestrate_funding_step_d_interest_reserve_funding(self, sample_development_project, sample_acquisition, sample_timeline, sample_settings):
        ...
        financing_plan = FinancingPlan(
            name="Interest Reserve Financing",
            facilities=[construction_facility]
        )
        ...
        assert (interest_expense == 0).all(), "Interest expense should be zero when funded from reserve"
        
        # But interest should still be tracked in the reserve details
        assert (interest_funded_from_reserve >= 0).all(), "Interest funded from reserve should be non-negative"
        ...
        assert total_debt >= total_interest_from_reserve, "Total debt should include interest reserve draws"

    def test_orchestrate_funding_step_d_integration_with_funding_cascade(self, sample_development_project, sample_acquisition, sample_timeline, sample_settings):
        """
