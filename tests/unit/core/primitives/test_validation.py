...
class TestValidationMixin:
    ...
    def test_validate_either_or_required_fails_neither(self):
        ...
        with pytest.raises(
            ValueError, match="Either field_a or field_b must be provided"
        ):
    ...
    def test_validate_date_ordering_success(self):
        ...
        data = {"start_date": date(2024, 1, 1), "end_date": date(2024, 12, 31)}
    ...
    def test_validate_date_ordering_fails(self):
        ...
        data = {"start_date": date(2024, 12, 31), "end_date": date(2024, 1, 1)}
    ...
    def test_validate_date_ordering_same_date_fails(self):
        ...
        data = {"start_date": date(2024, 1, 1), "end_date": date(2024, 1, 1)}
        with pytest.raises(ValueError, match="end_date must be after start_date"):
            ValidationMixin.validate_date_ordering(data, "start_date", "end_date")
    ...
    def test_validate_conditional_requirement_fails(self):
        ...
        with pytest.raises(
            ValueError, match="required is required when condition is trigger_value"
        ):
    ...
    def test_validate_conditional_requirement_multiple_values(self):
        ...
        with pytest.raises(
            ValueError, match="required is required when condition is value2"
        ):
            ValidationMixin.validate_conditional_requirement(
...
class TestValidateTermSpecification:
    ...
    def test_valid_with_end_date(self):
        ...
        model = self.TermModel(
            start_date=date(2024, 1, 1), end_date=date(2024, 12, 31), name="Test"
        )
    ...
    def test_valid_with_term_months(self):
        ...
        model = self.TermModel(start_date=date(2024, 1, 1), term_months=12, name="Test")
        assert model.start_date == date(2024, 1, 1)
    ...
    def test_fails_with_neither(self):
        ...
        with pytest.raises(
            ...
            self.TermModel(start_date=date(2024, 1, 1), name="Test")
    ...
    def test_fails_with_invalid_date_order(self):
        ...
        with pytest.raises(ValidationError, match="end_date must be after start_date"):
            self.TermModel(
                start_date=date(2024, 12, 31), end_date=date(2024, 1, 1), name="Test"
            )
    ...
    def test_fails_with_same_dates(self):
        ...
        with pytest.raises(ValidationError, match="end_date must be after start_date"):
            self.TermModel(
                start_date=date(2024, 1, 1), end_date=date(2024, 1, 1), name="Test"
            )
...
class TestMutualExclusivityValidator:
    ...
    def test_mutual_exclusivity_with_validation_mixin(self):
        ...
        # Test invalid cases
        with pytest.raises(
            ValidationError, match="Either option_a or option_b must be provided"
        ):
            MutualExclusiveModel(name="Test")

        with pytest.raises(
            ValidationError, match="Cannot provide both option_a and option_b"
        ):
...
class TestEdgeCases:
    ...
    def test_validate_term_specification_dict_validation_error(self):
        ...
        data = {"start_date": date(2024, 1, 1), "end_date": None, "term_months": None}
        with pytest.raises(
...
class TestDocumentationExamples:
    ...
    def test_timeline_like_model_example(self):
            ...
            def validate_start_definition(cls, data):
                ...
                if hasattr(data, "start_date"):
                    ...
                    if start_date is None and start_offset is None:
                        raise ValueError(
                            "Timeline must have either start_date or start_offset_months"
                        )
                    if start_date is not None and start_offset is not None:
                        raise ValueError(
                            "Cannot provide both start_date and start_offset_months"
                        )
        ...
        timeline1 = TimelineModel(
            name="Absolute Timeline", start_date=date(2024, 1, 1), duration_months=12
        )
        ...
        timeline2 = TimelineModel(
            name="Relative Timeline", start_offset_months=6, duration_months=12
        )
