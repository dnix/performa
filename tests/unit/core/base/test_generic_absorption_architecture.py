...
class TestGenericAbsorptionArchitecture:
    ...
    def test_office_absorption_plan_type_safety(self):
        ...
        assert all(
            isinstance(item, OfficeMiscIncome)
            for item in office_plan.stabilized_misc_income
        )
    ...
    def test_residential_absorption_plan_type_safety(self):
        ...
        assert all(
            isinstance(item, ResidentialMiscIncome)
            for item in residential_plan.stabilized_misc_income
        )
    ...
    def test_factory_methods_provide_safe_defaults(self):
        ...
        office_plan = OfficeAbsorptionPlan.with_typical_assumptions(
            ...
            start_date_anchor=date(2024, 1, 1),
            pace=OfficeFixedQuantityPace(
                type="FixedQuantity", quantity=5000, unit="SF", frequency_months=2
            ),
            leasing_assumptions="test",
        )
    ...
    def test_polymorphic_behavior_with_generic_function(self):
        ...
        office_plan = OfficeAbsorptionPlan.with_typical_assumptions(
            ...
            start_date_anchor=date(2024, 1, 1),
            pace=OfficeFixedQuantityPace(
                type="FixedQuantity", quantity=1000, unit="SF", frequency_months=1
            ),
            leasing_assumptions="test",
        )
    ...
    def test_no_silent_defaults_safety_architecture(self):
        ...
        assert "stabilized_expenses" in str(exc_info.value) or "Field required" in str(
            exc_info.value
        )
    ...
    def test_cross_contamination_prevention(self):
        ...
        assert (
            type(residential_plan.stabilized_expenses).__name__ == "ResidentialExpenses"
        )
