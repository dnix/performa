...
def test_per_unit_capex_residential_vs_cashflow_model():
    ...
    context = AnalysisContext(timeline=timeline, settings=GlobalSettings(), property_data=property_data)
...
def test_capex_now_applies_growth_rates():
    ...
    capex = CapExItemBase(
        ...
        reference=PropertyAttributeKey.UNIT_COUNT,  # Residential uses UNIT_COUNT
        frequency=FrequencyEnum.ANNUAL,
        growth_rate=PercentageGrowthRate(name="CapEx Growth", value=0.03),  # 3% growth - now applied!
    )
    ...
    assert year2_total > year1_total, "CapEx should now grow year over year (industry standard)"
    
    # Verify the year-over-year growth rate is approximately correct
    growth_rate = (year2_total / year1_total) - 1
    assert abs(growth_rate - 0.03) < 0.005, f"Growth rate should be ~3%, got {growth_rate:.1%}"
    
    # Verify we're in the right ballpark (with compounding)
    assert year1_total > 30000, "Year 1 should be above base due to continuous compounding"
