...
class TestInterestRateSystem:
    ...
    def test_rate_cap_functionality(self):
        ...
        high_sofr = pd.Series(
            [0.07], index=pd.period_range("2024-01", periods=1, freq="M")
        )
        capped_rate = rate.get_rate_for_period(
            pd.Period("2024-01", freq="M"), high_sofr
        )
    ...
    def test_floating_rate_missing_index_error(self):
        ...
        rate = InterestRate(
            details=FloatingRate(
                rate_index=RateIndexEnum.SOFR_30_DAY_AVG, spread=0.0275
            )
        )
...
class TestInterestOnlyPeriods:
    ...
    def test_io_cost_comparison(self):
        ...
        assert (
            io_summary["Total Interest Paid"] > standard_summary["Total Interest Paid"]
        )
...
class TestCovenantMonitoring:
    ...
    def test_compliant_covenant_monitoring(self):
        ...
        prop_values = pd.Series(
            [10_000_000] * 12, index=timeline.period_index
        )  # Very high value
        noi_values = pd.Series(
            [1_000_000] * 12, index=timeline.period_index
        )  # Very high NOI

        results = facility.calculate_covenant_monitoring(
            timeline, prop_values, noi_values
        )
    ...
    def test_breach_detection(self):
        """Test covenant breach detection with corrected DSCR calculations."""
        facility = self.create_test_facility()
        timeline = self.create_mock_timeline(12)

        # Create ACTUAL breach scenario using corrected calculations
        # With $5M loan at 6.5% for 5 years: annual debt service ≈ $1,148,798
        # For DSCR breach (< 1.20): need NOI < $1,378,558 (1.20 * $1,148,798)
        prop_values = pd.Series(
            [5_500_000] * 12, index=timeline.period_index  
        )  # Property value for LTV calculation
        
        # Use NOI that will actually cause DSCR breach: $35k monthly = $420k annual
        # This will give DSCR ≈ 1.10, breaching the 1.20 minimum
        noi_values = pd.Series([35_000] * 12, index=timeline.period_index)  # Monthly NOI for DSCR breach
        ...
        assert any(results["Covenant_Status"] == "BREACH"), "Should detect covenant breaches"
        ...
        assert (results["DSCR"] < 1.20).all(), "All periods should breach DSCR (constant series)"
    ...
    def test_dscr_varies_with_noi_changes(self):
        ...
        # Create property values (constant)
        prop_values = pd.Series([7_000_000] * 12, index=timeline.period_index)
        
        # Create varying NOI over time - start low, end high
        base_noi = 50_000
        noi_values = pd.Series([
            base_noi + (i * 5_000) for i in range(12)  # NOI grows $5k per month
        ...
        # DSCR should change over time as NOI changes
...
class TestRefinancingOrchestration:
    ...
    def test_construction_to_permanent_refinancing_workflow(self):
        ...
        assert (
            transaction["transaction_type"] == "construction_to_permanent_refinancing"
        )
        ...
        expected_dscr_loan = (
            refinance_noi / 1.25
        ) / permanent_facility._calculate_annual_debt_constant()
        expected_debt_yield_loan = refinance_noi / 0.08  # 8% debt yield

        assert sizing_analysis["ltv_constraint"] == pytest.approx(
            expected_ltv_loan, rel=1e-2
        )
        assert sizing_analysis["dscr_constraint"] == pytest.approx(
            expected_dscr_loan, rel=1e-2
        )
        assert sizing_analysis["debt_yield_constraint"] == pytest.approx(
            expected_debt_yield_loan, rel=1e-2
        )

        # Verify most restrictive constraint
        most_restrictive_amount = min(
            expected_ltv_loan, expected_dscr_loan, expected_debt_yield_loan
        )
        assert transaction["new_loan_amount"] == pytest.approx(
            most_restrictive_amount, rel=1e-2
        )
        ...
        assert transaction["closing_costs"] == pytest.approx(
            transaction["new_loan_amount"] * 0.015, rel=1e-2
        )
        expected_net_proceeds = (
            transaction["new_loan_amount"]
            - transaction["payoff_amount"]
            - transaction["closing_costs"]
        )
        assert transaction["net_proceeds"] == pytest.approx(
            expected_net_proceeds, rel=1e-2
        )
    ...
    def test_manual_sizing_override(self):
        ...
        property_value_series = pd.Series(
            [10_000_000] * len(timeline.period_index), index=timeline.period_index
        )
        noi_series = pd.Series(
            [800_000] * len(timeline.period_index), index=timeline.period_index
        )
    ...
    def test_covenant_monitoring_integration(self):
        ...
        assert (
            covenant_monitoring["ongoing_debt_yield_min"]
            > permanent_facility.debt_yield_hurdle
        )

        print("✓ Covenant monitoring integration test passed:")
        print(
            f"  Sizing LTV: {permanent_facility.ltv_ratio:.1%} → Ongoing LTV: {covenant_monitoring['ongoing_ltv_max']:.1%}"
        )
        print(
            f"  Sizing DSCR: {permanent_facility.dscr_hurdle:.2f}x → Ongoing DSCR: {covenant_monitoring['ongoing_dscr_min']:.2f}x"
        )
        print(
            f"  Sizing Debt Yield: {permanent_facility.debt_yield_hurdle:.1%} → Ongoing Debt Yield: {covenant_monitoring['ongoing_debt_yield_min']:.1%}"
        )
    ...
    def test_refinancing_with_interest_only_periods(self):
        ...
        noi_series = pd.Series(
            [700_000] * len(timeline.period_index), index=timeline.period_index
        )
        ...
        amortization = permanent_facility.generate_amortization(
            loan_amount=7_000_000, start_date=amort_timeline.period_index[0]
        )
        ...
        print(
            f"  Interest-Only Period: {permanent_facility.interest_only_months} months"
        )
        print(f"  First Month I/O Payment: ${first_36_months['Payment'].iloc[0]:,.0f}")
        print(
            f"  First Month Amortizing Payment: ${amortizing_months['Payment'].iloc[0]:,.0f}"
        )
