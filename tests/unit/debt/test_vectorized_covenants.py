...
class TestVectorizedCovenantMonitoring:
    ...
    def test_vectorized_basic_functionality(self):
        ...
        noi_values = pd.Series(
            [160_000] * 12, index=timeline.period_index
        )  # $160k annual NOI (sufficient for DSCR > 1.2)
        ...
        assert not results.iloc[0][
            "LTV_Breach"
        ]  # Should not breach with these parameters
        assert all(
            results["Covenant_Status"] == "COMPLIANT"
        )  # All periods should be compliant
    ...
    def test_vectorized_safe_division_edge_cases(self):
        ...
        property_values = pd.Series(
            [0, 2_000_000, 0, 1_500_000, 2_500_000, 0], index=timeline.period_index
        )
        noi_values = pd.Series(
            [0, 120_000, 100_000, 0, 150_000, 80_000], index=timeline.period_index
        )
...
class TestVectorizedPerformance:
    ...
    def test_vectorized_performance_vs_iterative_simulation(self):
        ...
        # Create test data
        property_values = pd.Series(
            [2_000_000] * len(timeline.period_index), index=timeline.period_index
        )
        noi_values = pd.Series(
            [120_000] * len(timeline.period_index), index=timeline.period_index
        )
        ...
        print(
            f"   Estimated speedup: {estimated_iterative_time / vectorized_time:.1f}x"
        )
        print(
            f"   Throughput: {len(timeline.period_index) / vectorized_time:.0f} periods/second"
        )
    ...
    def test_portfolio_scale_performance(self):
        ...
        for i in range(num_loans):
            facility = PermanentFacility(
                ...
                interest_rate=InterestRate(
                    details=FixedRate(rate=0.05 + i * 0.001)
                ),  # Vary rates slightly
                loan_term_years=10,
                ltv_ratio=0.75,
                dscr_hurdle=1.25,
            ...
            property_values = pd.Series(
                [property_values_base[i]] * len(timeline.period_index),
                index=timeline.period_index,
            )
            noi_values = pd.Series(
                [noi_values_base[i]] * len(timeline.period_index),
                index=timeline.period_index,
            )
        ...
        print(
            f"   Throughput: {total_calculations / portfolio_time:.0f} calculations/second"
        )
...
def test_vectorized_integration_validation():
    ...
    assert (
        results.iloc[-1]["Covenant_Status"] == "PAID_OFF"
    ...
    print(
        f"   Initial Debt Yield: {initial_debt_yield:.3f} → Final Debt Yield: {final_debt_yield:.3f}"
    )
