...
class TestTrapModeRelease:
    ...
    def test_trap_mode_release(self):
        ...
        assert release_amount == 50_000, f"Release should equal deposit, got {release_amount}"
        ...
        assert release_date == expected_date, f"Release should occur at {expected_date}, got {release_date}"


class TestPrepayModeReducesBalance:
    """Test PREPAY mode reduces facility outstanding balance."""

    @pytest.mark.skip(reason="Outdated: tests implementation details. Balance now implicit via ledger, not facility attribute.")
...
class TestPrepayModePostsTransactions:
    ...
    @pytest.mark.skip(reason="Covered by integration tests. Mock complexity not worth maintaining.")
