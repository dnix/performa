...
class TestDebtCalculationsAgainstBenchmarks:
    ...
    def test_dscr_calculation_through_ledger(self):
        ...
        expected_dscr = manual_dscr_calculation(
            benchmark["annual_noi"], expected_annual_ds
        )
    ...
    def test_interest_only_periods_against_manual_calculation(self):
        ...
        schedule = facility.generate_amortization(
            loan_amount=loan_amount, start_date=timeline_short.period_index[0]
        )
    ...
    def test_ltv_and_debt_yield_calculations(self):
        ...
        expected_ltv = manual_ltv_calculation(
            benchmark["loan_amount"], benchmark["property_value"]
        )
        expected_debt_yield = manual_debt_yield_calculation(
            benchmark["annual_noi"], benchmark["loan_amount"]
        )
        ...
        property_values = pd.Series(
            [benchmark["property_value"]] * 12, index=self.timeline.period_index[:12]
        )
        monthly_noi = benchmark["annual_noi"] / 12
        noi_values = pd.Series(
            [monthly_noi] * 12, index=self.timeline.period_index[:12]
        )

        print(
            f"Debug: Monthly NOI = ${monthly_noi:,.0f}, Annual NOI = ${benchmark['annual_noi']:,.0f}"
        )
        ...
        print(
            f"Debug: Covenant results LTV = {actual_ltv:.3%}, Debt Yield = {actual_debt_yield:.3%}"
        )
        print(
            f"Debug: Expected Debt Yield calculation: ${benchmark['annual_noi']:,.0f} / ${benchmark['loan_amount']:,.0f} = {expected_debt_yield:.3%}"
        )
        ...
        print(
            f"✓ Debt Yield validation: {actual_debt_yield:.3%} vs {expected_debt_yield:.3%}"
        )
    ...
    def test_sizing_trifecta_against_manual_constraints(self):
        ...
        actual_loan_amount = facility.calculate_refinance_amount(
            property_value=property_value, noi=annual_noi
        )
