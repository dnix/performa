...
class TestRealisticCovenantMonitoring:
    ...
    def test_stabilized_office_compliant_scenario(self):
        ...
        property_values = pd.Series([scenario['property_value']] * 12, index=timeline.period_index)
        noi_values = pd.Series([scenario['annual_noi']] * 12, index=timeline.period_index)
        ...
        assert abs(actual_ltv - expected_ltv) < 0.05, f"LTV {actual_ltv:.3f} vs expected {expected_ltv:.3f}"
        
        # DSCR should be in healthy range
        actual_dscr = results.iloc[0]['DSCR']
        dscr_min, dscr_max = scenario['expected_dscr_range']
        assert dscr_min <= actual_dscr <= dscr_max, f"DSCR {actual_dscr:.3f} outside range {dscr_min}-{dscr_max}"
        
        # Should not have covenant breaches
        breach_count = (results['Covenant_Status'] == 'BREACH').sum()
        assert breach_count == 0, f"Expected no breaches for compliant scenario, found {breach_count}"
        
        print(f"✅ Stabilized Office Scenario: LTV={actual_ltv:.3f}, DSCR={actual_dscr:.2f}, Breaches={breach_count}")
    ...
    def test_value_add_multifamily_compliant_scenario(self):
        ...
        facility = PermanentFacility(
            ...
            interest_rate=InterestRate(details=FixedRate(rate=0.055)),  # Match debt constant calculation
            loan_term_years=10,
        ...
        assert breach_count == 0, f"Conservative multifamily scenario should be compliant, found {breach_count} breaches"
        assert actual_dscr >= 1.25, f"DSCR {actual_dscr:.3f} should be healthy for conservative deal"
        
        print(f"✅ Value-Add Multifamily: LTV={actual_ltv:.3f}, DSCR={actual_dscr:.2f}, Breaches={breach_count}")
    ...
    def test_distressed_retail_breach_scenario(self):
        ...
        facility = PermanentFacility(
            ...
            interest_rate=InterestRate(details=FixedRate(rate=0.060)),  # Higher rate for distressed
            loan_term_years=10,
        ...
        dscr_min, dscr_max = scenario['expected_dscr_range']
        assert dscr_min <= actual_dscr <= dscr_max, f"DSCR {actual_dscr:.3f} outside distressed range {dscr_min}-{dscr_max}"
        
        print(f"✅ Distressed Retail (Breaches Expected): LTV={actual_ltv:.3f}, DSCR={actual_dscr:.2f}, Breaches={breach_count}")
    ...
    def test_vectorized_performance_with_realistic_data(self):
        ...
        for i, scenario in enumerate(scenarios):
            ...
            property_values = pd.Series([scenario['property_value']] * len(timeline.period_index), 
                                      index=timeline.period_index)
            noi_values = pd.Series([scenario['annual_noi']] * len(timeline.period_index), 
                                 index=timeline.period_index)
        ...
        for result in portfolio_results:
            if result['expected_breaches']:
                assert result['breach_count'] > 0, "Breach scenario should have breaches"
            else:
                assert result['breach_count'] == 0, "Compliant scenario should not have breaches"
        
        print(f"✅ Portfolio Performance Test: {len(scenarios)} realistic scenarios processed successfully")
