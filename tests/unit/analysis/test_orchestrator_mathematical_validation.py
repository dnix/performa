...
class TestCashFlowOrchestratorMathematical:
    ...
    def test_orchestrator_calculates_effective_gross_income_for_dependent_models(self, context):
        """
    ...
    def test_orchestrator_full_integration_with_noi_calculation(self, context):
        ...
        class TestLease(CashFlowModel):
            def compute_cf(self, context):
                return {"base_rent": pd.Series(15000.0, index=context.timeline.period_index)}
        ...
        print(f"   Total OpEx: ${total_opex:,.0f} (expected ${expected_total_opex:,.0f})")
    ...
    def test_orchestrator_zero_dependent_models_edge_case(self, context):
        ...
        class SimpleLease(CashFlowModel):
            def compute_cf(self, context):
                return {"base_rent": pd.Series(5000.0, index=context.timeline.period_index)}
...
def test_effective_gross_income_bug_reproduction():
    ...
    assert pgr == pytest.approx(expected_pgr, abs=0.01)
    assert egi == pytest.approx(expected_egi, abs=0.01), f"EGI was {egi}, expected {expected_egi}"
    assert total_opex == pytest.approx(expected_total_opex, abs=0.01), f"Total OpEx was {total_opex}, expected {expected_total_opex}"
    assert noi == pytest.approx(expected_noi, abs=0.01), f"NOI was {noi}, expected {expected_noi}"
