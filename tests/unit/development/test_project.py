    ...
    return ResidentialDevelopmentBlueprint(
        ...
        absorption_plan=ResidentialAbsorptionPlan.with_typical_assumptions(
            ...
            leasing_assumptions=ResidentialDirectLeaseTerms(
                monthly_rent=3500.0, lease_term_months=12, security_deposit_months=1.5
            ),
            start_date_anchor="AnalysisStart",
        ),
    )
...
def test_single_asset_type_development_project(
    construction_plan, financing_plan, office_blueprint, disposition_plan
):
...
def test_blueprint_discriminated_union_validation(
    office_blueprint, residential_blueprint
):
...
def test_development_project_pydantic_serialization(
    ...
    assert (
        reconstructed_project.blueprints[0].use_type == project.blueprints[0].use_type
    )
    assert (
        reconstructed_project.blueprints[1].use_type == project.blueprints[1].use_type
    )


def test_empty_blueprints_list(construction_plan, financing_plan, disposition_plan):
...
def test_blueprint_name_inheritance_to_assets(office_blueprint, residential_blueprint):
