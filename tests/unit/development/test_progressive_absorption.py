...
def progressive_development_project():
    ...
    absorption_plan = ResidentialAbsorptionPlan(
        ...
        leasing_assumptions=ResidentialDirectLeaseTerms(monthly_rent=2000.0, term_months=12),
...
def test_development_framework_progressive_revenue(progressive_development_project):
    ...
    month_revenues = [revenue_series.iloc[i] for i in range(min(8, len(revenue_series)))]
    ...
    assert month_revenues[1] == 0, f"Month 2 should have $0 revenue, got ${month_revenues[1]:,.0f}"
...
def test_stabilized_revenue_reaches_expected_level(progressive_development_project):
    ...
    if len(revenue_series) >= 9:
        ...
        print(f"✅ Stabilized revenue: ${month9_revenue:,.0f} (expected: ${expected_stabilized:,.0f})")
