...
class TestOfficeLease(unittest.TestCase):
    ...
    def test_project_future_cash_flows_market_rollover(self):
        ...
        original_lease_rent = future_df.loc[
            pd.Period("2023-01", "M") : pd.Period("2023-12", "M"), "base_rent"
        ]
        ...
        self.assertEqual(
            future_df.loc[pd.Period("2024-03", "M"), "base_rent"], 0
        )  # Still in downtime
    ...
    def test_project_future_cash_flows_with_ti_lc(self):
        ...
        self.assertEqual(
            future_df.loc[pd.Period("2024-03", "M"), "leasing_commission"], 0
        )
    ...
    def test_project_future_cash_flows_renew_scenario(self):
        ...
        self.assertAlmostEqual(
            future_df.loc[renewal_start_period, "base_rent"],
            expected_renewal_rent,
            places=2,
        )
    ...
    def test_signing_date_payment_timing(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "signing_date must be on or before start_date"
        ):
            OfficeLeaseSpec(
                ...
                start_date=date(
                    2023, 12, 1
                ),  # Before signing_date - should fail validation
                term_months=12,
                base_rent_value=50.0,
                base_rent_reference=PropertyAttributeKey.NET_RENTABLE_AREA,
                upon_expiration=UponExpirationEnum.MARKET,
        ...
        with self.assertRaisesRegex(
            ValueError, "TI payment_timing is 'signing' but no signing_date provided"
        ):
        ...
        try:
            ...
            self.fail(
                f"TI compute_cf should not raise exception when lease has signing_date: {e}"
            )
