...
class TestOfficeRentAbatement(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.recovery_method = OfficeRecoveryMethod(
            ...
            recoveries=[
                Recovery(
                    expenses=ExpensePool(name="CAM Pool", expenses=[self.operating_expense]),
                    structure="net"
                )
            ]
        )
    ...
    def test_full_rent_abatement_basic(self):
        ...
        for period in abated_periods:
            self.assertEqual(cash_flows["base_rent"][period], 0.0, 
        ...
        for period in normal_periods:
            self.assertAlmostEqual(cash_flows["base_rent"][period], monthly_rent, places=2,
    ...
    def test_partial_rent_abatement(self):
        ...
        for period in abated_periods:
            self.assertAlmostEqual(cash_flows["base_rent"][period], abated_rent, places=2,
    ...
    def test_abatement_without_recoveries(self):
        ...
        context.resolved_lookups[self.operating_expense.uid] = self.operating_expense.compute_cf(context=context)
        for recovery in self.recovery_method.recoveries:
            context.recovery_states[recovery.uid] = RecoveryCalculationState(recovery_uid=recovery.uid)
        ...
        for period in abated_periods:
            ...
            self.assertAlmostEqual(cash_flows["recoveries"][period], monthly_recovery, places=2,
    ...
    def test_abatement_with_recoveries(self):
        ...
        for period in abated_periods:
            self.assertEqual(cash_flows["base_rent"][period], 0.0,
                           f"Base rent should be $0 during abatement in {period}")
            self.assertEqual(cash_flows["recoveries"][period], 0.0,
    ...
    def test_abatement_partial_with_recoveries(self):
        ...
        for period in abated_periods:
            ...
            self.assertAlmostEqual(cash_flows["recoveries"][period], abated_recovery, places=2,
    ...
    def test_abatement_extends_beyond_lease_term(self):
        ...
        self.assertAlmostEqual(cash_flows["abatement"].sum(), expected_total_abatement, places=2)
    ...
    def test_revenue_calculation_with_abatement(self):
        ...
        for period in abated_periods:
            self.assertAlmostEqual(cash_flows["revenue"][period], abated_revenue, places=2,
                                 msg=f"Revenue should be 25% of normal during abatement in {period}")

        # After abatement
        normal_periods = self.timeline.period_index[2:]
        for period in normal_periods:
            self.assertAlmostEqual(cash_flows["revenue"][period], normal_revenue, places=2,
