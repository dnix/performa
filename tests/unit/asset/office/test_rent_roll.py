...
def test_office_vacant_suite_validation():
    # This should work
    OfficeVacantSuite(suite="101", floor="1", area=1000, use_type="office", is_divisible=True, subdivision_average_lease_area=200)

    # This should fail because average area is missing
    with pytest.raises(ValueError, match="'subdivision_average_lease_area' must be set"):
        OfficeVacantSuite(suite="101", floor="1", area=1000, use_type="office", is_divisible=True)

    # This should fail because average is larger than total
    with pytest.raises(ValueError, match="'subdivision_average_lease_area' cannot be greater"):
        OfficeVacantSuite(suite="101", floor="1", area=1000, use_type="office", is_divisible=True, subdivision_average_lease_area=1200)
