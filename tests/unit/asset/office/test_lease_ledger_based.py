...
class TestOfficeLeaseLedgerBehavior(unittest.TestCase):
    ...
    def test_simple_lease_creates_correct_revenue_in_ledger(self):
        ...
        losses = OfficeLosses(
            general_vacancy=OfficeGeneralVacancyLoss(rate=0.0),  # No vacancy for simple test
            credit_loss=OfficeCreditLoss(rate=0.0),      # No collection loss for simple test
        )
        ...
        self.assertAlmostEqual(
            ...
            60000.0,
            places=2,
            msg="Annual revenue should be $60k"
        )
