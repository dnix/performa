...
def sample_lease() -> OfficeLease:
    ...
    analysis_start_date = date(
        2023, 11, 1
    )  # Analysis starts early enough to capture signing
    return OfficeLease.from_spec(
        spec=spec, timeline=timeline, analysis_start_date=analysis_start_date
    )
...
def test_lc_partial_term(sample_context: AnalysisContext):
    ...
    timeline = Timeline.from_dates(
        start_date=date(2024, 1, 1), end_date=date(2027, 6, 30)
    )
...
def test_commission_tier_split_payment_validation():
    ...
    with pytest.raises(ValueError, match="Payment percentages must sum to 1.0"):
        CommissionTier(
            ...
            year_end=5,
            rate=0.06,
            signing_percentage=0.6,
            commencement_percentage=0.5,  # Sum = 1.1
        )


def test_lc_split_payment_timing(
    sample_context: AnalysisContext, sample_lease: OfficeLease
):
...
def test_lc_multiple_tiers_different_payment_schedules(
    sample_context: AnalysisContext, sample_lease: OfficeLease
):
...
def test_lc_split_payment_without_signing_date_error(sample_context: AnalysisContext):
    ...
    timeline = Timeline.from_dates(
        start_date=spec_no_signing.start_date,
        end_date=spec_no_signing.computed_end_date,
    )
    analysis_start_date = date(
        2023, 11, 1
    )  # Analysis starts early enough to capture signing
    lease_no_signing = OfficeLease.from_spec(
        spec=spec_no_signing, timeline=timeline, analysis_start_date=analysis_start_date
    )
    ...
    with pytest.raises(
        ValueError,
        match="LC tier requires signing payment but no signing_date provided",
    ):
        lc.compute_cf(context=sample_context)


def test_lc_backward_compatibility_without_lease_context(
