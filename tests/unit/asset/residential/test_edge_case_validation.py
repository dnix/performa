...
class TestZeroDowntimeScenarios:
    ...
    def test_zero_downtime_immediate_transition(self):
        ...
        rollover_profile = ResidentialRolloverProfile(
            ...
            renewal_terms=ResidentialRolloverLeaseTerms(market_rent=2100.0, term_months=12),
        ...
        property_model = ResidentialProperty(
            ...
            address=Address(street="123 Zero St", city="Test", state="CA", zip_code="90210", country="USA"),
            gross_area=800.0,
        ...
        assert pgr_series.iloc[5] == 2500.0  # Month 6 (Jun 2024) - premium lease starts (zero downtime)
...
class TestExtendedDowntimeScenarios:
    ...
    def test_extended_downtime_major_renovation(self):
        ...
        property_model = ResidentialProperty(
            ...
            address=Address(street="123 Major St", city="Test", state="CA", zip_code="90210", country="USA"),
            gross_area=800.0,
            net_rentable_area=800.0,
            unit_mix=ResidentialRentRoll(unit_specs=[unit_spec]),
            capital_plans=[],
        ...
        timeline = Timeline(start_date=date(2024, 1, 1), duration_months=36)  # 3-year analysis
        settings = GlobalSettings()
        ...
        assert pgr_series.iloc[4] == 2000.0, "Should have original rent in month 5 (May 2024)"
        ...
        if len(pgr_series) > 23:  
            assert pgr_series.iloc[23] == 3000.0, "Should have premium rent after extended renovation (month 24)"
...
class TestLeaseExpirationBeyondAnalysis:
    ...
    def test_lease_expiration_beyond_analysis_period(self):
        ...
        unit_spec = ResidentialUnitSpec(
            ...
            current_avg_monthly_rent=2000.0,
            rollover_profile=rollover_profile,
            lease_start_date=date(2025, 6, 1)  # Expires June 2026 - BEYOND 24-month analysis (ends Dec 2025)
        )
        
        property_model = ResidentialProperty(
            name="Future Expiration Property",
            address=Address(street="123 Future St", city="Test", state="CA", zip_code="90210", country="USA"),
            gross_area=800.0,
            net_rentable_area=800.0,
            unit_mix=ResidentialRentRoll(unit_specs=[unit_spec]),
            capital_plans=[],
        ...
        timeline = Timeline(start_date=date(2024, 1, 1), duration_months=24)  # 2-year analysis
        settings = GlobalSettings()
...
class TestCircularReferenceDetection:
    ...
    def test_circular_reference_prevention_documentation(self):
        ...
        property_model = ResidentialProperty(
            ...
            address=Address(street="123 Circle St", city="Test", state="CA", zip_code="90210", country="USA"),
            gross_area=800.0,
            net_rentable_area=800.0,
            unit_mix=ResidentialRentRoll(unit_specs=[unit_spec]),
            capital_plans=[],
...
class TestEdgeCaseDocumentation:
    ...
    def test_capital_plan_timing_independence(self):
        ...
        property_model = ResidentialProperty(
            ...
            address=Address(street="123 Timing St", city="Test", state="CA", zip_code="90210", country="USA"),
            gross_area=800.0,
            net_rentable_area=800.0,
            unit_mix=ResidentialRentRoll(unit_specs=[unit_spec]),
        ...
        assert pgr_series.iloc[4] == 2000.0, "Should have original rent in month 5 (before expiration)"
        assert pgr_series.iloc[5] == 0.0, "Should have no revenue in month 6 (expiration + downtime)"
        assert pgr_series.iloc[6] == 0.0, "Should have no revenue in month 7 (downtime continues)"  
        assert pgr_series.iloc[7] == 2500.0, "Should have premium rent in month 8 (after transformation)"
