...
def test_residential_rent_roll_aggregation():
    ...
    assert rent_roll.weighted_avg_rent == pytest.approx(
        2500.0, rel=1e-2
    )  # Weighted average
...
def test_rent_roll_computed_properties():
    ...
    # Single unit type
    single_spec = ResidentialRentRoll(
        unit_specs=[
            ResidentialUnitSpec(
...
def test_complete_residential_property_creation():
    ...
    assert (
        property_model.net_rentable_area == property_model.unit_mix.total_rentable_area
    )
...
def test_rent_roll_with_vacant_units():
    ...
    assert rent_roll.current_average_rent_per_occupied_unit == pytest.approx(
        current_income / 35, rel=1e-6
    )
    assert rent_roll.average_rent_per_unit == pytest.approx(
        total_potential / 40, rel=1e-6
    )
...
def test_vacant_units_analysis_integration():
    ...
    rollover_profile = ResidentialRolloverProfile(
        ...
        renewal_terms=ResidentialRolloverLeaseTerms(market_rent=2200.0, term_months=12),
    )
...
def test_edge_cases_with_vacant_units():
    ...
    with pytest.raises(
        ValueError, match="must have at least one unit specification or vacant unit"
    ):
        empty_roll = ResidentialRentRoll(unit_specs=[], vacant_units=[])
