...
class TestDirectCapValuation:
    ...
    def test_direct_cap_validation_cap_rate(self):
        ...
        with pytest.raises(ValidationError, match="Cap rate.*should be between 1% and 20%"):
    ...
    def test_direct_cap_validation_occupancy(self):
        ...
        # Valid occupancy
        valuation = DirectCapValuation(name="Test", cap_rate=0.065, occupancy_adjustment=0.95)
        assert valuation.occupancy_adjustment == 0.95
        
        # Invalid occupancy - too low
        with pytest.raises(ValidationError, match="Occupancy adjustment.*should be between 10% and 100%"):
    ...
    def test_direct_cap_validation_market_adjustments(self):
        ...
        with pytest.raises(ValidationError, match="Market adjustment.*should be between 0.10 and 3.00"):
            DirectCapValuation(
                name="Test", cap_rate=0.065,
                market_adjustments={"bad_factor": 0.05}
            )
        ...
        with pytest.raises(ValidationError, match="Market adjustment.*should be between 0.10 and 3.00"):
            DirectCapValuation(
                name="Test", cap_rate=0.065,
                market_adjustments={"bad_factor": 5.0}
            )
    ...
    def test_combined_market_adjustment(self):
        ...
        valuation = DirectCapValuation(
            name="Test", cap_rate=0.065,
            market_adjustments={"location": 1.10}
        )
    ...
    def test_calculate_value_basic(self):
        ...
        expected_keys = [
            "property_value", "base_value", "effective_noi", "cap_rate",
            "market_adjustment_factor", "occupancy_rate", "implied_yield"
        ]
    ...
    def test_sensitivity_analysis(self):
        ...
        # Check columns
        expected_cols = ["cap_rate", "property_value", "cap_rate_delta", "value_change_pct"]
    ...
    def test_calculate_metrics(self):
        ...
        expected_keys = [
            "property_value", "base_value", "effective_noi", "cap_rate",
            "market_adjustment_factor", "occupancy_rate", "implied_yield",
            "total_area", "value_per_sf", "noi_per_sf",
            "acquisition_cost", "value_premium", "value_premium_pct", "acquisition_cap_rate"
        ]
    ...
    def test_factory_methods(self):
        ...
        stabilized_val = DirectCapValuation.stabilized_value(cap_rate=0.065, occupancy_rate=0.95)
