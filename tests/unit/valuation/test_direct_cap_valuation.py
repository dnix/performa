...
class TestDirectCapValuation:  # noqa: PLR0904
    ...
    def test_get_noi_basis_ntm(self):
        ...
        timeline = Timeline.from_dates(date(2024, 1, 1), date(2025, 12, 31))  # 24 months
    ...
    def test_get_noi_basis_stabilized(self):
        ...
        timeline = Timeline.from_dates(date(2024, 1, 1), date(2024, 12, 31))
        noi_series = pd.Series([8000, 9000, 10000, 11000] * 3, index=timeline.period_index)
        context = create_test_context(timeline, noi_series)
        
        valuation = DirectCapValuation(
            name="Stabilized Test",
            cap_rate=0.06,
            noi_basis_kind="Stabilized"
        )
    ...
    def test_compute_cf_with_hold_period(self):
        ...
        timeline = Timeline.from_dates(date(2024, 1, 1), date(2029, 12, 31))  # 72 months
    ...
    def test_compute_cf_default_timing(self):
        ...
        timeline = Timeline.from_dates(date(2024, 1, 1), date(2026, 12, 31))  # 36 months
