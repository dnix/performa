...
class TestSalesCompValuation:
    ...
    def test_sales_comp_creation_basic(self, sample_comparables):
        ...
        valuation = SalesCompValuation(
            name="Market Analysis", comparables=sample_comparables
        )
    ...
    def test_sales_comp_validation_outlier_threshold(self, sample_comparables):
        ...
        with pytest.raises(
            ValidationError, match="Outlier threshold.*should be between 0.5 and 5.0"
        ):
            SalesCompValuation(
    ...
    def test_calculate_value(self, sample_comparables):
        ...
        expected_keys = [
            "property_value",
            "indicated_price_per_sf",
            "subject_area",
            "comparables_used",
            "comparables_excluded",
            "price_per_sf_range",
            "price_per_sf_median",
            "price_per_sf_std_dev",
            "coefficient_of_variation",
        ]
        ...
        assert (
            result["property_value"] == result["indicated_price_per_sf"] * subject_area
        )
    ...
    def test_calculate_value_with_outlier_exclusion(self, sample_comparables):
        ...
        result_included = valuation.calculate_value(
            subject_area, exclude_outliers=False
        )

        # Should use fewer comparables when excluding outliers
        assert (
            result_excluded["comparables_used"] <= result_included["comparables_used"]
        )
        assert (
            result_excluded["comparables_excluded"]
            >= result_included["comparables_excluded"]
        )
    ...
    def test_calculate_value_range(self, sample_comparables):
        ...
        # Check relationships
        assert (
            value_range["low_value"]
            <= value_range["mean_value"]
            <= value_range["high_value"]
        )
    ...
    def test_factory_method(self, sample_comparables):
        ...
        valuation = SalesCompValuation.create_from_data(
            name="Factory Test", sales_data=sales_data, minimum_comparables=2
        )
    ...
    def test_model_immutability(self, sample_comparables):
        ...
        modified_valuation = valuation.model_copy(
            update={"weighting_method": "distance"}
        )
