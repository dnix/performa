...
class TestDirectEntry:
    ...
    def test_validation_unit_multiplier_missing_params(self):
        ...
        # Missing units
        with pytest.raises(ValueError, match="Unit multiplier mode requires 'units' and 'rate_per_unit'"):
            DirectEntry(name="Test", mode="unit_multiplier", rate_per_unit=150.0, unit_type="SF")
            
        # Missing rate_per_unit
        with pytest.raises(ValueError, match="Unit multiplier mode requires 'units' and 'rate_per_unit'"):
            DirectEntry(name="Test", mode="unit_multiplier", units=100_000, unit_type="SF")
            
        # Missing unit_type
        with pytest.raises(ValueError, match="Unit multiplier mode requires 'unit_type'"):
            DirectEntry(name="Test", mode="unit_multiplier", units=100_000, rate_per_unit=150.0)
    ...
    def test_validation_yield_target_missing_params(self):
        ...
        with pytest.raises(ValueError, match="Yield target mode requires 'target_cap_rate'"):
            DirectEntry(name="Test", mode="yield_target", noi_basis_kind="LTM")
            
        # Missing noi_basis_kind
        with pytest.raises(ValueError, match="Yield target mode requires 'noi_basis_kind'"):
    ...
    def test_validation_unit_type_accepted(self):
        ...
        for unit_type in valid_types:
            entry = DirectEntry.unit_multiplier(f"Test {unit_type}", 100, 200, unit_type)
    ...
    def test_realistic_unit_types(self):
        ...
        for unit_type, units, rate in test_cases:
            entry = DirectEntry.unit_multiplier(f"Test {unit_type}", units, rate, unit_type)
            result = entry.calculate_value()
    ...
    def test_noi_basis_integration_with_yield_target(self):
        ...
        stabilized_entry = DirectEntry.yield_target("Stabilized Target", target_cap, "Stabilized")
