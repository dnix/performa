...
def irr_cash_flows() -> pd.Series:
    ...
    flows = [-100000.0] + [8000.0] * 11 + [108000.0]  # Initial investment + monthly flows + exit
...
class TestCalculateNOI:
    ...
    def test_noi_with_series_inputs(self, sample_revenue_series: pd.Series, sample_expense_series: pd.Series):
        """Test NOI calculation with pandas Series inputs."""
        noi = PropertyMetrics.calculate_noi(sample_revenue_series, sample_expense_series)
...
class TestCalculateStabilizedNOI:
    ...
    def test_stabilized_noi_invalid_period(self, sample_cash_flows_df: pd.DataFrame):
        ...
        with pytest.raises(ValueError, match="not found in cash flows"):
            PropertyMetrics.calculate_stabilized_noi(
                sample_cash_flows_df,
                stabilization_period=invalid_period
            )
    ...
    def test_stabilized_noi_invalid_index_type(self):
        ...
        df = pd.DataFrame({
            'revenue': [10000.0] * 12,
            'expenses': [6000.0] * 12
        }, index=range(12))  # Regular integer index
...
class TestCalculateIRR:
    ...
    def test_irr_basic_calculation(self, irr_cash_flows: pd.Series):
        ...
        assert 0.5 < irr < 3.0  # Adjusted range for this cash flow pattern with monthly compounding
...
class TestCalculateYieldOnCost:
    ...
    def test_yield_on_cost_basic(self):
        ...
        yield_on_cost = PropertyMetrics.calculate_yield_on_cost(stabilized_noi, total_cost)
...
class TestCalculateCashOnCashReturn:
    ...
    def test_cash_on_cash_basic(self):
        ...
        cash_on_cash = PropertyMetrics.calculate_cash_on_cash_return(first_year_cf, initial_equity)
...
class TestCalculateComprehensiveMetrics:
    ...
    def test_comprehensive_metrics_without_disposition(self, sample_cash_flows_df: pd.DataFrame):
    ...
    def test_comprehensive_metrics_with_disposition(self, sample_cash_flows_df: pd.DataFrame):
        ...
        expected_multiple = (expected_total_return + initial_investment) / initial_investment
    ...
    def test_comprehensive_metrics_disposition_date_not_in_index(self, sample_cash_flows_df: pd.DataFrame):
    ...
    def test_comprehensive_metrics_irr_failure(self, sample_monthly_index: pd.PeriodIndex):
    ...
    def test_comprehensive_metrics_short_cash_flows(self, sample_monthly_index: pd.PeriodIndex):
    ...
    def test_comprehensive_metrics_missing_columns(self, sample_monthly_index: pd.PeriodIndex):
...
class TestPropertyMetricsIntegration:
    ...
    def test_realistic_development_scenario(self):
        ...
        assert metrics['first_year_cash_flow'] < 84000.0  # Lower due to development phase
