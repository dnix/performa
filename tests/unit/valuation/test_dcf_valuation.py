...
class TestDCFValuation:
    ...
    def test_dcf_validation_growth_rate(self):
        ...
        with pytest.raises(ValidationError, match="Terminal growth rate.*should be between 0% and 10%"):
            DCFValuation(
                name="Test", discount_rate=0.15, terminal_cap_rate=0.065,
                hold_period_years=10, terminal_growth_rate=0.12
            )
        ...
        with pytest.raises(ValidationError, match="Input should be greater than or equal to 0"):
            DCFValuation(
                name="Test", discount_rate=0.08, terminal_cap_rate=0.065,
                hold_period_years=10, terminal_growth_rate=-0.01
            )
    ...
    def test_dcf_validation_reversion_costs(self):
        ...
        dcf = DCFValuation(
            name="Test", discount_rate=0.08, terminal_cap_rate=0.065, 
            hold_period_years=10, reversion_costs_rate=0.025
        )
        assert dcf.reversion_costs_rate == 0.025
        
        # Reversion costs too low (<0.5%) should fail
        with pytest.raises(ValidationError, match="Reversion costs rate.*should be between 0.5% and 10%"):
            DCFValuation(
                name="Test", discount_rate=0.08, terminal_cap_rate=0.065,
                hold_period_years=10, reversion_costs_rate=0.003
            )
        ...
        with pytest.raises(ValidationError, match="Reversion costs rate.*should be between 0.5% and 10%"):
            DCFValuation(
                name="Test", discount_rate=0.08, terminal_cap_rate=0.065,
                hold_period_years=10, reversion_costs_rate=0.12
            )
...
class TestDCFContextIntegration:
    ...
    def test_compute_cf_with_short_hold_period(self):
        ...
        mock_context = Mock()
        timeline = Timeline(start_date=datetime(2024, 1, 1), duration_months=60)  # 5 years
        mock_context.timeline = timeline
    ...
    def test_compute_cf_edge_case_zero_hold_period_end(self):
        ...
        mock_context = Mock()
        timeline = Timeline(start_date=datetime(2024, 1, 1), duration_months=6)  # Only 6 months
        mock_context.timeline = timeline
