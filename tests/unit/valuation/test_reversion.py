...
def sample_unlevered_analysis(sample_timeline: Timeline) -> UnleveredAnalysisResult:
    ...
    cash_flows = pd.DataFrame({
        ...
        UnleveredAggregateLineKey.TOTAL_OPERATING_EXPENSES.value: [35000.0] * 12 + [30000.0] * 12,
...
class TestParameterValidation:
    ...
    def test_valid_transaction_costs_range(self):
        ...
        for cost_rate in valid_costs:
            reversion = ReversionValuation(
                name="Test", 
                cap_rate=0.065,
                transaction_costs_rate=cost_rate
            )
    ...
    def test_invalid_transaction_costs_too_low(self):
        ...
        assert "Transaction costs rate (0.2%) should be between 0.5% and 10%" in str(exc_info.value)
    ...
    def test_invalid_transaction_costs_too_high(self):
        ...
        assert "Transaction costs rate (15.0%) should be between 0.5% and 10%" in str(exc_info.value)
    ...
    def test_invalid_asset_specific_cap_rates(self):
        ...
        assert "Cap rate for retail (25.0%) should be between 1% and 20%" in str(exc_info.value)
    ...
    def test_net_sale_proceeds_rate(self):
        ...
        for transaction_costs, expected_net_rate in test_cases:
            reversion = ReversionValuation(
                name="Test",
                cap_rate=0.065,
                transaction_costs_rate=transaction_costs
            )
...
class TestReversionMetrics:
    ...
    def test_calculate_metrics_basic(self):
        ...
        assert abs(metrics["profit_margin"] - ((net_proceeds - total_cost_basis) / total_cost_basis)) < 0.01
        assert metrics["reversion_cap_rate"] == 0.065
        assert abs(metrics["stabilized_yield_on_cost"] - (stabilized_noi / total_cost_basis)) < 0.001
    ...
    def test_calculate_metrics_zero_cost_basis(self):
        ...
        assert metrics["total_profit"] > 0  # Should still have profit (net proceeds - 0)


class TestCashFlowComputation:
    """Test cash flow computation (compute_cf method)."""
    
    def test_compute_cf_with_unlevered_analysis(self, mock_analysis_context, sample_unlevered_analysis):
    ...
    def test_compute_cf_with_resolved_lookups_fallback(self, mock_analysis_context, sample_timeline):
    ...
    def test_compute_cf_error_handling(self, mock_analysis_context):
        ...
        mock_analysis_context.unlevered_analysis = None
        mock_analysis_context.resolved_lookups = None  # This should cause an AttributeError
    ...
    def test_compute_cf_with_empty_noi_series_in_unlevered_analysis(self, mock_analysis_context, sample_timeline):
        ...
        mock_unlevered_analysis.get_series.return_value = pd.Series(dtype=float)  # Empty series
    ...
    def test_compute_cf_with_empty_resolved_lookups_fallback(self, mock_analysis_context, sample_timeline):
        ...
        assert isinstance(cf_series, pd.Series)
        assert (cf_series == 0.0).all()  # Should be all zeros due to empty fallback NOI series
    
    def test_compute_cf_with_invalid_resolved_lookups_fallback(self, mock_analysis_context, sample_timeline):
        ...
        assert isinstance(cf_series, pd.Series)
        assert (cf_series == 0.0).all()  # Should be all zeros due to invalid fallback data
...
class TestEdgeCasesAndErrorHandling:
    ...
    def test_very_high_noi_calculation(self):
        ...
        assert abs(gross_value - expected_gross) < 100000.0  # Allow for larger tolerance
...
class TestIntegrationScenarios:
    ...
    def test_mixed_use_reversion_workflow(self):
        ...
        assert abs(metrics["profit_margin"] - ((expected_net - total_cost_basis) / total_cost_basis)) < 0.01 