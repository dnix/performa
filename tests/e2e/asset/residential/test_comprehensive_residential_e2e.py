...
def test_e2e_institutional_scale_residential():
    ...
    property_model = ResidentialProperty(
        ...
        expenses=ResidentialExpenses(
            operating_expenses=operating_expenses, capital_expenses=capital_expenses
        ),
    ...
    expected_income = sum(
        spec.unit_count * spec.current_avg_monthly_rent for spec in unit_specs
    )
    ...
    print(
        f"   🚀 Processing Rate: {property_model.unit_count / analysis_time:.0f} units/second"
    )
...
def test_e2e_performance_stress_test():
    ...
    performance_rollover = ResidentialRolloverProfile(
        ...
        downtime_months=1,
        term_months=12,
        market_terms=ResidentialRolloverLeaseTerms(market_rent=2500.0, term_months=12),
        renewal_terms=ResidentialRolloverLeaseTerms(market_rent=2500.0, term_months=12),
    )
    ...
    assert (
        analysis_time < 5.0
    )  # Must complete under 5.0 seconds (adjusted for renovation logic)
    assert units_per_second > 100  # Must process at least 300 units/second
...
def test_e2e_vacant_units_lease_up_scenario():
    ...
    rent_roll = ResidentialRentRoll(
        unit_specs=occupied_specs, vacant_units=vacant_units
    )
    ...
    print(
        f"   💰 Current $/SF/Month: ${current_income / (property_model.net_rentable_area * 0.75):.2f}"
    )
    print(
        f"   🎯 Potential $/SF/Month: ${total_potential / property_model.net_rentable_area:.2f}"
    )
    ...
    print(
        f"   🎯 Successfully modeled {property_model.unit_mix.vacant_unit_count} vacant units"
    )
