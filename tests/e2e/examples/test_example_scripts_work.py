...
def test_value_add_renovation_example_works():
    ...
    assert (
        "ROLLING RENOVATION" in result.stdout
        or "Analysis completed successfully" in result.stdout
    )


@pytest.mark.parametrize(
    "expected_yield_range", [(20.0, 30.0)]
...
def test_manual_calculation_consistency():
    ...
    assert (
        10000 <= cost_per_unit <= 20000
    ), f"Cost per unit ${cost_per_unit} outside $10K-$20K range"

    rent_increase_pct = (post_reno_rent / current_rent - 1) * 100
    assert (
        20 <= rent_increase_pct <= 30
    ), f"Rent increase {rent_increase_pct}% outside 20-30% range"

    assert (
        0.20 <= yield_on_renovation <= 0.35
    ), f"Yield {yield_on_renovation:.1%} outside 20-35% range"
...
class TestExampleValidation:
    ...
    def test_example_script_validation_framework_exists(self):
        ...
        assert (
            "target_absorption_plan_id" in content
        ), "Should use absorption plan linking"
    ...
    def test_corrected_example_uses_proper_approach(self):
        ...
        assert (
            "upon_expiration='vacate'" not in content
        ), "Should not use old broken vacate approach"
    ...
    def test_no_more_fake_results(self):
        ...
        assert any(
            word in output_lower for word in ["year", "revenue", "noi", "analysis"]
