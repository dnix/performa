...
class SalesComparable(Model):
    ...
    cap_rate: Optional[PositiveFloat] = Field(
        default=None, description="Cap rate at sale"
    )
    noi: Optional[PositiveFloat] = Field(
        default=None, description="NOI at time of sale"
    )
...
class SalesCompValuation(Model):
    ...
    def calculate_statistics(self, use_adjustments: bool = True) -> Dict[str, float]:
        ...
        """
        data = (
            self.adjusted_price_per_sf_data
            if use_adjustments
            else self.price_per_sf_data
        )
    ...
    def calculate_value_range(
        self, subject_area: float, confidence_level: float = 0.95
    ) -> Dict[str, float]:
        """
        ...
        return {
            ...
            "range_pct": (margin_of_error / mean_price_per_sf * 100)
            if mean_price_per_sf > 0
            else 0.0,
        }
    ...
    def create_from_data(
        cls, name: str, sales_data: List[Dict], **kwargs
    ) -> "SalesCompValuation":
        """
        ...
        return cls(name=name, comparables=comparables, **kwargs)
