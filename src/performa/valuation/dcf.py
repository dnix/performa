...
class DCFValuation(Model):
    ...
    hold_period_years: int = Field(
        ..., description="Analysis period length in years"
    )
    ...
    reversion_costs_rate: PositiveFloat = Field(
        default=0.025, description="Transaction costs at sale as percentage of gross value"
    )
    ...
    def calculate_present_value(
        ...
        if self.terminal_growth_rate is not None:
            ...
            terminal_noi_with_growth = terminal_noi * ((1.0 + self.terminal_growth_rate) ** self.hold_period_years)
        ...
        return {
            ...
            "operations_percentage": (pv_operations / total_pv * 100) if total_pv > 0 else 0.0,
            "terminal_percentage": (pv_terminal / total_pv * 100) if total_pv > 0 else 0.0
        }
    ...
    def calculate_metrics(
        ...
        value_per_dollar = present_value / initial_investment if initial_investment > 0 else 0.0
        ...
        return {
            **dcf_results,
            ...
            "profit": npv,
            "profit_margin": npv / initial_investment if initial_investment > 0 else 0.0,
        }
    ...
    def calculate_sensitivity_analysis(
        ...
        discount_rates = [
            self.discount_rate + i * (discount_rate_range[1] - discount_rate_range[0]) / (steps - 1) 
            for i in range(steps)
        ]
        cap_rates = [
            self.terminal_cap_rate + i * (cap_rate_range[1] - cap_rate_range[0]) / (steps - 1)
            for i in range(steps)
        ]
        ...
        for disc_rate in discount_rates:
            for cap_rate in cap_rates:
                ...
                temp_dcf = self.model_copy(update={
            ...
            if hasattr(context, 'unlevered_analysis') and context.unlevered_analysis:
                ...
                if not noi_series.empty:
                    ...
                    if annual_noi:
                        ...
                        hold_period_end = min(self.hold_period_years * 12, len(context.timeline.period_index))
                        if hold_period_end > 0:
                            disposition_period = context.timeline.period_index[hold_period_end - 1]
