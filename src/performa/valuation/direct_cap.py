...
class DirectCapValuation(BaseValuation):
    ...
    kind: Literal["direct_cap"] = Field(
        default="direct_cap", description="Discriminator for polymorphic valuation"
    )
    ...
    def calculate_metrics(
        self, context: "DealContext", total_cost_basis: Optional[float] = None
    ) -> Dict[str, float]:
        """
    ...
    @classmethod
    def conservative(
        cls, name: str = "Conservative Valuation", cap_rate: float = 0.065, **kwargs
    ) -> "DirectCapValuation":
    ...
    @classmethod
    def aggressive(
        cls, name: str = "Aggressive Valuation", cap_rate: float = 0.055, **kwargs
    ) -> "DirectCapValuation":
