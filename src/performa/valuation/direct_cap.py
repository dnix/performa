...
class DirectCapValuation(Model):
    ...
    kind: Literal["direct_cap"] = Field(
        default="direct_cap",
        description="Discriminator field for polymorphic valuation",
    )

    # === VALUATION PARAMETERS ===
    cap_rate: PositiveFloat = Field(..., description="Cap rate for property valuation")
    ...
    def calculate_value(
        self, first_year_noi: float, potential_noi: Optional[float] = None
    ) -> Dict[str, float]:
        """
        ...
        return {
            ...
            "implied_yield": effective_noi / adjusted_value
            if adjusted_value > 0
            else 0.0,
        }
    ...
    def calculate_sensitivity_analysis(
        ...
        for cap_rate in cap_rates:
            ...
            results.append({
                ...
                "value_change_pct": (
                    value_result["property_value"]
                    / self.calculate_value(first_year_noi)["property_value"]
                    - 1
                )
                * 100,
            })
    ...
    def calculate_metrics(
        ...
        if acquisition_cost is not None:
            value_results.update({
                ...
                "value_premium_pct": (
                    value_results["property_value"] / acquisition_cost - 1
                )
                * 100
                if acquisition_cost > 0
                else 0.0,
                "acquisition_cap_rate": first_year_noi / acquisition_cost
                if acquisition_cost > 0
                else 0.0,
            })
    ...
    @classmethod
    def market_value(
        cls, cap_rate: float, name: str = "Market Value", **kwargs
    ) -> "DirectCapValuation":
        """Factory method for market value assessment."""
        return cls(name=name, cap_rate=cap_rate, **kwargs)
    ...
    def stabilized_value(
        ...
        **kwargs,
        ...
        return cls(
            name=name, cap_rate=cap_rate, occupancy_adjustment=occupancy_rate, **kwargs
        )

    @classmethod
    def conservative(
        cls, cap_rate: float = 0.075, name: str = "Conservative Value", **kwargs
    ) -> "DirectCapValuation":
    ...
    @classmethod
    def aggressive(
        cls, cap_rate: float = 0.055, name: str = "Aggressive Value", **kwargs
    ) -> "DirectCapValuation":
