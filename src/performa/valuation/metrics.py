...
class PropertyMetrics:
    ...
    def calculate_noi(
        ...
        if isinstance(revenues, pd.Series) and isinstance(operating_expenses, pd.Series):
            ...
            raise ValueError("Revenues and operating_expenses must be the same type (both Series or both numbers)")
    
    @staticmethod
    def calculate_stabilized_noi(
        cash_flows: pd.DataFrame,
        stabilization_period: Optional[pd.Period] = None
    ) -> float:
        """
        ...
        if stabilization_period in cash_flows.index:
            monthly_noi = revenues[stabilization_period] - expenses[stabilization_period]
            return monthly_noi * 12  # Annualize
        else:
            raise ValueError(f"Stabilization period {stabilization_period} not found in cash flows")
    ...
    def calculate_comprehensive_metrics(
        ...
        if len(net_cf) > 0:
            first_year_cf = net_cf.iloc[:12].sum() if len(net_cf) >= 12 else net_cf.sum()
            metrics['first_year_cash_flow'] = first_year_cf
            metrics['cash_on_cash_return'] = PropertyMetrics.calculate_cash_on_cash_return(
        ...
        if disposition_value is not None:
            ...
            metrics['total_return_multiple'] = (total_return + initial_investment) / initial_investment
