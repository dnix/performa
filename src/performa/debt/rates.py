...
class FloatingRate(Model):
    ...
    reset_frequency: Literal["monthly", "quarterly", "semi-annually", "annually"] = (
        Field(
            default="monthly",
...
AnyRateStructure = Annotated[
    Union[FixedRate, FloatingRate], Field(discriminator="rate_type")
]
...
class InterestRate(Model):
    ...
    def get_rate_for_period(
        self, period: pd.Period, index_curve: Optional[pd.Series] = None
    ) -> float:
        """
        ...
        try:
            if period in index_curve.index:
                ...
                if len(available_periods) == 0:
                    raise KeyError(
                        f"No index data available for period {period} or earlier"
                    )
    ...
    def __str__(self) -> str:
        ...
        if self.details.rate_type == "fixed":
            ...
            cap_str = (
                f" (cap: {self.details.interest_rate_cap:.3%})"
                if self.details.interest_rate_cap
                else ""
            )
            floor_str = (
                f" (floor: {self.details.interest_rate_floor:.3%})"
                if self.details.interest_rate_floor
                else ""
            )
