...
class DealFee(Model):
    ...
    timeline: Timeline = Field(..., description="Timeline for fee payment schedule")
    ...
    @classmethod
    def validate_value(
        cls, v: Union[PositiveFloat, pd.Series, Dict[date, PositiveFloat]]
    ) -> Union[PositiveFloat, pd.Series, Dict[date, PositiveFloat]]:
        ...
        if isinstance(v, pd.Series):
            ...
            for key, val in v.items():
                ...
                if not isinstance(val, (int, float)) or val < 0:
                    raise ValueError(
                        f"Fee value for {key} must be non-negative, got {val}"
                    )
            ...
            raise TypeError(
                f"Fee value must be a positive number, pandas Series, or date->value dict, got {type(v)}"
            )
    ...
    def calculate_total_fee_series(
        self, periods: int, index: Optional[pd.Index] = None
    ) -> pd.Series:
        """
        ...
        if isinstance(self.value, pd.Series):
            ...
            if index is None:
                raise ValueError(
                    "Index must be provided when value is a milestone dict"
                )
            if not isinstance(index, pd.PeriodIndex):
                raise ValueError(
                    "Index must be a PeriodIndex when value is a milestone dict"
                )
            ...
            if len(index) != periods:
                raise ValueError(
                    f"Index length ({len(index)}) must match periods ({periods})"
                )
