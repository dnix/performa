...
class DebtAnalyzer:
    ...
    def analyze_financing_structure(self, property_value_series: pd.Series, noi_series: pd.Series, unlevered_analysis: UnleveredAnalysisResult) -> FinancingAnalysisResult:
        """
    ...
    def _process_refinancing_transactions(self, property_value_series: pd.Series, noi_series: pd.Series) -> None:
        """
        ...
        if permanent_facility and covenant_params.get('monitoring_enabled', False):
            try:
                ...
                if len(monitoring_periods) > 0:
                    ...
                    property_value_series = pd.Series(0.0, index=self.timeline.period_index)
    ...
    def calculate_dscr_metrics(self, unlevered_analysis: UnleveredAnalysisResult) -> None:
        """
    ...
    def _extract_noi_time_series(self, unlevered_analysis: UnleveredAnalysisResult) -> pd.Series:
        """
    ...
    def _calculate_basic_dscr_fallback(self, error: Exception) -> None:
        ...
        self.financing_analysis.dscr_time_series = pd.Series(0.0, index=self.timeline.period_index)
