...
class DispositionAnalyzer:
    ...
    def process_disposition(
        ...
        self._record_gross_proceeds(
            ledger, gross_proceeds, deal_uid, asset_uid
        )
        ...
        for facility in debt_facilities:
            ...
            if outstanding > 0:
                ...
                self._record_debt_payoff(
                    ledger, facility.name, payoff_amount, 
                    disposition_date, deal_uid, asset_uid
                )
        ...
        try:
            ...
            if "Construction" in facility.name or "Renovation" in facility.name:
                ...
                refinance_payoff_mask = (
                    ...
                    (ledger_df["item_name"].str.contains("Refinancing Payoff", na=False)) &
                ...
                if refinance_payoff_mask.any():
                    ...
                    payoff_amount = abs(ledger_df[refinance_payoff_mask]["amount"].sum())
                    logger.debug(f"{facility.name} was paid off during refinancing: ${payoff_amount:,.0f}")
                    return 0.0
            ...
            # Get all debt service payments (includes both interest and principal)
            service_mask = (
                (ledger_df["item_name"].str.contains(facility.name, na=False)) &
                (ledger_df["subcategory"] == FinancingSubcategoryEnum.DEBT_SERVICE)
            )
            ...
            if "Permanent" in facility.name or hasattr(facility, "amortization_months"):
                ...
                original_loan = facility.loan_amount if hasattr(facility, "loan_amount") and facility.loan_amount else proceeds
            ...
            # Fallback to facility method if available
            if hasattr(facility, "get_outstanding_balance"):
                balance = facility.get_outstanding_balance(disposition_date)
                # But cap it at loan_amount to avoid wild estimates
                if hasattr(facility, "loan_amount") and facility.loan_amount:
                    return min(balance, facility.loan_amount)
                return balance if balance > 0 else 0.0
            
            # Last resort: use loan_amount if available
            return getattr(facility, "loan_amount", 0.0) or 0.0
