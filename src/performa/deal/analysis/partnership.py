...
class PartnershipAnalyzer:
    ...
    def calculate_partner_distributions(self, levered_cash_flows: pd.Series) -> PartnerDistributionResult:
        """
        ...
        if not self.deal.has_equity_partners:
            ...
            self.partner_distributions = self._calculate_single_entity_distributions(levered_cash_flows)
    ...
    def _calculate_single_entity_distributions(self, cash_flows: pd.Series) -> SingleEntityDistributionResult:
        """
        ...
        if len(remaining_cash_flows) > 1 and total_investment > 0:
            try:
                dates = [period.to_timestamp().date() for period in remaining_cash_flows.index]
    ...
    def _calculate_fee_distributions(self, cash_flows: pd.Series) -> Dict[str, Any]:
        ...
        if self.deal.has_equity_partners:
            for partner in self.deal.equity_partners.partners:
                ...
                fee_cash_flows_by_partner[partner.name] = pd.Series(0.0, index=self.timeline.period_index)
        ...
        if self.deal.deal_fees:
            for fee in self.deal.deal_fees:
                try:
                    ...
                    is_partner = False
                    if self.deal.has_equity_partners:
                        partner_names = [p.name for p in self.deal.equity_partners.partners]
                    ...
                    fee_cf_reindexed = fee_cf.reindex(self.timeline.period_index, fill_value=0.0)
                    ...
                    if is_partner:
                        ...
                        if payee_name not in partner_fees_by_partner:
                            ...
                            fee_cash_flows_by_partner[payee_name] = pd.Series(0.0, index=self.timeline.period_index)
                        ...
                        if payee_name not in third_party_fees:
                            ...
                            third_party_fee_cash_flows[payee_name] = pd.Series(0.0, index=self.timeline.period_index)
                        
                        third_party_fees[payee_name] += fee_amount
                        total_third_party_fees += fee_amount
                        third_party_fee_details[payee_name][fee_name] = float(fee_amount)
        ...
        return {
            ...
            "developer_fee_by_partner": {payee: amount for payee, amount in partner_fees_by_partner.items()},
        }
    ...
    def _combine_fee_and_waterfall_results(
        ...
        if "total_metrics" in combined_results:
            combined_results["total_metrics"]["total_distributions"] += fee_details["total_partner_fees"]
        ...
        combined_results["waterfall_details"]["total_fees"] = fee_details["total_partner_fees"]
        ...
        if "total_metrics" in combined_results:
            ...
            combined_results["total_distributions"] = metrics.get("total_distributions", 0.0)
        ...
        if "partner_distributions" in combined_results:
            partner_results = {}
            for partner_name, partner_metrics in combined_results["partner_distributions"].items():
                partner_results[partner_name] = {
                    ...
                    "total_distributions": partner_metrics.get("total_distributions", 0.0),
                    ...
                    "ownership_percentage": partner_metrics.get("ownership_percentage", 0.0),
                    "distributions_from_fees": 0.0,  # Add fee-specific fields
                    "distributions_from_waterfall": partner_metrics.get("total_distributions", 0.0),  # Add waterfall-specific field
                    "developer_fee": 0.0,
                    "fee_details": {},  # Empty dictionary by default
                    "fee_cash_flows": pd.Series(0.0, index=self.timeline.period_index),  # Zero by default
                    "fee_count": 0,  # Add fee count field
                }
            
            # Add fee distributions to partner results
            if "partner_fees_by_partner" in fee_details:
                for payee_name, fee_amount in fee_details["partner_fees_by_partner"].items():
                    if payee_name in partner_results:
                        partner_results[payee_name]["distributions_from_fees"] = fee_amount
                        partner_results[payee_name]["developer_fee"] = fee_amount
                        
                        # Use the fee details from the dictionary format
                        fee_details_dict = fee_details["fee_details_by_partner"].get(payee_name, {})
                        partner_results[payee_name]["fee_details"] = fee_details_dict
                        partner_results[payee_name]["fee_count"] = len(fee_details_dict)
                        partner_results[payee_name]["fee_cash_flows"] = fee_details["fee_cash_flows_by_partner"].get(payee_name, pd.Series(0.0, index=self.timeline.period_index))
                        
                        # Update total distributions to include fees
                        partner_results[payee_name]["total_distributions"] = partner_results[payee_name]["distributions_from_waterfall"] + fee_amount
