...
class ValuationEngine:
    ...
    def extract_property_value_series(self, unlevered_analysis: UnleveredAnalysisResult) -> pd.Series:
        """
        ...
        if unlevered_analysis.cash_flows is not None and hasattr(unlevered_analysis.cash_flows, 'columns'):
            # Search for value-related columns in the actual data
            value_columns = [col for col in unlevered_analysis.cash_flows.columns 
                           if any(term in col.lower() for term in ['value', 'asset_value', 'property_value'])]
            if value_columns:
                # Use the first available value column
                self.property_value_series = unlevered_analysis.cash_flows[value_columns[0]].reindex(
        ...
        try:
            ...
            if not noi_series.empty and noi_series.sum() > 0:
                ...
                cap_rate = 0.065  # 6.5% default market cap rate (institutional standard)
                
                if self.deal.exit_valuation and hasattr(self.deal.exit_valuation, 'cap_rate'):
        ...
        if self.deal.acquisition and hasattr(self.deal.acquisition, 'acquisition_cost'):
            ...
            self.property_value_series = pd.Series(values, index=self.timeline.period_index)
            return self.property_value_series
    ...
    def extract_noi_series(self, unlevered_analysis: UnleveredAnalysisResult) -> pd.Series:
        """
    ...
    def calculate_disposition_proceeds(self, unlevered_analysis: UnleveredAnalysisResult = None) -> pd.Series:
        """
