...
class CashFlowEngine:
    ...
    def calculate_levered_cash_flows(
        ...
        if disposition_proceeds is None:
            disposition_proceeds = self._calculate_disposition_proceeds(unlevered_analysis)
    ...
    def _execute_funding_cascade(self, base_uses: pd.Series, funding_components: Dict[str, Any]) -> Dict[str, Any]:
        ...
        funding_components["equity_contributions"] = pd.Series(0.0, index=self.timeline.period_index)
        funding_components["debt_draws"] = pd.Series(0.0, index=self.timeline.period_index)
        funding_components["loan_proceeds"] = pd.Series(0.0, index=self.timeline.period_index)
        funding_components["interest_expense"] = pd.Series(0.0, index=self.timeline.period_index)
        funding_components["compounded_interest"] = pd.Series(0.0, index=self.timeline.period_index)
        funding_components["equity_cumulative"] = pd.Series(0.0, index=self.timeline.period_index)
            ...
            for facility in self.deal.financing.facilities:
                if hasattr(facility, 'kind') and facility.kind == 'construction':
                    ...
                    if facility.tranches:
                        ...
                        if hasattr(tranche, 'interest_rate') and hasattr(tranche.interest_rate, 'effective_rate'):
                            cash_annual_rate = float(tranche.interest_rate.effective_rate)
                    break
        ...
        for i, period in enumerate(self.timeline.period_index):
            ...
            if i > 0:
                ...
                if previous_balance > 0:
                    ...
                    if fund_interest_from_reserve:
                        ...
                        funding_components["interest_reserve_utilized"][period] = cash_interest
                        ...
                        funding_components["compounded_interest"][period] += cash_interest
            ...
            if period_uses > 0:
                ...
                period_equity, period_debt = self._fund_period_uses(
                    period_uses, equity_target, equity_funded_cumulative, debt_funded_cumulative, 
                    i, funding_components, working_uses
                )
    ...
    def _fund_period_uses(
        self, period_uses: float, equity_target: float, 
        equity_funded: float, debt_funded: float, 
        period_idx: int, funding_components: Dict[str, Any], working_uses: pd.Series
    ) -> tuple[float, float]:
        """
    ...
    def _calculate_multi_tranche_debt(
        self, debt_needed: float, period_idx: int, funding_components: Dict[str, Any], working_uses: pd.Series
    ) -> float:
        """
        ...
        try:
            ...
            if construction_facility.tranches:
                ...
                if debt_draw > 0:
                    funding_components["debt_draws_by_tranche"][first_tranche.name].iloc[period_idx] = debt_draw
    ...
    def _assemble_levered_cash_flow_results(
        ...
        if hasattr(financing_analysis, 'refinancing_cash_flows'):
            ...
            loan_payoff_series += refinancing_loan_payoffs.reindex(self.timeline.period_index, fill_value=0.0)
        ...
        cash_flow_components = CashFlowComponents(
            ...
            acquisition_costs=funding_components["total_uses"],  # Total uses (positive outflows)
            ...
            equity_contributions=funding_components["equity_contributions"],  # Positive values (outflows)
        ...
        cash_flow_summary = CashFlowSummary(
            ...
            total_distributions=self._calculate_total_distributions(unlevered_analysis, disposition_proceeds),
        ...
        interest_reserve_capacity = self._calculate_interest_reserve_capacity(cascade_results["total_project_cost"])
        ...
        interest_reserve_details = InterestReserveDetails(
            ...
            interest_reserve_capacity=pd.Series(interest_reserve_capacity, index=self.timeline.period_index),
    ...
    def _extract_unlevered_cash_flows(self, unlevered_analysis: UnleveredAnalysisResult) -> pd.Series:
        """
    ...
    def _calculate_debt_service_series(self, financing_analysis: FinancingAnalysisResult) -> pd.Series:
        """
    ...
    def _calculate_loan_payoff_series(self, financing_analysis: FinancingAnalysisResult) -> pd.Series:
        """
                    ...
                    if hasattr(facility, 'get_outstanding_balance'):
                        for period in self.timeline.period_index:
                            ...
                            if balance > 0:
                                ...
                                if self.deal.exit_valuation and hasattr(self.deal.exit_valuation, 'disposition_date') and period == self.deal.exit_valuation.disposition_date:
    ...
    def _calculate_total_distributions(self, unlevered_analysis: UnleveredAnalysisResult, disposition_proceeds: pd.Series) -> float:
        """
    ...
    def _calculate_disposition_proceeds(self, unlevered_analysis: UnleveredAnalysisResult = None) -> pd.Series:
        """
                ...
                if unlevered_analysis:
                    ...
                    if hasattr(context, 'resolved_lookups'):
                        try:
                            ...
                            context.resolved_lookups[UnleveredAggregateLineKey.NET_OPERATING_INCOME.value] = noi_series
                        except Exception:
                            pass
