...
class VacancyLossConfig(Model):
    ...
    rate: FloatBetween0And1 = Field(
        default=0.05, 
        description="Vacancy rate as decimal (e.g., 0.05 for 5%)"
    )
    ...
    reduce_by_rollover_vacancy: bool = Field(
        default=True,
        description="Reduce by vacancy already accounted for in rollovers"
    )
...
class CreditLossConfig(Model):
    ...
    rate: FloatBetween0And1 = Field(
        default=0.01, 
        description="Credit loss rate as decimal (e.g., 0.01 for 1%)"
    )
...
class Losses(Model):
    ...
    vacancy: Optional[VacancyLossConfig] = Field(
        default_factory=VacancyLossConfig,
        description="Vacancy loss configuration"
    )
    collection: Optional[CreditLossConfig] = Field(
        default_factory=CreditLossConfig,
...
class VacancyLossModel(CashFlowModel):
    ...
    reference: UnleveredAggregateLineKey = UnleveredAggregateLineKey.POTENTIAL_GROSS_REVENUE
