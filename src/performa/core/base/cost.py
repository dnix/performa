...
class CommissionTier(Model):
    ...
    def validate_payment_percentages(self) -> "CommissionTier":
        ...
        if not math.isclose(
            self.signing_percentage + self.commencement_percentage, 1.0
        ):
            raise ValueError(
...
class TenantImprovementAllowanceBase(CashFlowModel):
    ...
    def compute_cf(self, context: AnalysisContext) -> pd.Series:
        ...
        if self.reference is None:
            ...
            if self.reference == PropertyAttributeKey.NET_RENTABLE_AREA:
                ...
                if area:
                    ...
                    raise ValueError(
                        "Area required for NET_RENTABLE_AREA TI calculation but not available from lease context or TI model"
                    )
            else:
                ...
                if context.current_lease and hasattr(
                    context.current_lease, attribute_name
                ):
        ...
        if self.payment_method == "upfront":
            ...
            monthly_amount = (
                total_amount / len(self.timeline.period_index)
                if not self.timeline.period_index.empty
                else 0
            )
