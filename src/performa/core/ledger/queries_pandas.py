...
class LedgerQueries:  # noqa: PLR0904 (ignore too many public methods)
    ...
    def __init__(self, ledger):
        """Initialize with ledger DataFrame or Ledger object."""
        # Handle both DataFrame and Ledger objects
        if hasattr(ledger, 'ledger_df'):
            # It's a Ledger object, get the DataFrame
    ...
    def operational_cash_flow(self) -> pd.Series:
        ...
        unified_index = self._create_unified_index(
            noi_series, capex_series, ti_series, lc_series
        )
    ...
    def debt_balance(self) -> pd.Series:
        ...
        if not debt_increases.empty:
            ...
            increases_df["balance_impact"] = increases_df[
                "amount"
        ...
        if not principal_payments.empty:
            ...
            principal_df["balance_impact"] = principal_df[
                "amount"
            ]  # Already negative, reduces balance
            balance_impacts.append(principal_df)

        if not debt_payoffs.empty:
            payoffs_df = debt_payoffs.copy()
            payoffs_df["balance_impact"] = payoffs_df[
                "amount"
            ]  # Already negative, reduces balance
    ...
    def construction_draws(self) -> pd.Series:
        ...
        construction_flows = self.ledger[
            ...
            & (
                ...
                (
                    self.ledger["subcategory"]
                    .astype(str)
                    .str.contains(
                        "|".join(construction_subcategories), case=False, na=False
                    )
                )
                |
