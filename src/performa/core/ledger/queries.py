...
class LedgerQueries:  # noqa: PLR0904 (ignore too many public methods)
    ...
    def __init__(self, ledger: pd.DataFrame):
        ...
        if ('date' in ledger.columns and 
            ledger.index.names and 'date' in ledger.index.names):
    ...
    def capex(self) -> pd.Series:
        ...
        capex = capital_txns[
            ~capital_txns['item_name'].str.contains(r'^TI\b|Tenant Improvement|^LC\b|Leasing Commission', 
    ...
    def ti(self) -> pd.Series:
        ...
        ti = self.ledger[
            self.ledger['item_name'].str.contains(r'^TI\b|Tenant Improvement', 
                                                 case=False, na=False, regex=True)
        ]
        if ti.empty:
    ...
    def lc(self) -> pd.Series:
        ...
        lc = self.ledger[
            self.ledger['item_name'].str.contains(r'^LC\b|Leasing Commission', 
                                                 case=False, na=False, regex=True)
        ]
        if lc.empty:
    ...
    def misc_income(self) -> pd.Series:
        ...
        misc_txns = self.ledger[
            (self.ledger['flow_purpose'] == 'Operating') &
            (self.ledger['subcategory'] == 'Miscellaneous')  # Fixed: use correct enum value
        ]
        if misc_txns.empty:
    ...
    def expense_reimbursements(self) -> pd.Series:
        ...
        reimburse_txns = self.ledger[
            (self.ledger['flow_purpose'] == 'Operating') &
            (self.ledger['subcategory'] == 'Recovery')  # Updated to match RevenueSubcategoryEnum.RECOVERY
        ]
