    ...
    def convert_all(
        ...
        for series, metadata in series_batch:
            try:
                ...
                raise ValueError(
                    f"Batch conversion failed for '{metadata.item_name}': {e}"
                ) from e
        ...
        if record_count > 0:
            ...
            logger.debug(
                f"Converted {record_count} records in {elapsed:.1f}ms ({ms_per_record:.3f}ms/record)"
            )
            ...
            if not isinstance(series.index, (pd.DatetimeIndex, pd.PeriodIndex)):
                ...
                try:
                    ...
                    if all(isinstance(x, datetime_like_types) for x in series.index):
                        ...
                        raise ValueError(
                            f"Series index must be datetime-like, got {type(series.index)} with elements: {[type(x).__name__ for x in series.index[:3]]}"
                        )
