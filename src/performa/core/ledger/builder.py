...
class LedgerBuilder:
    ...
    series_batch: List[Tuple[pd.Series, SeriesMetadata]] = field(default_factory=list, init=False)
    
    # Ledger ownership - the builder owns the DataFrame
    _current_ledger: Optional[pd.DataFrame] = field(default=None, init=False, repr=False)
    ...
    def to_dataframe(self) -> pd.DataFrame:
        ...
        if self.series_batch:
            try:
                batch_records = SeriesBatchConverter.convert_all(
                    self.series_batch,
                    skip_zeros=self.settings.skip_zero_values
                )
        ...
        if self.settings.enable_smart_indexing and len(df) > self.settings.large_ledger_threshold:
    ...
    def _empty_ledger(self) -> pd.DataFrame:
        ...
        return pd.DataFrame(columns=[
            'transaction_id', 'date', 'amount', 'flow_purpose', 
            'category', 'subcategory', 'item_name', 'source_id',
            'asset_id', 'pass_num', 'deal_id', 'entity_id', 'entity_type'
        ])
    ...
    def _apply_categorical_dtypes(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        categorical_columns = [
            'flow_purpose', 'category', 'subcategory', 
    ...
    def _validate_batch(self, records: List[TransactionRecord]) -> List[TransactionRecord]:
    ...
    def estimate_final_count(self) -> int:
        ...
        estimated_from_series = SeriesBatchConverter.estimate_record_count(
            self.series_batch, 
            self.settings.skip_zero_values
        )
