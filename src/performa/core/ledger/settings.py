...
class LedgerGenerationSettings(BaseModel):
    ...
    validate_metadata: bool = Field(
        default=True, description="Validate SeriesMetadata before conversion to records"
    )
    ...
    preserve_precision: bool = Field(
        default=True, description="Preserve full floating point precision in amounts"
    )

    include_transaction_ids: bool = Field(
        default=True, description="Include unique transaction IDs for auditability"
    )
    ...
    def memory_optimized(self) -> bool:
        ...
        return (
            self.skip_zero_values
            and self.use_categorical_dtypes
            and self.clear_intermediate_data
        )
    ...
    def performance_optimized(self) -> bool:
        ...
        return (
            self.enable_smart_indexing
            and self.batch_size >= 1000
            and not self.validate_transactions  # Validation has performance cost
        )
