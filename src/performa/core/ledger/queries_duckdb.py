...
This module provides DuckDB-based implementations of ledger query methods,
offering significant performance improvements for large datasets while
maintaining complete compatibility with the pandas-based implementation.

Key Features:
    - SQL-optimized queries for financial calculations
    - Identical API to pandas implementation
    - Automatic fallback to pandas for complex operations
    - Support for standard real estate financial metrics
    - Efficient handling of time series data

Usage:
    The DuckDB backend is automatically selected when USE_DUCKDB_LEDGER 
    environment variable is set to 'true'. All calculations produce
    identical results to the pandas implementation.
...
Example:
    ...
    # All methods work identically to pandas version
    irr = queries.levered_cash_flow().sum()
    noi = queries.noi()
    ```
"""
...
class LedgerQueries:  # noqa: PLR0904
    ...
    **Usage Examples:**
    ...
    def __init__(self, ledger: "Ledger"):
        ...
        self._ledger = ledger  # Keep reference for DataFrame access

    @property
    def ledger(self) -> pd.DataFrame:
        """
        Access the ledger as a pandas DataFrame for complex filtering operations.
        
        This property provides backward compatibility for code that needs to
        perform complex DataFrame operations not covered by the query methods.
    ...
    def ucf(self) -> pd.Series:
        ...
        Uses optimized pandas implementation for reliable performance.
        Legacy method maintained for backward compatibility.

        Returns:
            Time series of unlevered cash flow by period
        """
        # EXACT PANDAS LOGIC FROM queries_main.py lines 448-453
