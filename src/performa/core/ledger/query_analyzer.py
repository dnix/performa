...
class DuckDBQueryAnalyzer:
    ...
    def _parse_execution_plan(
        self, query: str, plan_lines: List[str]
        ...
        for line in plan_lines:
            ...
            if any(
                op in line_upper for op in ["HASH_GROUP_BY", "SORT", "FILTER", "JOIN"]
            ):
        ...
        if has_seq_scan and not has_index_usage:
            recommendations.append(
                "Query is using sequential scans - consider adding strategic indexes"
            )

        if total_time_ms > 100.0:  # Slow query threshold
            recommendations.append(
                "Query is slow - consider query optimization or additional indexes"
            )

        if "GROUP BY" in query.upper() and has_seq_scan:
            recommendations.append(
                "GROUP BY queries benefit from indexes on grouping columns"
            )

        if "WHERE" in query.upper() and has_seq_scan:
            recommendations.append(
                "WHERE clause filtering would benefit from indexes on filtered columns"
            )
        ...
        return QueryPerformanceMetrics(
            ...
            if recommendations
            else ["Query appears to be well-optimized"],
        )
    ...
    def recommend_indexes(
        ...
        for query in common_queries:
            ...
            if "WHERE" in query_upper:
                ...
                if "DATE" in query_upper and (
                    ">" in query or "<" in query or "BETWEEN" in query_upper
                ):
                    recommendations.append(
                        IndexRecommendation(
