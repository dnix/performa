...
class CashFlowModel(Model):
    ...
    @classmethod
    def validate_value(
        cls, v: Union[PositiveFloat, pd.Series, Dict, List]
        ...
        if isinstance(v, pd.Series):
            ...
            if (v < 0).any():
                raise ValueError(
                    "All values in CashFlowModel Series must be non-negative"
                )
    ...
    def _cast_to_flow(self, value: Union[float, pd.Series, Dict, List]) -> pd.Series:
        ...
        if isinstance(value, dict):
            ...
            return self._align_flow_series(
                value
            )  # Align series to the model's timeline
        elif isinstance(value, list):
            if len(value) != len(periods):
                raise ValueError(
                    f"List length {len(value)} does not match timeline length {len(periods)}."
                )
    ...
    def compute_cf(
        ...
        if self.reference is None:
            ...
            if dependency_cf is None:
                raise ValueError(
                    f"Unresolved aggregate dependency for '{self.name}': {self.reference.value}"
                )
