...
class ValidationMixin:
    ...
    def validate_conditional_requirement(
        ...
        if condition_value in condition_values and required_value is None:
            msg = (
                error_message
                or f"{required_field} is required when {condition_field} is {condition_value}"
            )
            raise ValueError(msg)
...
def validate_term_specification(cls, data: Any) -> Any:
    ...
    if isinstance(data, dict):
        ...
        ValidationMixin.validate_date_ordering(
            data, "start_date", "end_date", "end_date must be after start_date"
        )
        return data
    else:
...
def validate_mutual_exclusivity(
    field_a: str, field_b: str, error_message: Optional[str] = None
):
    """
    ...
    def validator(cls, data: Any) -> Any:
        if isinstance(data, dict):
            ValidationMixin.validate_either_or_required(
                data, field_a, field_b, error_message
            )
        else:
            # For model instances
...
def validate_monthly_period_index(
    series: pd.Series, field_name: str = "series"
) -> pd.Series:
    """
