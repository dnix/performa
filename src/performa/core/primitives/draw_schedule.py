...
class DrawScheduleParameters(Model):
    ...
    periods: PositiveInt = Field(gt=0, description="Number of periods to distribute over")
    index: Optional[pd.PeriodIndex] = Field(default=None, description="Optional PeriodIndex for the resulting Series")
...
class DrawSchedule(Model, ValidationMixin, ABC):
    ...
    def apply_to_amount(
        ...
        amount: float, 
        periods: int, 
        index: Optional[pd.PeriodIndex] = None
    ) -> pd.Series:
        """
...
class ManualDrawSchedule(DrawSchedule):
    ...
    def validate_values(cls, v: List[Union[int, float]]) -> List[Union[int, float]]:
        ...
        for i, val in enumerate(v):
            if val <= 0:
                raise ValueError(f"All values must be positive, but value at index {i} is {val}")
    ...
    @classmethod
    def create_for_timeline(cls, values: List[Union[int, float]], timeline) -> "ManualDrawSchedule":
        """
    ...
    @classmethod
    def create_for_periods(cls, values: List[Union[int, float]], periods: int) -> "ManualDrawSchedule":
        """
...
class FirstLastDrawSchedule(DrawSchedule):
    ...
    def effective_first_percentage(self) -> float:
        ...
        if self.first_percentage is not None:
            ...
            return round(1.0 - float(self.last_percentage), 10)  # Round to avoid float precision issues
        else:
