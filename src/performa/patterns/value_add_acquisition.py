...
class ValueAddAcquisitionPattern(PatternBase):
    ...
    def _derive_timeline(self) -> Timeline:
        """Derive timeline from acquisition date and hold period."""
        start_date = self.analysis_start_date or self.acquisition_date
        duration_months = self.analysis_duration_months or (
            self.hold_period_years * 12 + 12
        )

        return Timeline(start_date=start_date, duration_months=duration_months)
