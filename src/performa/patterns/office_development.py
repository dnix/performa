...
class OfficeDevelopmentPattern(DevelopmentPatternBase):
    ...
    leasing_frequency_months: PositiveInt = Field(
        ...
        ge=1,
        le=12,
        description="Months between new lease executions"
    )
    stabilized_occupancy_rate: FloatBetween0And1 = Field(
        default=0.95,
        description="Target occupancy rate at stabilization"
    )
    
    # === CONSTRUCTION COST MODEL ===
    construction_cost_psf: PositiveFloat = Field(
        default=350.0,
        description="Construction cost per square foot of gross area"
    )
    soft_costs_rate: FloatBetween0And1 = Field(
        default=0.15,
        description="Soft costs as percentage of hard construction costs"
    )
    ...
    def developer_fee(self) -> float:
        ...
        return (self.hard_construction_costs + self.soft_costs) * self.developer_fee_rate
    ...
    def create(self) -> Deal:
        ...
        total_timeline_months = max(
            self.construction_start_months + self.construction_duration_months + 12,  # +12 for stabilization
        ...
        construction_loan = ConstructionFacility(
            ...
            tranches=[
                DebtTranche(
                    ...
                    interest_rate=InterestRate(details=FixedRate(rate=self.construction_interest_rate)),
                    fee_rate=self.construction_fee_rate,
                    ltc_threshold=self.construction_ltc_ratio,
                )
            ],
            interest_calculation_method=getattr(InterestCalculationMethod, self.interest_calculation_method),
        ...
        permanent_loan = PermanentFacility(
            ...
            interest_rate=InterestRate(details=FixedRate(rate=self.permanent_interest_rate)),
        ...
        partnership = PartnershipStructure(
            ...
            ) if self.distribution_method == "waterfall" else None,
        )
