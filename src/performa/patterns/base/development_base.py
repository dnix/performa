...
class DevelopmentPatternBase(PatternBase):
    ...
    land_closing_costs_rate: FloatBetween0And1 = Field(
        default=0.025, 
        description="Land closing costs as percentage of land cost"
    )
    ...
    construction_interest_rate: FloatBetween0And1 = Field(
        default=0.065,
        description="Interest rate for construction loan"
    )
    construction_fee_rate: FloatBetween0And1 = Field(
        default=0.01,
        description="Origination fee rate for construction loan"
    )
    interest_calculation_method: Literal["NONE", "SIMPLE", "SCHEDULED", "ITERATIVE"] = Field(
        default="SCHEDULED",
        description="Construction loan interest calculation method"
    )
    interest_reserve_rate: FloatBetween0And1 = Field(
        default=0.15,
        description="Interest reserve as percentage of loan amount"
    )
    ...
    permanent_ltv_ratio: FloatBetween0And1 = Field(
        default=0.70,
        le=0.80,
        description="Loan-to-value ratio for permanent financing"
    )
    permanent_interest_rate: FloatBetween0And1 = Field(
        default=0.055,
        description="Interest rate for permanent loan"
    )
    permanent_loan_term_years: PositiveInt = Field(
        default=10,
        ge=5,
        le=30,
        description="Permanent loan term in years"
    )
    ...
    distribution_method: Literal["pari_passu", "waterfall"] = Field(
        default="waterfall",
        description="Partnership distribution methodology"
    )
    gp_share: FloatBetween0And1 = Field(
        default=0.10,
        description="General Partner ownership percentage"
    )
    lp_share: FloatBetween0And1 = Field(
        default=0.90,
        description="Limited Partner ownership percentage"
    )
    preferred_return: FloatBetween0And1 = Field(
        default=0.08,
        description="LP preferred return rate (for waterfall structures)"
    )
    promote_tier_1: FloatBetween0And1 = Field(
        default=0.20,
        description="GP promote rate above preferred return"
    )
    ...
    exit_cap_rate: FloatBetween0And1 = Field(
        default=0.065,
        description="Exit capitalization rate for sale valuation"
    )
    exit_costs_rate: FloatBetween0And1 = Field(
        default=0.025,
        description="Exit transaction costs as percentage of sale price"
    )
