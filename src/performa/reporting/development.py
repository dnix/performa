...
class SourcesAndUsesReport(Report):
    ...
    def __init__(
        self, development_project, template: Optional[ReportTemplate] = None, **kwargs
    ):
        super().__init__(
    ...
    @classmethod
    def from_development_project(
        cls, development_project, template: Optional[ReportTemplate] = None
    ):
    ...
    def generate_data(self) -> Dict[str, Any]:
        ...
        return {
            "project_info": {
                ...
                "asset_type": str(self.project.property_type).replace(
                    "AssetTypeEnum.", ""
                ),
            ...
            "uses": {
                ...
                "Direct Construction Costs": self.format_currency(
            ...
            "sources": {
                ...
                if total_uses > 0
                else f"{self.format_currency(sources.get('equity', 0))} (0.0%)",
            ...
            "key_metrics": {
                "Loan-to-Cost (Senior)": self.format_percentage(
                    sources.get("senior_debt", 0) / total_uses if total_uses > 0 else 0
                ),
                "Total Leverage": self.format_percentage(
                    (sources.get("senior_debt", 0) + sources.get("mezzanine", 0))
                    / total_uses
                    if total_uses > 0
                    else 0
                ),
                "Equity Requirement": self.format_percentage(
    ...
    def _categorize_project_uses(self, capital_plan) -> Dict[str, float]:
        ...
        for item in capital_plan.capital_items:
            ...
            if any(term in work_type for term in ["land", "acquisition", "purchase"]):
                ...
                term in work_type
                for term in ["construction", "building", "renovation", "improvement"]
            ):
                uses["hard_costs"] += value
            elif any(
                term in work_type
                for term in ["fee", "legal", "permit", "design", "architect"]
            ):
    ...
    def _calculate_cost_per_unit(self) -> str:
        ...
        try:
            ...
            if hasattr(program, "total_units"):
                cost_per_unit = (
                    self.project.construction_plan.total_cost / program.total_units
                )
    ...
    def _calculate_cost_per_sf(self) -> str:
        ...
        try:
            ...
            if hasattr(program, "total_area"):
                cost_per_sf = (
                    self.project.construction_plan.total_cost / program.total_area
                )
...
class DevelopmentSummaryReport(Report):
    ...
    def generate_data(self) -> Dict[str, Any]:
        ...
        return {
            "project_overview": {
                ...
                "Asset Type": str(self.project.property_type).replace(
                    "AssetTypeEnum.", ""
                ),
            ...
            "financial_summary": {
                ...
                "Estimated Stabilized NOI": self.format_currency(
                    estimated_stabilized_noi
                ),
                "Estimated Stabilized Value": self.format_currency(estimated_value),
                "Estimated Profit on Cost": self.format_percentage(
                    estimated_stabilized_noi / total_cost
                ),
                "Estimated Development Margin": self.format_currency(
    ...
    def _get_construction_start_date(self) -> str:
        ...
        try:
            ...
            if capital_plan.capital_items:
                earliest_start = min(
                    item.timeline.start_date for item in capital_plan.capital_items
                )
    ...
    def _get_estimated_completion_date(self) -> str:
        ...
        try:
            ...
            if capital_plan.capital_items:
                latest_end = max(
                    item.timeline.end_date for item in capital_plan.capital_items
                )
    ...
    def _get_leasing_summary(self) -> Dict[str, str]:
        ...
        try:
            ...
            absorption_plan = (
                self.project.blueprints[0].absorption_plan
                if self.project.blueprints
                else None
            )
            if absorption_plan:
                return {
    ...
    def _get_construction_summary(self) -> Dict[str, str]:
        ...
        hard_costs = sum(
            ...
            and any(
                term in (item.work_type or "").lower()
...
class ConstructionDrawReport(Report):
    ...
    def generate_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "period_costs": {
                "Direct Construction Costs": self.format_currency(
                    period_costs.get("construction", 0)
                ),
                "Soft Costs & Professional Fees": self.format_currency(
                    period_costs.get("soft_costs", 0)
                ),
                "Developer Fee": self.format_currency(
                    period_costs.get("developer_fee", 0)
                ),
                "Interest & Financing Costs": self.format_currency(
                    period_costs.get("financing", 0)
                ),
                "Contingency Utilization": self.format_currency(
                    period_costs.get("contingency", 0)
                ),
            ...
            "cumulative_summary": {
                ...
                "Previous Draws": self.format_currency(
                    cumulative_costs["previous_draws"]
                ),
                "Current Draw Request": self.format_currency(
                    sum(period_costs.values())
                ),
                "Total Draws to Date": self.format_currency(
                    cumulative_costs["total_to_date"]
                ),
                "Remaining Budget": self.format_currency(
                    remaining_budget["total_remaining"]
                ),
                "Percent Complete": self.format_percentage(
                    cumulative_costs["percent_complete"]
                ),
            },
            "loan_status": {
                "Total Loan Amount": self.format_currency(
                    loan_info.get("total_commitment", 0)
                ),
                "Amount Outstanding": self.format_currency(
                    loan_info.get("outstanding_balance", 0)
                ),
                "Available for Draw": self.format_currency(
                    loan_info.get("available_commitment", 0)
                ),
            ...
            "compliance": {
                ...
                "Loan Compliance": sum(period_costs.values())
                <= loan_info.get("available_commitment", float("inf")),
    ...
    def _get_draw_number(self, project, period: date) -> int:
        ...
        try:
            construction_start = min(
                item.timeline.start_date
                for item in project.construction_plan.capital_items
            )
            months_elapsed = (period.year - construction_start.year) * 12 + (
                period.month - construction_start.month
            )
        ...
        for item in self.project.construction_plan.capital_items:
            try:
                ...
                if any(
                    ...
                    term in work_type
                    for term in ["fee", "legal", "design", "architect"]
                ):
    ...
    def _calculate_cumulative_costs_to_date(self) -> Dict[str, float]:
        ...
        return {
            ...
            "percent_complete": (cumulative_amount + current_draw) / total_budget
            if total_budget > 0
            else 0,
        }
...
class LeasingStatusReport(Report):
    ...
    def generate_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "leasing_summary": {
                "Total Leasable Area": self._format_area(
                    self.project.net_rentable_area
                ),
                "Leased Area": self._format_area(leasing_metrics["leased_area"]),
                "Available Area": self._format_area(leasing_metrics["available_area"]),
                "Percent Leased": self.format_percentage(
                    leasing_metrics["occupancy_rate"]
                ),
            ...
            "rental_summary": {
                ...
                "Rent Achievement": self.format_percentage(
                    leasing_metrics["rent_achievement"]
                ),
                "Weighted Average Lease Term": f"{leasing_metrics['avg_lease_term']:.1f} years",
                "Annual Rent Roll": self.format_currency(
                    leasing_metrics["annual_rent_roll"]
                ),
            },
            "absorption_progress": {
                "Target Monthly Absorption": self._format_area(
                    leasing_metrics["monthly_absorption_target"]
                ),
                "Actual Monthly Absorption": self._format_area(
                    leasing_metrics["actual_monthly_absorption"]
                ),
                "Absorption vs Target": self.format_percentage(
                    leasing_metrics["absorption_performance"]
                ),
                "Pipeline (Under LOI)": self._format_area(
                    leasing_metrics["pipeline_area"]
                ),
                "Pipeline Value": self.format_currency(
                    leasing_metrics["pipeline_value"]
                ),
            },
