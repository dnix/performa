...
def compare_deal_configurations(
    deal1: "Deal", deal2: "Deal", include_defaults: bool = False
) -> Dict[str, Any]:
    """
...
def _extract_deal_configuration(deal: "Deal", include_defaults: bool) -> Dict[str, Any]:
    ...
    if deal.acquisition:
        config["acquisition"] = _extract_object_config(
            deal.acquisition, include_defaults
        )

    # Financing configuration
    if deal.financing:
        config["financing"] = _extract_financing_config_deep(
            deal.financing, include_defaults
        )
    ...
    if deal.equity_partners:
        config["partnership"] = _extract_object_config(
            deal.equity_partners, include_defaults
        )
            ...
            if len(value1) != len(value2):
                ...
                for i, (item1, item2) in enumerate(zip(value1, value2)):
                    if isinstance(item1, dict) and isinstance(item2, dict):
                        nested_diffs = _deep_compare_configs(
...
def _assess_difference_impact(differences: List[Dict[str, Any]]) -> str:
    ...
    high_impact_types = [
        ...
        "timeline",
        "refinance_timing",
        "duration_months",  # Added timeline-related
    ]
