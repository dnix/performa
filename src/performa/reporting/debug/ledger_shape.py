...
def compare_ledger_shapes(
    results1: "DealResults", results2: "DealResults"
) -> Dict[str, Any]:
    """
...
def _analyze_transaction_counts(ledger_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    subcategory_counts = (
        ledger_df.groupby("subcategory", observed=True).size().to_dict()
    )
...
def _compare_transaction_counts(
    ledger1: pd.DataFrame, ledger2: pd.DataFrame
) -> Dict[str, Any]:
    """Compare transaction counts between two ledgers."""
    counts1 = (
        ledger1.groupby(["category", "subcategory"], observed=True)
        .size()
        .reset_index(name="count1")
    )
    counts2 = (
        ledger2.groupby(["category", "subcategory"], observed=True)
        .size()
        .reset_index(name="count2")
    )
    ...
    return {
        ...
        "category_subcategory_diffs": significant.to_dict("records")
...
def _generate_shape_warnings(analysis: Dict[str, Any]) -> None:
    ...
    if total_count > 10000:
        ...
        recommendations.append(
            "Consider analysis timeline reduction if performance issues"
        )
    ...
    if timeline["transaction_frequency"] > 200:
        warnings.append(
            f"High transaction frequency: {timeline['transaction_frequency']:.1f} txns/period"
        )
        recommendations.append(
            "Verify component configurations for excessive transaction creation"
        )
...
def format_ledger_shape_comparison(comparison: Dict[str, Any]) -> str:
    ...
    if comparison["flow_amount_diff"]["has_significant_diffs"]:
        ...
        for diff in comparison["flow_amount_diff"]["significant_amount_diffs"][
            :5
        ]:  # Top 5
            subcategory = diff["subcategory"]
