...
def _generate_parity_summary(checks: Dict[str, Any], label1: str, label2: str) -> str:
    ...
    for metric, check in checks.items():
        ...
        if metric == "irr":
            lines.append(
                f"  IRR: {val1:.2%} vs {val2:.2%} ({diff_pct:.1f}% diff) {status}"
            )
        elif metric == "equity_multiple":
            lines.append(
                f"  EM: {val1:.2f}x vs {val2:.2f}x ({diff_pct:.1f}% diff) {status}"
            )
        elif metric == "equity_invested":
            lines.append(
                f"  Equity: ${val1:,.0f} vs ${val2:,.0f} ({diff_pct:.1f}% diff) {status}"
            )
        ...
        if not check["passes"]:
            if metric == "irr" and check["percentage"] > 5:
                recommendations.append(
                    "🎯 HIGH PRIORITY: Check exit cap rate and financing terms"
                )
            elif metric == "irr" and check["percentage"] > 1:
                recommendations.append(
                    "🎯 MEDIUM: Check timeline duration and operating assumptions"
                )
            elif metric == "equity_multiple" and check["percentage"] > 5:
                recommendations.append(
                    "🎯 HIGH PRIORITY: Check financing configuration and equity distributions"
                )
            elif metric == "equity_invested" and check["percentage"] > 10:
                recommendations.append(
                    "🎯 HIGH PRIORITY: Check financing auto-sizing and LTV calculations"
                )
...
def quick_parity_check(results1: "DealResults", results2: "DealResults") -> bool:
    ...
    return validation["passes"] and validation["parity_level"] in [
        "perfect",
        "excellent",
        "good",
    ]
