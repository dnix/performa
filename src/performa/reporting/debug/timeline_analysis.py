...
def compare_deal_timelines(deal1: "Deal", deal2: "Deal") -> Dict[str, Any]:
    ...
    for component in all_components:
        ...
        if timeline1.get('duration_months') != timeline2.get('duration_months'):
            diff = {
                ...
                'impact': 'high' if abs((timeline1.get('duration_months', 0) - 
                                      timeline2.get('duration_months', 0))) > 1 else 'medium'
            }
            result['differences'].append(diff)
            result['has_mismatches'] = True
    ...
    if result['has_mismatches']:
        duration_diffs = [d for d in result['differences'] if d['issue'] == 'duration_mismatch']
        if duration_diffs:
            max_diff = max(abs(d['deal1_value'] - d['deal2_value']) for d in duration_diffs if d['deal1_value'] and d['deal2_value'])
            result['summary'] = f"Timeline duration differences up to {max_diff} periods found"
            result['impact_assessment'] = f"Expected impact: ~{max_diff * 5}% transaction count difference"
...
def _extract_deal_timelines(deal: "Deal") -> Dict[str, Any]:
    ...
    if deal.financing and hasattr(deal.financing, 'facilities') and deal.financing.facilities:
        for i, facility in enumerate(deal.financing.facilities):
            ...
            if loan_term_months:
                timelines[f'financing_facility_{i}'] = {
                    ...
                    'amortization_months': getattr(facility, 'amortization_months', None),
