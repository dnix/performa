...
def validate_flow_reasonableness(
    ...
    validation = {
        'revenue_analysis': _validate_revenue_flows(ledger_df, deal_type, property_type),
        'expense_analysis': _validate_expense_flows(ledger_df, deal_type, property_type),
    ...
    for analysis_type in ['revenue_analysis', 'expense_analysis', 'financing_analysis', 'capital_analysis']:
    ...
    # Generate summary
    if validation['has_warnings']:
        validation['summary'] = f"{len(validation['warnings'])} flow reasonableness issues detected"
        validation['recommendations'] = _generate_flow_recommendations(validation['warnings'], deal_type)
    else:
...
def validate_aggregate_flows(results: "DealAnalysisResult") -> Dict[str, Any]:
    ...
    sources = {
        'loan_proceeds': ledger_df[ledger_df['subcategory'] == 'Loan Proceeds']['amount'].sum(),
        'equity_contributions': ledger_df[ledger_df['subcategory'] == 'Equity Contribution']['amount'].sum(),
        'revenue': category_flows.get('Revenue', 0),
        'exit_proceeds': ledger_df[ledger_df['subcategory'] == 'Other']['amount'].sum()  # Gross sale proceeds
    }
    
    uses = {
        'purchase_price': abs(ledger_df[ledger_df['subcategory'] == 'Purchase Price']['amount'].sum()),
        'operating_expenses': abs(category_flows.get('Expense', 0)),
        'debt_service': abs(ledger_df[ledger_df['subcategory'] == 'Interest Payment']['amount'].sum()) + 
                       abs(ledger_df[ledger_df['subcategory'] == 'Principal Payment']['amount'].sum()),
        'equity_distributions': abs(ledger_df[ledger_df['subcategory'] == 'Equity Distribution']['amount'].sum())
    }
    ...
    # Generate summary
    if validation['balance_issues']:
        validation['balance_summary'] = f"Balance issues detected: {balance_ratio:.1f}% imbalance"
...
def _validate_expense_flows(ledger_df, deal_type: str, property_type: str) -> Dict[str, Any]:
    """Validate expense flows against benchmarks."""
    expense_flows = ledger_df[ledger_df['category'] == 'Expense']['amount'].sum()
    
    analysis = {
        'total_expenses': abs(expense_flows),
        'warnings': []
    }
...
def _validate_financing_flows(ledger_df, deal_type: str) -> Dict[str, Any]:
    ...
    loan_proceeds = financing_flows[financing_flows['subcategory'] == 'Loan Proceeds']['amount'].sum()
    interest_payments = financing_flows[financing_flows['subcategory'] == 'Interest Payment']['amount'].sum() 
    principal_payments = financing_flows[financing_flows['subcategory'] == 'Principal Payment']['amount'].sum()
    prepayments = financing_flows[financing_flows['subcategory'] == 'Prepayment']['amount'].sum()
    ...
    if loan_proceeds > 0 and abs(prepayments) == 0 and deal_type in ['stabilized', 'value_add']:
        analysis['warnings'].append({
            'type': 'no_loan_payoff',
...
def _validate_capital_flows(ledger_df, deal_type: str) -> Dict[str, Any]:
    ...
    purchase_price = capital_flows[capital_flows['subcategory'] == 'Purchase Price']['amount'].sum()
    exit_proceeds = capital_flows[capital_flows['subcategory'] == 'Other']['amount'].sum()  # Usually gross sale
...
def _perform_balance_checks(sources: Dict[str, float], uses: Dict[str, float], metrics: Any) -> Dict[str, Any]:
    ...
    return {
        ...
        'balance_check_passes': abs((total_sources - total_uses) - getattr(metrics, 'net_profit', 0)) < 10000
    }


def _generate_flow_recommendations(warnings: List[Dict[str, Any]], deal_type: str) -> List[str]:
    ...
    if 'no_interest_on_loan' in warning_types:
        recommendations.append("🔍 Check debt facility configuration and compute_cf execution")
