...
def validate_valuation_exclusion(df: pd.DataFrame, method_name: str = "unknown") -> None:
    """
...
def generate_ledger_pivot_analysis(ledger_df: pd.DataFrame, top_n: int = 15) -> Dict[str, Any]:
    """
    ...
    # Category-level aggregation
    try:
        category_pivot = ledger_df.groupby('category', observed=True)['amount'].agg(['count', 'sum']).round(0)
        analysis['by_category'] = {}
        for cat, row in category_pivot.iterrows():
            analysis['by_category'][str(cat)] = {
                'entries': int(row['count']),
                'total': float(row['sum']),
                'percentage': float(row['sum'] / ledger_df['amount'].sum() * 100) if ledger_df['amount'].sum() != 0 else 0
            }
    ...
    try:
        subcat_pivot = ledger_df.groupby(['category', 'subcategory'], observed=True)['amount'].agg(['count', 'sum']).round(0)
        subcat_pivot['abs_sum'] = abs(subcat_pivot['sum'])
        subcat_pivot_sorted = subcat_pivot.sort_values('abs_sum', ascending=False).head(top_n)
    ...
    try:
        # Construction costs
        construction_mask = ledger_df['subcategory'].str.contains('Construction|Hard Costs|Soft Costs', case=False, na=False)
        semantic_totals['construction_costs'] = abs(ledger_df[construction_mask]['amount'].sum())
        
        # Acquisition costs
        acquisition_mask = ledger_df['subcategory'].isin(['Purchase Price', 'Closing Costs', 'Transaction Costs'])
        semantic_totals['acquisition_costs'] = abs(ledger_df[acquisition_mask]['amount'].sum())
        
        # Disposition proceeds
        disposition_mask = (ledger_df['subcategory'] == 'Other') & (ledger_df['amount'] > 0)
        semantic_totals['disposition_proceeds'] = ledger_df[disposition_mask]['amount'].sum()
        
        # Debt flows
        semantic_totals['debt_proceeds'] = ledger_df[ledger_df['subcategory'] == 'Loan Proceeds']['amount'].sum()
        debt_service_mask = ledger_df['subcategory'].isin(['Interest Payment', 'Principal Payment'])
        semantic_totals['debt_service'] = abs(ledger_df[debt_service_mask]['amount'].sum())
        
        # Equity flows
        semantic_totals['equity_contributions'] = ledger_df[ledger_df['subcategory'] == 'Equity Contribution']['amount'].sum()
        semantic_totals['equity_distributions'] = abs(ledger_df[ledger_df['subcategory'] == 'Equity Distribution']['amount'].sum())
    ...
    try:
        ...
        if semantic_totals.get('total_noi', 0) > 0 and semantic_totals.get('debt_service', 0) > 0:
            validation_metrics['dscr_implied'] = semantic_totals['total_noi'] / semantic_totals['debt_service']
        
        # Equity multiple implied (distributions / contributions)
        if semantic_totals.get('equity_contributions', 0) > 0:
            validation_metrics['equity_multiple_implied'] = semantic_totals.get('equity_distributions', 0) / semantic_totals['equity_contributions']
        
        # Net equity flow
        validation_metrics['net_equity_flow'] = semantic_totals.get('equity_distributions', 0) - semantic_totals.get('equity_contributions', 0)
        
        # Project cost total
        validation_metrics['total_project_cost'] = semantic_totals.get('acquisition_costs', 0) + semantic_totals.get('construction_costs', 0)
...
def analyze_cash_flow_timeline(results: "DealResults", deal_archetype: Optional[str] = None) -> Dict[str, Any]:
    """
...
def _analyze_cash_flow_signs(cash_flow: pd.Series, flow_name: str) -> Dict[str, Any]:
    ...
    return {
        ...
        'first_period_sign': 'positive' if cash_flow.iloc[0] > 0 else 'negative' if cash_flow.iloc[0] < 0 else 'zero',
        'last_period_sign': 'positive' if cash_flow.iloc[-1] > 0 else 'negative' if cash_flow.iloc[-1] < 0 else 'zero',
        ...
        'avg_positive': float(cash_flow[positive_periods].mean()) if positive_periods.any() else 0.0,
        'avg_negative': float(cash_flow[negative_periods].mean()) if negative_periods.any() else 0.0,
    }
...
def _analyze_sign_flips(ucf: pd.Series, lcf: pd.Series) -> Dict[str, Any]:
    ...
    for flow_name, flow in [('UCF', ucf), ('LCF', lcf)]:
        ...
        for i in range(1, len(signs)):
            if signs.iloc[i] != current_sign:
                phases.append({
                    'start': phase_start,
                    'end': flow.index[i - 1],
                    'sign': 'positive' if current_sign > 0 else 'negative' if current_sign < 0 else 'zero',
                    'periods': i - flow.index.get_loc(phase_start),
                    'total_flow': float(flow.iloc[flow.index.get_loc(phase_start):i].sum()),
                })
        ...
        sign_flips[flow_name] = {
            ...
            'total_flips': len(flip_periods) - 1,  # First one is always a "change" from nothing
            'phases': phases,
        }
...
def _validate_timeline_expectations(
    ...
    if operational_periods > 0:
        operational_ucf = ucf.iloc[construction_periods:construction_periods + operational_periods]
    ...
    if exit_periods > 0:
        ...
        validation['exit_phase'] = {
            ...
            'actual_sign': 'large_positive' if exit_ucf.max() > ucf.std() * 2 else 'positive' if exit_ucf.mean() > 0 else 'negative',
            'max_flow': float(exit_ucf.max()),
            'total_flow': float(exit_ucf.sum()),
            'periods_analyzed': exit_periods,
            'meets_expectation': exit_ucf.max() > ucf.std() * 2,  # Exit should be materially larger
        }
    ...
    validation['overall_pattern'] = {
        ...
        'lcf_follows_ucf': abs(ucf.sum() - lcf.sum()) < abs(ucf.sum()) * 0.5,  # LCF shouldn't be wildly different
    }
    
    return validation


def _check_reasonable_progression(ucf: pd.Series, deal_archetype: Optional[str]) -> bool:
...
def _generate_timeline_warnings(analysis: Dict[str, Any]) -> None:
    ...
    if ucf_analysis and not ucf_analysis.get('error'):
        ...
        if (analysis['basic_info'].get('deal_archetype') == 'development' and 
            ucf_analysis.get('first_period_sign') == 'positive'):
            warnings.append("Development UCF positive in first period - unusual, expect negative construction costs")
        
        # Warning: Last period negative
        if (ucf_analysis.get('last_period_sign') == 'negative' and 
            abs(ucf_analysis.get('last_period_value', 0)) > 1_000_000):
            warnings.append(f"UCF strongly negative in final period (${ucf_analysis.get('last_period_value', 0):,.0f}) - missing exit proceeds?")
        
        # Warning: No positive periods
        if ucf_analysis.get('positive_periods', 0) == 0:
            warnings.append("UCF never positive - no operating cash flows or exit proceeds detected")
    ...
    for phase_name, phase_data in timeline_validation.items():
        if isinstance(phase_data, dict) and 'meets_expectation' in phase_data:
            if not phase_data['meets_expectation']:
                ...
                warnings.append(f"{phase_name.title()}: Expected {expected} cash flows, got {actual}")
    ...
    if 'UCF' in sign_flips and 'phases' in sign_flips['UCF']:
        ...
        if len(phases) >= 3:
            insights.append(f"UCF shows {len(phases)} distinct phases: {' → '.join([p['sign'] for p in phases])}")
        elif len(phases) == 2:
            insights.append(f"UCF shows 2-phase pattern: {phases[0]['sign']} then {phases[1]['sign']}")
