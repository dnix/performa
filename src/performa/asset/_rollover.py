...
class RolloverLeaseTerms(Model):
    ...
    Attributes:
        term_months: The standard term length in months
    ...
    **Blended Terms (MARKET):**
        ...
        if isinstance(terms.market_rent, (int, float)):
            ...
            if terms.growth_rate:
                ...
                if as_of_date < growth_base_date:
                    logger.debug(
                        f"as_of_date {as_of_date} is before growth_base_date {growth_base_date}. No growth applied."
                    )
                ...
                try:
                    ...
                    logger.error(
                        f"Error creating period range for growth calculation: {e}"
                    )
                ...
                period_rates = pd.Series(
                    0.0, index=growth_periods
                )  # Initialize with 0% growth
                ...
                if isinstance(growth_value, (float, int)):
                    ...
                    if not isinstance(aligned_rates.index, pd.PeriodIndex):
                        try:
                            aligned_rates.index = pd.PeriodIndex(
                                aligned_rates.index, freq="M"
                            )
                        except Exception as e:
                            logger.error(
                                f"Growth Series index cannot be converted to PeriodIndex: {e}"
                            )
                    ...
                    try:
                        ...
                        logger.error(
                            f"Growth Dict keys cannot be converted to PeriodIndex: {e}"
                        )
                        return base_market_rent  # Cannot apply growth
                ...
                logger.debug(
                    f"Applied growth factor {cumulative_growth_factor:.4f} to base rent {base_market_rent:.2f} (Base Date: {growth_base_date}, As Of: {as_of_date})"
                )
                return grown_rent
                # --- End Implementation ---
            else:
            ...
            raise ValueError(
                "List format for market_rent requires a timeline context. Use a Series or dictionary instead."
            )
    ...
    def blend_lease_terms(self) -> RolloverLeaseTerms:
        ...
        **Downtime Assumption:**
        ...
        if isinstance(self.market_terms.market_rent, (int, float)) and isinstance(
            self.renewal_terms.market_rent, (int, float)
        ):
        ...
        elif isinstance(self.market_terms.market_rent, pd.Series) and isinstance(
            self.renewal_terms.market_rent, pd.Series
        ):
            # Align series and calculate weighted average
            renewal_reindexed = self.renewal_terms.market_rent.reindex(
                self.market_terms.market_rent.index.union(
                    self.renewal_terms.market_rent.index
                )
            )
            ...
            renewal_reindexed = renewal_reindexed.fillna(method="ffill").fillna(
                method="bfill"
            )
            market_reindexed = market_reindexed.fillna(method="ffill").fillna(
                method="bfill"
            )
        ...
        if (
            self.renewal_terms.growth_rate is not None
            and self.market_terms.growth_rate is not None
        ):
            blended_growth_rate = (
        ...
        if self.renewal_terms.rent_escalation and self.market_terms.rent_escalation:
            ...
            if (
                self.renewal_terms.rent_escalation.type
                == self.market_terms.rent_escalation.type
                and self.renewal_terms.rent_escalation.unit_of_measure
                == self.market_terms.rent_escalation.unit_of_measure
            ):
                # If types and units match, we can blend the amount
                blended_rent_escalation = self.market_terms.rent_escalation.model_copy(
                    deep=True
                )
        ...
        if self.renewal_terms.rent_abatement and self.market_terms.rent_abatement:
            ...
            blended_rent_abatement = self.market_terms.rent_abatement.model_copy(
                deep=True
            )
            ...
            blended_rent_abatement.months = round(
                ...
                + self.market_terms.rent_abatement.months
                * (1 - self.renewal_probability)
            )
            # Blend abatement ratio
            blended_rent_abatement.abated_ratio = (
                self.renewal_terms.rent_abatement.abated_ratio
                * self.renewal_probability
                + self.market_terms.rent_abatement.abated_ratio
                * (1 - self.renewal_probability)
            )
        ...
        if self.renewal_terms.ti_allowance and self.market_terms.ti_allowance:
            ...
            if (
                blended_ti.unit_of_measure
                == self.renewal_terms.ti_allowance.unit_of_measure
                and isinstance(blended_ti.value, (int, float))
                and isinstance(self.renewal_terms.ti_allowance.value, (int, float))
            ):
                blended_ti.value = (
                    self.renewal_terms.ti_allowance.value * self.renewal_probability
                    + self.market_terms.ti_allowance.value
                    * (1 - self.renewal_probability)
                )
        ...
        if (
            ...
            and self.market_terms.leasing_commission
        ):
            ...
            if hasattr(blended_lc, "rate") and hasattr(
                self.renewal_terms.leasing_commission, "rate"
            ):
                if isinstance(blended_lc.rate, (int, float)) and isinstance(
                    self.renewal_terms.leasing_commission.rate, (int, float)
                ):
                    blended_lc.rate = (
                        self.renewal_terms.leasing_commission.rate
                        * self.renewal_probability
                        + self.market_terms.leasing_commission.rate
                        * (1 - self.renewal_probability)
                    )
