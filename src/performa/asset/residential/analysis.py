...
class ResidentialAnalysisScenario(AnalysisScenarioBase):
    ...
    def prepare_models(self, context: AnalysisContext) -> List[CashFlowModel]:
        ...
        if hasattr(prop, "absorption_plans") and prop.absorption_plans:
            for absorption_plan in prop.absorption_plans:
                try:
                    ...
                    if hasattr(prop.unit_mix, 'vacant_units') and prop.unit_mix.vacant_units:
                    ...
                    logger.warning(f"Failed to process absorption plan '{absorption_plan.name}': {e}")
        ...
        if transformative_leases:
            logger.info(f"Found {len(transformative_leases)} leases for value-add transformation")
            ...
            for lease in transformative_leases:
                try:
                    ...
                    logger.warning(f"Failed to create post-renovation lease for {lease.name}: {e}")
    ...
    def _find_transformative_leases(self, models: List[CashFlowModel]) -> List[ResidentialLease]:
        """
        ...
        for model in models:
            if (isinstance(model, ResidentialLease) and 
                hasattr(model, 'rollover_profile') and 
                model.rollover_profile and
                model.rollover_profile.upon_expiration == UponExpirationEnum.REABSORB and
                model.rollover_profile.target_absorption_plan_id is not None):
                
                transformative_leases.append(model)
                logger.debug(f"Found transformative lease: {model.name} with target plan {model.rollover_profile.target_absorption_plan_id}")
    ...
    def _create_post_renovation_lease(
        ...
        new_lease_start_date = original_end_date + relativedelta(months=renovation_downtime + 1)
        ...
        post_reno_renewal_terms = ResidentialRolloverLeaseTerms(
            market_rent=lease_terms.monthly_rent or rollover_profile.renewal_terms.market_rent,
            term_months=lease_terms.lease_term_months or 12,
        ...
        stabilized_upon_expiration = lease_terms.upon_expiration or UponExpirationEnum.MARKET
        stabilized_renewal_probability = lease_terms.stabilized_renewal_probability or 0.7
        stabilized_downtime_months = lease_terms.stabilized_downtime_months or 1
        
        logger.debug(f"Post-renovation profile: {stabilized_upon_expiration}, renewal={stabilized_renewal_probability}, downtime={stabilized_downtime_months}")
        ...
        if not generated_specs:
            logger.warning(f"Absorption plan {target_plan.name} generated no unit specs for {original_lease.name}")
            return None
        ...
        post_renovation_lease = self._create_lease_from_unit_spec(
            unit_spec=renovated_unit_spec,
            unit_index=0,
            context=context
        )
        
        # Update lease name (immutable instance)
        post_renovation_lease = post_renovation_lease.model_copy(update={
            'name': f"{original_lease.name} (Post-Renovation)"
        })
        
        logger.info(f"Created post-renovation lease: {post_renovation_lease.name} starting {new_lease_start_date}")
        logger.debug(f"  Premium rent: ${post_renovation_lease.monthly_rent:,.0f}/month")
