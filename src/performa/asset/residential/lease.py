    ...
    def project_future_cash_flows(self, context: "AnalysisContext") -> pd.DataFrame:
        ...
        if self.rollover_profile and self.timeline.end_date < context.timeline.end_date:
            try:
                ...
                while (current_lease.rollover_profile and 
                       current_lease.timeline.end_date < context.timeline.end_date and 
                       renewal_count < max_renewals):
                    
                    next_lease = current_lease._create_speculative_lease_instance(context)
                    if not next_lease:
                        break
                ...
                logger.warning(f"Failed to create speculative lease for {self.name}: {e}")
