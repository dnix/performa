...
class Recovery(Model):
    ...
    base_amount_unit: Optional[Literal["total", "psf"]] = (
        "psf"  # Unit for base_amount (Total $ or $/SF)
    )
...
class RecoveryMethod(Model):
    ...
    def calculate_recoveries(
        ...
        logger.debug(
            f"Calculating recoveries for method '{self.name}' over timeline: {timeline.min()} to {timeline.max()}"
        )

        if self.gross_up and occupancy_rate is None:
            logger.warning(
                f"Gross-up enabled for '{self.name}' but occupancy_rate was not provided. Gross-up will be skipped."
            )
        ...
        for recovery in self.recoveries:
            ...
            for item in items_in_pool:
                ...
                variable_ratio = (
                    item.variable_ratio
                    if is_opex_item and item.variable_ratio is not None
                    else 0.0
                )

                logger.debug(
                    f"  Fetching raw CF for item: {item.name} ({item.model_id}), Variable: {variable_ratio:.1%}"
                )
                try:
                    raw_item_cf = lookup_fn(item.model_id)
                    if not isinstance(raw_item_cf, pd.Series):
                        logger.warning(
                            f"    Lookup for item {item.name} did not return a Series (type: {type(raw_item_cf)}). Attempting to create Series."
                        )
                        if isinstance(raw_item_cf, (int, float)):
                            raw_item_cf = pd.Series(float(raw_item_cf), index=timeline)
                        else:
                            raise TypeError(
                                "Cannot handle non-Series, non-scalar lookup result for expense item CF."
                            )

                except LookupError as e:
                    logger.error(
                        f"    Failed to lookup cash flow for expense item {item.name} ({item.model_id}). Skipping item. Error: {e}"
                    )
                    continue
                except Exception as e:
                    logger.error(
                        f"    Unexpected error processing item {item.name} ({item.model_id}). Skipping item. Error: {e}",
                        exc_info=True,
                    )
                    continue
                ...
                if is_eligible_for_grossup:
                    if occupancy_rate is None:
                        logger.debug(
                            f"    Skipping gross-up for item {item.name} as occupancy_rate is None."
                        )
                        ...
                        safe_occupancy = aligned_occupancy.where(
                            aligned_occupancy > 0, 0.0001
                        )
                        grossed_up_variable = variable_part / safe_occupancy

                        item_cf_to_add = fixed_part + variable_part.where(
                            ~needs_gross_up, grossed_up_variable
                        )

                        periods_grossed_up = needs_gross_up.sum()
                        if periods_grossed_up > 0:
                            logger.debug(
                                f"    Applied gross-up for item {item.name} ({periods_grossed_up} periods). Target: {target_occupancy:.1%}. Raw Sum: {raw_item_cf.sum():.2f}, Grossed-Up Sum: {item_cf_to_add.sum():.2f}"
                            )
                        ...
                        if current_occupancy < target_occupancy:
                            if current_occupancy <= 0:
                                logger.warning(
                                    f"    Occupancy rate is {current_occupancy} for item {item.name}. Gross-up calculation might yield unexpected results or errors. Clamping to 0.0001 for division."
                                )
                            ...
                            logger.debug(
                                f"    Applied gross-up for item {item.name} (constant occupancy {current_occupancy:.1%}). Target: {target_occupancy:.1%}. Raw Sum: {raw_item_cf.sum():.2f}, Grossed-Up Sum: {item_cf_to_add.sum():.2f}"
                            )
                        else:
                            logger.debug(
                                f"    Skipping gross-up for item {item.name} as constant occupancy {current_occupancy:.1%} >= target {target_occupancy:.1%}."
                            )
                    else:
                        logger.warning(
                            f"    Unsupported occupancy_rate type ({type(occupancy_rate)}) for gross-up calculation on item {item.name}."
                        )
            ...
            if expense_pool.pool_size_override is not None:
                if expense_pool.pool_size_override > 0:
                    ...
                    logger.debug(
                        f"  Using pool size override ({denominator}) as denominator."
                    )
                else:
                    logger.warning(
            ...
            if recovery.structure == "net":
                ...
                if annual_base_year_stop is None:
                    ...
                    logger.debug(
                        f"    Using Pre-calculated Annual Stop: {annual_base_year_stop:.2f}, Monthly Stop: {monthly_stop:.2f}"
                    )
                    ...
                    if (
                        global_settings
                        and hasattr(global_settings, "recoveries")
                        and hasattr(
                            global_settings.recoveries, "freeze_share_at_baseyear"
                        )
                    ):
                        freeze_share_flag = (
                            global_settings.recoveries.freeze_share_at_baseyear
                        )
                    elif global_settings and hasattr(
                        global_settings, "freeze_share_at_baseyear"
                    ):  # Check top level
                    ...
                    if freeze_share_flag and frozen_share is not None:
                        ...
                        logger.debug(
                            f"    Using frozen base year pro-rata share: {share_to_use:.4f}"
                        )
                    else:
            ...
            logger.debug(
                f"  Calculated base recovery CF sum (before admin/caps/floors): {recovery_cf.sum():.2f}"
            )
            ...
            logger.debug(
                f"  Recovery CF added. Cumulative total sum for method '{self.name}': {total_recoveries.sum():.2f}"
            )
