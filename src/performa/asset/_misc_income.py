...
class MiscIncome(CashFlowModel):
    ...
    def compute_cf(
        ...
        logger.debug(
            f"Computing cash flow for MiscIncome: '{self.name}' ({self.model_id})"
        )
        ...
        if self.reference is not None:
            if lookup_fn is None:
                raise ValueError(
                    f"Reference '{self.reference}' is set for MiscIncome '{self.name}', but no lookup_fn was provided."
                )
            ...
            if isinstance(looked_up_value, pd.Series):
                ...
                if self.unit_of_measure == UnitOfMeasureEnum.BY_PERCENT and isinstance(
                    ...
                    logger.warning(
                        f"MiscIncome '{self.name}' referenced aggregate series '{self.reference}' but UoM was '{self.unit_of_measure}'. Using standard calc."
                    )
                else:
                    raise TypeError(
                        f"MiscIncome '{self.name}' referenced aggregate series '{self.reference}' with unsupported UoM '{self.unit_of_measure}'."
                    )
                ...
                logger.debug(
                    f"  Base value derived from looked up series ref '{self.reference}'."
                )
            elif isinstance(looked_up_value, (float, int, str, date, dict)):
                calculated_flow = super().compute_cf(lookup_fn=lookup_fn)
                base_value_source = looked_up_value
                logger.debug(
                    f"  Base value derived from looked up scalar ref '{self.reference}': {base_value_source}"
                )
            else:
                raise TypeError(
                    f"MiscIncome '{self.name}' received unexpected type ({type(looked_up_value)}) from lookup_fn for ref '{self.reference}'."
                )
        else:
            calculated_flow = super().compute_cf(lookup_fn=lookup_fn)
        ...
        if self.growth_rate is not None:
            ...
            if hasattr(self, "_apply_compounding_growth"):
                effective_growth_start = (
                    self.growth_start_date
                    or self.timeline.start_date.to_timestamp().date()
                )
                logger.debug(
                ...
                logger.warning(
                    f"MiscIncome '{self.name}' has growth_rate but _apply_compounding_growth method not found."
                )
        ...
        if (
            occupancy_rate is not None
            and self.is_variable
            and self.variable_ratio is not None
        ):
            if pd.api.types.is_numeric_dtype(calculated_flow):
                logger.debug(
                    f"  Applying actual variable income adjustment (Ratio: {self.variable_ratio*100:.1f}%)"
                )
                fixed_ratio = 1.0 - self.variable_ratio
                variable_ratio = (
                    self.variable_ratio
                )  # Explicitly use self.variable_ratio
                ...
                if isinstance(occupancy_rate, pd.Series):
                    ...
                    adjustment_ratio = fixed_ratio + (
                        variable_ratio * aligned_occupancy
                    )
                    logger.debug(
                ...
                logger.warning(
                    f"Cannot apply occupancy adjustment to non-numeric series for MiscIncome '{self.name}'. Calculated flow type: {calculated_flow.dtype}"
                )
        elif self.is_variable and occupancy_rate is None:
            logger.warning(
                f"MiscIncome '{self.name}' is variable, but no occupancy_rate was provided for adjustment. Income calculated without variable adjustment."
            )

        logger.debug(
            f"Finished computing cash flow for MiscIncome: '{self.name}'. Final Sum: {calculated_flow.sum():.2f}"
        )
