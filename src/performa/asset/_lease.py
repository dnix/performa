...
# --- Lease Specification (Definition) ---
class LeaseSpec(Model):
    ...
    **Convention for Initial Vacancy:**
    ...
    rent_escalation: Optional[RentEscalation] = None # Reuse
    rent_abatement: Optional[RentAbatement] = None   # Reuse

    # Recoveries (Hold the actual RecoveryMethod instance)
    recovery_method: Optional[RecoveryMethod] = None # Reuse

    # Lease Costs (Hold the actual instances)
    ti_allowance: Optional[TenantImprovementAllowance] = None # Reuse
    leasing_commission: Optional[LeasingCommission] = None # Reuse

    # Rollover
    upon_expiration: UponExpirationEnum
    rollover_profile_ref: Optional[str] = None # e.g., ID/name of RolloverProfile
    ...
    def check_term(self) -> "LeaseSpec":
        ...
        if self.end_date and self.term_months:
            # Optional: Could calculate and validate consistency, or just prioritize one
            # For now, let computed fields handle it
            pass
        if self.end_date and self.end_date <= self.start_date:
             raise ValueError("end_date must be after start_date")
        # TODO: Add validation that if TI/LC/Recovery instances are provided,
        # their internal timelines/references might need adjustment or clearing
        # if they were copied from elsewhere?
        return self

    @computed_field
    @property
    def computed_end_date(self) -> date:
        """Calculates end date if term_months is provided."""
        if self.end_date:
            return self.end_date
        if self.term_months:
            # Assuming monthly frequency for term addition
            return (pd.Period(self.start_date, freq='M') + self.term_months - 1).end_time.date()
        raise ValueError("Cannot compute end_date without end_date or term_months") # Should be caught by validator

    @computed_field
    @property
    def computed_term_months(self) -> int:
        """Calculates term months if end_date is provided."""
        if self.term_months:
            return self.term_months
        if self.end_date:
            periods = pd.period_range(start=self.start_date, end=self.end_date, freq='M')
            return len(periods)
        raise ValueError("Cannot compute term_months without end_date or term_months") # Should be caught by validator
...
class Lease(CashFlowModel):
    ...
    # Rollover attributes
    upon_expiration: UponExpirationEnum
    rollover_profile: Optional["RolloverProfile"] = None # Used string hint
    ...
    def from_spec(
        ...
        # Determine status
        status = LeaseStatusEnum.CONTRACT if spec.start_date < analysis_start_date else LeaseStatusEnum.SPECULATIVE
        ...
        if spec.rollover_profile_ref and lookup_fn:
            try:
                ...
                if isinstance(fetched_profile, RolloverProfile):
                    ...
                    logger.warning(f"Lookup for rollover ref '{spec.rollover_profile_ref}' did not return RolloverProfile, got {type(fetched_profile)}")
            except Exception as e:
                logger.error(f"Failed to lookup rollover ref '{spec.rollover_profile_ref}': {e}")
        ...
        if spec.ti_allowance:
            try:
                ...
                ti_instance = spec.ti_allowance.model_copy(
                    deep=True,
                    update={"timeline": timeline, "reference": spec.area}
                )
                logger.debug(f"  Created context-specific TI instance for Lease '{spec.tenant_name} - {spec.suite}'")
            except Exception as e:
                 logger.error(f"  Failed to update context for TI Allowance: {e}", exc_info=True)
                 if cls.model_config.get('validate_assignment', True): raise
        ...
        if spec.leasing_commission:
            try:
                ...
                if spec.base_rent_unit_of_measure == UnitOfMeasureEnum.PSF:
                ...
                lc_instance = spec.leasing_commission.model_copy(
                    deep=True,
                    update={"timeline": timeline, "value": annual_rent}
                )
                logger.debug(f"  Created context-specific LC instance for Lease '{spec.tenant_name} - {spec.suite}'")
            except Exception as e:
                 logger.error(f"  Failed to update context for Leasing Commission: {e}", exc_info=True)
                 if cls.model_config.get('validate_assignment', True): raise
    ...
    def compute_cf(
        ...
        if isinstance(self.value, (int, float)):
             ...
             if self.unit_of_measure == UnitOfMeasureEnum.PSF:
                 ...
                 pass # Already monthly total amount
                  ...
                  raise NotImplementedError(f"Base rent unit {self.unit_of_measure} conversion not implemented in compute_cf")
        ...
        if self.recovery_method:
            if property_data is None:
                 logger.warning(f"Recovery method present for lease '{self.name}' but property_data not provided. Recoveries will be zero.")
            else:
    ...
    def _create_speculative_lease_instance(
        ...
        return Lease(
            ...
            unit_of_measure=UnitOfMeasureEnum.PSF, # Rollover likely assumes PSF?
    ...
    def project_future_cash_flows(
        ...
        if self.rollover_profile and self.lease_end < projection_end_date:
            ...
            profile = self.rollover_profile
            logger.debug(f"Lease '{self.name}' expires {self.lease_end}. Action: {action}. Projecting rollover...")
            ...
            next_lease_start_date = self.lease_end + pd.DateOffset(months=downtime_months)
            ...
            if downtime_months > 0:
                market_rent_at_rollover = profile._calculate_rent(
                    profile.market_terms, self.lease_end, global_settings=global_settings
                )
                ...
                if downtime_start_period <= downtime_end_period:
                     downtime_index = pd.period_range(start=downtime_start_period, end=downtime_end_period, freq='M')
                     vacancy_loss_series = pd.Series(market_rent_during_downtime, index=downtime_index)
            ...
            if action == UponExpirationEnum.RENEW:
                ...
                next_tenant = Tenant(id=f"Vacant-{self.suite}", name=f"Vacant - {self.suite}")
                next_name_suffix = " - Market"
                next_lease_type = LeaseTypeEnum.NET # Market typically Net?
            elif action == UponExpirationEnum.MARKET:
                next_lease_terms = profile.market_terms
                next_tenant = Tenant(id=f"Market-{self.suite}", name=f"Market - {self.suite}")
                next_name_suffix = " - Market"
                ...
                logger.warning(f"Unhandled UponExpirationEnum: {action} for lease '{self.name}'. Stopping projection.")
            ...
            logger.debug(f"Lease '{self.name}' has no rollover profile. Projection stops at lease end.")
             ...
             logger.debug(f"Lease '{self.name}' ends ({self.lease_end}) at or after projection end date ({projection_end_date}). No rollover needed.")
        ...
        if vacancy_loss_series is not None:
            ...
            combined_df['vacancy_loss'] = combined_df['vacancy_loss'].combine_first(vacancy_loss_series)
    ...
    def _instantiate_lease_costs_from_terms(
        ...
        if lease_terms.ti_allowance:
            ...
            ti_config = lease_terms.ti_allowance.model_copy(deep=True) # Copy to avoid modifying original RLA
...
# Ensure the module defining RolloverProfile is imported before Lease.model_rebuild
from . import _rollover 

# Resolve forward references for models defined in this file
Lease.model_rebuild()
LeaseSpec.model_rebuild() # LeaseSpec might use forward refs to components in this file
RentEscalation.model_rebuild() # If they themselves use forward refs
RentAbatement.model_rebuild()
# TenantImprovementAllowance and LeasingCommission are imported, their rebuilds happen in their own files.
# RecoveryMethod is imported, its rebuild happens in its own file. 