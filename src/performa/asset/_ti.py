    ...
    *total* TI amount, potentially based on a `reference` (e.g., to Property area
    ...
    def validate_amortization(self) -> "TenantImprovementAllowance":
        ...
        if self.payment_method == "amortized":
            ...
            if self.amortization_term_months is None:
                raise ValueError(
                    "amortization_term_months is required for amortized TI"
                )
        return self
    ...
    def compute_cf(
        ...
        if self.payment_method == "upfront":
            payment_date = (
                self.payment_date or self.timeline.start_date.to_timestamp().date()
            )
            ...
            logger.debug(
                f"  Upfront payment of {total_amount:.2f} scheduled for {payment_period}."
            )
            return ti_cf
        ...
        # If amortized, we calculate a loan-like payment schedule
            ...
            logger.debug(
                f"  Amortized payment calculated: {monthly_payment:.2f} per month for {self.amortization_term_months} months."
            )
            return ti_cf
