...
class CommercialRolloverProfileBase(RolloverProfileBase):
    ...
    def _calculate_rent(
        ...
        if isinstance(terms.market_rent, (int, float)):
            ...
            if isinstance(growth_value, (float, int)):
                ...
                aligned_rates = growth_value.reindex(growth_periods, method="ffill").fillna(0.0)
            ...
            if as_of_period in terms.market_rent.index:
                ...
                rent = terms.market_rent[earlier_periods[-1]] if len(earlier_periods) > 0 else terms.market_rent.iloc[0]
    ...
    def blend_lease_terms(self) -> CommercialRolloverLeaseTermsBase:
        ...
        blended_market_rent = (self.renewal_terms.market_rent * self.renewal_probability) + (self.market_terms.market_rent * market_prob)
        ...
        blended_ti = self.renewal_terms.ti_allowance if self.renewal_probability > 0.5 else self.market_terms.ti_allowance
        blended_lc = self.renewal_terms.leasing_commission if self.renewal_probability > 0.5 else self.market_terms.leasing_commission
