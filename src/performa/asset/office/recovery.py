...
class OfficeRecoveryMethod(CommercialRecoveryMethodBase):
    ...
    def calculate_recoveries(
        ...
        for recovery_item in self.recoveries:
            ...
            pool_cf = self._get_pool_cash_flow(
                recovery_item, timeline, occupancy_rate, lookup_fn, global_settings
            )

            pro_rata = tenant_area / (
                recovery_item.denominator or property_data.net_rentable_area
            )
            ...
            if recovery_item.structure == "net":
                ...
                monthly_stop = (
                    recovery_item.base_amount
                    * (
                        tenant_area
                        if recovery_item.base_amount_unit == "psf"
                        else pro_rata
                    )
                ) / 12
                ...
                recovery_cf = pd.Series(
                    (recovery_item.base_amount or 0) / 12, index=timeline
                )
            ...
            if recovery_item.recovery_ceiling:
                recovery_cf = recovery_cf.clip(
                    upper=recovery_item.recovery_ceiling / 12
                )
    ...
    def _get_pool_cash_flow(
        self, recovery_item, timeline, occupancy_rate, lookup_fn, global_settings
    ):
