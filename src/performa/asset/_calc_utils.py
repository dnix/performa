...
def get_period_occupancy(
    ...
    try:
        target_periods = pd.period_range(
            start=period_start, end=period_end, freq=frequency
        )
    ...
    if not initial_leases:
        ...
        return pd.Series(
            0.0, index=target_periods, name="Occupancy Rate"
        )  # Return zero occupancy series

    for lease in initial_leases:
        try:
            # Get the lease's own timeline periods (usually monthly)
            lease_periods = lease.timeline.period_index
            if lease_periods.empty:
                continue

            # Find periods where this lease is active *within the target period*
            active_periods_in_target = target_periods.intersection(
                lease_periods.asfreq(frequency, how="start")
            )  # Ensure frequency match
            ...
            if not active_periods_in_target.empty:
                ...
                logger.debug(
                    f"  Lease '{lease.name}' (Area: {lease.area}) active during {active_periods_in_target.min()} to {active_periods_in_target.max()}"
                )
        except Exception as e:
            logger.warning(
                f"Could not process lease '{lease.name}' for occupancy calculation. Error: {e}",
                exc_info=True,
            )
...
def get_period_expenses(
    ...
    """
    logger.debug(
        f"Calculating expenses for items {expense_item_ids} over period: {period_start} to {period_end}"
    )
    ...
    if property_data.expenses:
        if property_data.expenses.operating_expenses:
            all_expenses.extend(
                property_data.expenses.operating_expenses.expense_items or []
            )
        if property_data.expenses.capital_expenses:
            ...
            all_expenses.extend(
                property_data.expenses.capital_expenses.expense_items or []
            )

    target_items_map: Dict[UUID, ExpenseItem] = {
        item.model_id: item
        for item in all_expenses
        if item.model_id in expense_item_ids
    }
        ...
        if isinstance(key, str):
            if hasattr(property_data, key):
                ...
                if isinstance(value, (int, float, str, date)):
                    ...
                    logger.warning(
                        f"Simple lookup accessed property attribute '{key}' with complex type {type(value)}. Returning None."
                    )
                    return None
            else:
                # Could be an AggregateLineKey, but we don't resolve those here
                logger.debug(
                    f"Simple lookup cannot resolve string key '{key}' (not a direct property attribute)."
                )
                return None
        elif isinstance(key, UUID):
            logger.debug(
                f"Simple lookup does not resolve model UUIDs ({key}). Returning None."
            )
            return None
        else:
    ...
    for item_id, item in target_items_map.items():
        try:
            ...
            if not isinstance(full_cf.index, pd.PeriodIndex):
                logger.warning(
                    f"    Expense item '{item.name}' compute_cf did not return PeriodIndex. Attempting conversion."
                )
                try:
                    full_cf.index = pd.PeriodIndex(
                        full_cf.index, freq="M"
                    )  # Assume monthly base
                except Exception as e:
                    logger.error(
                        f"    Failed to convert index for item '{item.name}'. Skipping. Error: {e}"
                    )
                    continue
            ...
            target_periods_monthly = target_periods.asfreq(
                "M", how="start"
            )  # Use monthly for slicing base CF
            sliced_cf = full_cf[
                target_periods_monthly.min() : target_periods_monthly.max()
            ]
            ...
            logger.debug(
                f"    Successfully calculated and aligned CF for item '{item.name}'. Sum: {aligned_cf.sum():.2f}"
            )

        except Exception as e:
            logger.error(
                f"  Failed to calculate expenses for item '{item.name}' ({item_id}). Error: {e}",
                exc_info=True,
            )
    ...
    logger.debug(
        f"Finished calculating period expenses. Results for {len(results)} items."
    )
    return results
...
def gross_up_period_expenses(
    ...
    if occupancy_series is None or occupancy_series.empty:
        logger.warning(
            "Cannot perform gross-up without occupancy series. Returning raw expenses."
        )
        return raw_expenses
    ...
    for item_id, raw_cf in raw_expenses.items():
        ...
        if (
            item
            and isinstance(item, OpExItem)
            and item.is_recoverable
            and item.variable_ratio is not None
            and item.variable_ratio > 0
        ):
            ...
            if isinstance(timeline, pd.PeriodIndex) and not isinstance(
                occupancy_series.index, pd.PeriodIndex
            ):
                try:
                    occupancy_series.index = pd.PeriodIndex(
                        occupancy_series.index, freq=timeline.freq
                    )
                except Exception as e:
                    logger.error(
                        f"Failed to align occupancy index to PeriodIndex for gross-up of {item_id}. Skipping gross-up. Error: {e}"
                    )
                    grossed_up_results[item_id] = item_cf_to_add
                    continue

            aligned_occupancy = occupancy_series.reindex(
            ...
            if periods_grossed_up > 0:
                logger.debug(
                    f"    Util: Applied gross-up for item {item.name} ({periods_grossed_up} periods). Target: {gross_up_target_rate:.1%}. Raw Sum: {raw_cf.sum():.2f}, Grossed-Up Sum: {item_cf_to_add.sum():.2f}"
                )
