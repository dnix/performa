...
class AssetAnalysisResult:
    ...
    def summary_df(self) -> pd.DataFrame:
        ...
        summary_data = {
            UnleveredAggregateLineKey.POTENTIAL_GROSS_REVENUE.value: self._to_period_series(queries.pgr()),
            UnleveredAggregateLineKey.RENTAL_ABATEMENT.value: self._to_period_series(queries.rental_abatement()),
            UnleveredAggregateLineKey.MISCELLANEOUS_INCOME.value: self._to_period_series(queries.misc_income()),
            UnleveredAggregateLineKey.GENERAL_VACANCY_LOSS.value: self._to_period_series(queries.vacancy_loss()),
            UnleveredAggregateLineKey.CREDIT_LOSS.value: self._to_period_series(queries.credit_loss()),
            UnleveredAggregateLineKey.EXPENSE_REIMBURSEMENTS.value: self._to_period_series(queries.expense_reimbursements()),
            UnleveredAggregateLineKey.EFFECTIVE_GROSS_INCOME.value: self._to_period_series(queries.egi()),
            UnleveredAggregateLineKey.TOTAL_OPERATING_EXPENSES.value: self._to_period_series(queries.opex()),
            UnleveredAggregateLineKey.NET_OPERATING_INCOME.value: self._to_period_series(queries.noi()),
            UnleveredAggregateLineKey.TOTAL_CAPITAL_EXPENDITURES.value: self._to_period_series(queries.capex()),
            UnleveredAggregateLineKey.TOTAL_TENANT_IMPROVEMENTS.value: self._to_period_series(queries.ti()),
            UnleveredAggregateLineKey.TOTAL_LEASING_COMMISSIONS.value: self._to_period_series(queries.lc()),
            UnleveredAggregateLineKey.UNLEVERED_CASH_FLOW.value: self._to_period_series(queries.ucf()),
        }
    ...
    def summary_stats(self) -> dict:
        ...
        if not self.ledger.empty:
            queries = self.get_ledger_queries()
            stats['capital_uses'] = queries.total_uses().sum() if not queries.total_uses().empty else 0.0
            stats['capital_sources'] = queries.total_sources().sum() if not queries.total_sources().empty else 0.0
