...
def register_scenario(model_cls: Type) -> Callable:
    ...
    def decorator(
        scenario_cls: Type["AnalysisScenarioBase"],
    ) -> Type["AnalysisScenarioBase"]:
        if model_cls in SCENARIO_REGISTRY:
            raise ValueError(
                f"Scenario for model {model_cls.__name__} is already registered."
            )
...
def get_scenario_for_model(model: Model) -> Type["AnalysisScenarioBase"]:
    ...
    if scenario_cls is None:
        raise TypeError(
            f"No analysis scenario registered for model type {model_type.__name__}"
        )
