...
from pyxirr import npv, xnpv, irr, xirr, mirr, pmt, fv
import pandas as pd
...
from pyobsplot import Plot, Obsplot, js
...
class RevenueItem(CashFlowItem):  # use abstract class?
    ...
    revenue_multiplier: PositiveFloat  # sales: $/{unit,space}; rental: $/{unit,rsf,space}/mo
...
class RentalRevenueItem(RevenueItem):
    ...
    # TIMING
    active_duration: PositiveInt = 30 * 12  # default 30 years (in months) - effectively infinite
    # RENTAL GROWTH
    ...
    def initial_rent_rate(self) -> PositiveFloat:
        # compute multiplier based on multiplicand
        if self.revenue_multiplicand in ("Whole Unit", "Parking Space"):
            return self.revenue_multiplier * self.program.unit_count  # $/unit/mo -> $/mo
        elif self.revenue_multiplicand == "RSF":
    ...
    def revenue_df(self) -> pd.DataFrame:
        ...
        growth_factors[12::12] = 1 + self.revenue_growth_rate  # Apply growth factor at the beginning of each year after the first year
...
class Revenue(Model):
    ...
    # REVENUE
    revenue_items: list[Annotated[AnyRevenueItem, Field(..., discriminator='subcategory')]]
    ...
    @property
    def structural_losses_df(self) -> pd.Series:
        return pd.concat([item.structural_losses_df for item in self.revenue_items if hasattr(item, 'structural_losses_df')])
    ...
    def program_summary(self) -> pd.DataFrame:
        ...
        programs = list(set([item.program for item in self.revenue_items]))  # unique programs
        return pd.DataFrame([{
            "Use": program.use,
...
def compute_construction_financing(project_pt, debt_to_equity, financing_interest_rate, financing_fee_rate=0.02):
    ...
    # Applying financing fee only once when crossing the threshold from equity to debt
    df['Financing Fees'] = np.where(
        (df['Cumulative Costs'].shift(1, fill_value=0) < equity_portion) & (df['Cumulative Costs'] >= equity_portion),  # if crossing the threshold from equity to debt
        debt_portion * financing_fee_rate,  # calculate the financing fees
        0  # otherwise, no fees
    )
...
phases = {
    ...
    "Stabilization": (my_project.stabilization_timeline.min(), my_project.stabilization_timeline.max()),
    "Development": (my_project.development_timeline.min(), my_project.development_timeline.max()),
}
...
for i, (phase, periods) in enumerate(time_blocks):
    for start, duration, color in periods:
        ax.broken_barh([(start, duration)], (i - 0.3, 0.8), facecolors=(color), edgecolor='none')  # Removed edgecolor
...
# PLOT COSTS
my_project.construction_before_financing_cf.plot(kind="bar", stacked=True, title="Development Uses Cash Flow")

# PLOT CUMULATIVE COSTS
my_project.construction_before_financing_cf.cumsum().plot(kind="bar", stacked=True, title="Development Cumulative Uses Cash Flow")


# PLOT CONSTRUCTION FINANCING
my_project.construction_financing_cf[['Equity Draw', 'Debt Draw', 'Interest Reserve']][0:32].plot(kind="bar", stacked=True, title="Development Sources Cash Flow")
plt.show()  # Add this line to display the plot

# CONSTRUCTION SOURCES PIE CHART
my_project.construction_financing_cf[['Equity Draw', 'Debt Draw', 'Interest Reserve']][0:32].sum().plot(kind="pie", title="Development Sources")
plt.show()  # Add this line to display the plot
...
my_project.unlevered_cash_flow.resample("Y").sum().plot(kind="bar", stacked=True, title="Unlevered Annual Cash Flow")
...
my_project.levered_cash_flow.resample("Y").sum().plot(kind="bar", stacked=True, title="Levered Annual Cash Flow", color="purple")
...
construction_before_financing_cf_long = reshape_dataframe(my_project.construction_before_financing_cf)
construction_before_financing_cf_long_cumsum = reshape_dataframe(my_project.construction_before_financing_cf.cumsum())
construction_before_financing_cf_long_annual = reshape_dataframe(my_project.construction_before_financing_cf.resample("Y").sum())
